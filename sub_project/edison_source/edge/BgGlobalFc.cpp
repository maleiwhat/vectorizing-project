...
int bgSolveCubic(double a, double b, double c, double d, double& s1, double& s2,
                 double& s3)
{
	double p, q;
	double r, s, t, z;
...
void bgZoomIn(unsigned char** dest, unsigned char* src, int w, int h,
              int zconst, bool interpolate)
{
...
void bgZoomOut(unsigned char** dest, unsigned char* src, int w, int h,
               int zconst, bool interpolate)
{
	//if dest or src is NULL or zconst is <= 1 exit
	if ((!(*dest)) || (!src) || (zconst <= 1))
