...
//   return (EllipseEval(x,y)-EllipseEval(x0,y0));
	return ((x * x - x0 * x0) / (rankTr_ * rankTr_) + (y * y - y0 * y0) /
	        (confTr_ * confTr_));
}
	...
	for (i = WL_; i >= -WL_; i--)
	{
		for (j = -WL_; j <= WL_; j++)
		{
			...
			if (sflag != 0)
			{
				...
				if (lsigind == 1)
				{
					a[colind + rowind * WW_] = 0.5 * fabs(cordi[0][0] - cordi[0][1]) * fabs(
					                               cordi[1][0] - cordi[1][1]);
				}
				...
				if (lsigind == 2)
				{
					...
					if (diffin[0] == 1)
					{
						...
						a[colind + rowind * WW_] = 0.5 * (fabs(comcoor - cordi[1][0]) + fabs(
						                                      comcoor - cordi[1][1]));
					}
					...
					if (diffin[1] == 1)
					{
						...
						a[colind + rowind * WW_] = 0.5 * (fabs(comcoor - cordi[0][0]) + fabs(
						                                      comcoor - cordi[0][1]));
					}
				}
				...
				if (lsigind == 3)
				{
					a[colind + rowind * WW_] = 1.0 - 0.5 * fabs(cordi[0][0] - cordi[0][1]) * fabs(
					                               cordi[1][0] - cordi[1][1]);
				}
			}
		}
...
void BgEdgeDetect::CreateFilters(void)
{
	...
	for (i = -WL_; i <= WL_; i++)
	{
		w = pow((double)2, (double)(-2 * WL_)) * factorial(2 * WL_) / (factorial(
		            WL_ - i) * factorial(WL_ + i));
		smofil_[i + WL_] = w;
	...
	for (j = 0; j < WW_; j++)
	{
		for (i = 0; i < WW_; i++)
		{
			mQ_[i][j] = (smofil_[j] * smofil_[i]) / norms + (diffil_[j] * diffil_[i]) /
			            normd;
...
void BgEdgeDetect::GetPixels(int* nopix, int* pixx, int* pixy, double x1,
...
void BgEdgeDetect::GetNmxPixels(int* nopix, int* pixx, int* pixy, double x1,
                                double x2, double y1, double y2)
{
	double minx, maxx, miny, maxy;
                               ...
                               int nMin, int nmxType, int hystTypeHigh, int hystTypeLow)
{
	...
	confTr_ = (float) nmxc;
	rankTr_ = (float) nmxr;
	NewNonMaxSupress(permRank_, permConf_, permGx_, permGy_, permNmxRank_,
	                 permNmxConf_, fcomp);
...
*/
void BgEdgeDetect::DoEdgeDetect(BgImage* cim, BgEdgeList* cel, double nmxr,
                                double nmxc,
                                double rh, double ch, double rl, double cl,
                                int nMin, int nmxType, int hystTypeHigh, int hystTypeLow)
{
...
void BgEdgeDetect::GaussFilter(BgImage* cim, float* fim, double sigma,
                               int width)
{
	double* filter;
	unsigned char* im;
...
void BgEdgeDetect::GaussDiffFilter(BgImage* cim, float* grx, float* gry,
                                   float* rezIm)
{
...
void BgEdgeDetect::NewNonMaxSupress(float* rank, float* conf, float* grx,
                                    float* gry, float* nmxRank, float* nmxConf,
...
void BgEdgeDetect::NewHysteresisTr(float* edge, float* low, BgEdgeList* cel,
                                   int nMin, float* mark, float* coord)
{
	float* tm;
	float* te;
	float* tl;
