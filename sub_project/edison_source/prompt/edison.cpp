...
int EDISON::Save(char* filename, int filetype, int outputtype)
{
	...
	case OUTPUT_EDGES:
		...
		error = writeImage(filename, (unsigned char*) NULL, edges_, numEdges_,
		                   filetype);
		break;
	}
...
int EDISON::UseResult(int outputtype)
{
	...
	case OUTPUT_SEGM_IMAGE:
		...
		memcpy(inputImage_, segmImage_,
		       height_ * width_ * dim_ * sizeof(unsigned char));
		break;

	case OUTPUT_FILT_IMAGE:
		if (!filtImageDefined_) { return EXE_OUTPUT_UNDEFINED; }

		memcpy(inputImage_, filtImage_,
		       height_ * width_ * dim_ * sizeof(unsigned char));
		break;
...
int EDISON::writeImage(char* filename, unsigned char* image, int* dataPoints,
                       int n, int filetype)
{
...
int EDISON::EdgeDetect(void)
{
	...
	BgEdgeList   edgeList;
	edgeDetector.DoEdgeDetect(&inputImage, &edgeList, nmxRank, nmxConf,
	                          hystHighRank, hystHighConf,
...
int EDISON::ComputeWeightMap(void)
{
	...
	int   gradWindowRadius = *((int*)(
	                               parameterList_[PARAM_GRADIENT_WINDOW_RADIUS]));
	...
	//compute weight map
	if (!weightMapDefined_ || !gradMapDefined_ || !confMapDefined_
	        || CmCGradWinChanged || CmCMixtureChanged)
	{
