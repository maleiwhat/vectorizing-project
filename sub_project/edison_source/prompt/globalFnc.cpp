...
int checkFloatingPoint(char* data)
{
	...
	while (ch = *(dptr++)) if (((ch < 48) || (ch > 57)) && (ch != '.')
		                           && (ch != '-')) { return SYN_ASSIGN_FLOAT; }
...
int findMissing(void** paramTable, int cmd)
{
	...
	if (CmCSynergistic)
	{
		...
		for (paramId = PARAM_GRADIENT_WINDOW_RADIUS;
		        paramId <= PARAM_EDGE_STRENGTH_THRESHOLD; paramId++)
		{
			if (!paramTable[paramId]) { return paramId; }
		}
	}
	...
	while (token = script->GetToken())
	{
		...
		if (tokenId != UNKNOWN_COMMAND)
		{
			...
			if (error)
			{
				...
				{
					CmCToken* ertok = new CmCToken((char*)
					                               PARAM_LIST[findMissing(paramTable, Command(token->token_))],
					                               token->lineNumber_);
...
unsigned char* CmCConvertToGrayscale(unsigned char* image, int height,
                                     int width)
{
...
//write image data
int CmCWriteImage(char* filename, unsigned char* image, int height, int width,
                  int dim, int filetype)
{
	int error, i;
	unsigned char* data;
...
//read image data
int CmCReadImage(char* filename, unsigned char** image, int& height, int& width,
                 int& dim)
{
	if (!filename) { return EXE_NULL_PTR; }
