...
tree* MeanShift::BuildKDTree(tree* subset, int length, int d, tree* parent)
{
	...
	else if (length > 1)
	{
		...
		subset[median].left   = BuildKDTree(subset           , median         ,
		                                    (d + 1) % N, &subset[median]);
				...
				if (diff < offset[j])
				{
					...
					for (j = 0; j < kp; j++)
					{
						if (kernel[j]) // not uniform kernel
						{
							...
							tw *= (((double)(x1) * increment[j] - u) * y0 + (u - (double)(
							            x0) * increment[j]) * y1) / (double)(x1 * increment[j] - x0 * increment[j]);
						}
			...
			if (diff < offset[k - 1])
			{
				...
				for (k = 0; k < kp; k++)
				{
					if (kernel[k]) // not uniform kernel
					{
						...
						y1 = w[k][x1];
						// Accumulate tw using linear interpolation
						tw *= (((double)(x1) * increment[k] - u) * y0 + (u - (double)(
						            x0) * increment[k]) * y1) / (double)(x1 * increment[k] - x0 * increment[k]);
					}
