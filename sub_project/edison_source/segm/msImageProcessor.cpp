...
void msImageProcessor::NonOptimizedFilter(float sigmaS, float sigmaR)
{
	...
	for (i = 0; i < L; i++)
	{
		...
		if ((i % PROGRESS_RATE == 0)
		        && ((ErrorStatus = msSys.Progress((float)(i / (float)(L)) * (float)(
		                               0.8)))) == EL_HALT)
		{
			break;
		}
...
void msImageProcessor::InitializeOutput(void)
{
	...
	if ((!(modes = new float [L * (N + 2)])) || (!(modePointCounts = new int [L]))
...
{
	// Declare Variables
	int     iterationCount, i, j, k, modeCandidateX, modeCandidateY,
	        modeCandidate_i;
	double  mvAbs, diff, el;
