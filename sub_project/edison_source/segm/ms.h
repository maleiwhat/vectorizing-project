...
const int       LIMIT           =
    100;      // define max. # of iterations to find mode
...
// Numerical Analysis
const double    DELTA           =
    0.00001;  // used for floating point to integer conversion
...
protected:
	...
	void MSVector(double*,
	              double*);                     // Computes the mean shift vector at a specified
	...
	void OptLatticeMSVector(double*,
	                        double*);           // Uses the lattice defined by DefineLattice to compute the
	...
	void ErrorHandler(char*, char*,
	                  char*);              // flags an error and halts the system
	...
	msSystem
	msSys;                              // used for function timing and system output
	...
	float*
	data;                              // memory allocated for data points stored by tree nodes
	...
	////////Lattice Data Structure////////
	int             height,
	                width;                      // Height and width of lattice
	...
	float*
	offset;                            // defines bandwidth offset caused by the use of a Gaussian kernel
	// (for example)
	...
	int*
	pointList;                         // a list of data points that due to basin of attraction will
	// converge to the same mode as the mode that mean shift is
	// currently being applied to

	int
	pointCount;                         // the number of points stored by the point list
	...
	float*
	weightMap;                         // weight map that may be used to weight the kernel
	// upon performing mean shift on a lattice

	bool
	weightMapDefined;                   // used to indicate if a lattice weight map has been
	// defined
	...
	ClassStateStruct
	class_state;                    //specifies the state of the class(i.e if data has been loaded into
...
private:
	...
	void generateLookupTable(
	    void);                     // Generates Weight Function Lookup Table

	void DestroyKernel(
	    void);                           // Destroys mean shift kernel, re-initializes kernel
	...
	void CreateBST(
	    void);                                 // Upload input into a kd-BST

	void InitializeInput(
	    float*);                        // Allocates memory for and initializes the input data structure

	void ResetInput(
	    void);                                // de-allocate memory for and re-initialize input data structure
	// and mode structure
	...
	kernelType*
	kernel;                            // kernel types for each subspace S[i]

	double**
	w;                                // weight function lookup table

	double*
	increment;                         // increment used by weight hashing function

	bool
	uniformKernel;                      // flag used to indicate if the kernel is uniform or not

	userWeightFunct* head,
	                 *cur;                        // user defined weight function linked list
	...
	tree*
	root;                              // root of kdBST used to store input

	tree*
	forest;                            // memory allocated for tree nodes

	float*
	range;                             // range vector used to perform range search on kd tree, indexed
	...
	double*
	uv;                                // stores normalized distance vector between
	// yk and xi

	double
	wsum;                               // sum of weights calculated at data points within the sphere
	...
	////////Lattice Data Structure////////
	int             LowerBoundX,
	                UpperBoundX;           // Upper and lower bounds for lattice search window
	// in the x dimension

	int             LowerBoundY,
	                UpperBoundY;           // Upper and lower bounds for lattice search window
	// in the y dimension
