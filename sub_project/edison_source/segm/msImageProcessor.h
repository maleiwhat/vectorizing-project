...
private:
	...
	void Connect(
	    void);                      // classifies mean shift filtered image regions using
	// private classification structure of this class

	void Fill(int,
	          int);                    // used by Connect to perform label each region in the
	...
	void BuildRAM(
	    void);                     // build a region adjacency matrix using the region list
	// object

	void DestroyRAM(
	    void);               // destroy the region adjacency matrix: de-allocate its memory
	...
	void ComputeEdgeStrengths(
	    void);         // computes the weights of the weighted graph using the weight
	// map

	//Usage: Prune(minRegion)
	void Prune(
	    int);                        // use the RAM to prune the image of spurious regions (regions
	...
	void DefineBoundaries(
	    void);             // defines the boundaries of each region using the classified segmented
	...
	bool InWindow(int,
	              int);                //returns true if the range data of the specified data points
	//are within the defined search window (defined by kernel
	//bandwidth h[1])

	float SqDistance(int,
	...
	void InitializeOutput(
	    void);             //Allocates memory needed by this class to perform image
	//filtering and segmentation

	void DestroyOutput(
	    void);                //De-allocates memory needed by this class to perform image
	//filtering and segmentation
	...
	/////////Image Boundaries/////////
	RegionList*
	regionList;            // stores the boundary locations for each region

	/////////Image Regions////////
	int
	regionCount;            // stores the number of connected regions contained by the
	// image
	...
	float*
	LUV_data;              //stores modes in float format on lattice
	float
	LUV_treshold;        //in float mode this determines what "close" means between modes
	...
	float*
	msRawData;             // Raw data output of mean shift algorithm
	// to the location of the data point on the lattice

	////////Data Modes////////
	std::vector<int>
	labels;             // assigns a label to each data point associating it to
	// a mode in modes (e.g. a data point having label l has
	// mode modes[l])

	float*
	modes;                 // stores the mode data of the input data set, indexed by labels

	int*
	modePointCounts;       // stores for each mode the number of point mapped to that mode,
	// indexed by labels
	...
	RAList*
	raList;                // an array of RAList objects containing an entry for each
	// region of the image

	//////////RAMatrix Free List///////////
	RAList*
	freeRAList;            // a pointer to the head of a region adjacency list object
	// free list

	RAList*
	raPool;                // a pool of RAList objects used in the construction of the
	// RAM
	...
	////////Transitive Closure/////////
	float
	rR2;                    //defines square range radius used when clustering pixels
