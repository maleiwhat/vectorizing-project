...
//--------------------------------------------------------------------------------------
HRESULT CreateShapeMesh(ID3D10Device* pDev10, ID3DX10Mesh** ppMesh,
                        VERTEX* pVertices, UINT NumVertices, WORD* pIndices, UINT NumIndices)
{
	HRESULT hr = S_OK;
	// Create the mesh
                             ...
                             float fDepth, ID3DX10Mesh** ppMesh)
{
	...
	// Create a mesh
	hr = CreateShapeMesh(pDevice, ppMesh, pVertices, cVertices, pwIndices,
	                     cFaces * 3);
	// Free up the memory
    ...
    UINT                uStacks)
{
	...
	float fNormalXY = (fSideLength > 0.00001f) ? (fLength / fSideLength)       :
	                  1.0f;
	float fNormalZ  = (fSideLength > 0.00001f) ? (-fDeltaRadius / fSideLength) :
	                  0.0f;
	// Generate vertices
	VERTEX* pVertex = pVertices;
	float fZ, fRadius;
...
HRESULT WINAPI DXUTCreateCylinder(ID3D10Device* pDevice, float fRadius1,
                                  float fRadius2, float fLength, UINT uSlices, UINT uStacks, ID3DX10Mesh** ppMesh)
{
	HRESULT hr = S_OK;
	WORD* pwIndices   = NULL;
	VERTEX* pVertices = NULL;
