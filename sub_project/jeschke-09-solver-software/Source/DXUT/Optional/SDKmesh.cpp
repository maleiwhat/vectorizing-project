...
//--------------------------------------------------------------------------------------
void CDXUTSDKMesh::LoadMaterials(ID3D10Device* pd3dDevice,
                                 SDKMESH_MATERIAL* pMaterials, UINT numMaterials,
                                 SDKMESH_CALLBACKS10* pLoaderCallbacks)
{
	...
	else
	{
		for (UINT m = 0; m < numMaterials; m++)
		{
			...
			if (pMaterials[m].DiffuseTexture[0] != 0)
			{
				StringCchPrintfA(strPath, MAX_PATH, "%s%s", m_strPath,
			...
			{
				StringCchPrintfA(strPath, MAX_PATH, "%s%s", m_strPath,
				                 pMaterials[m].NormalTexture);

				if (FAILED(DXUTGetGlobalResourceCache().CreateTextureFromFile(pd3dDevice,
				           strPath, &pMaterials[m].pNormalRV10)))
				{
			...
			if (pMaterials[m].SpecularTexture[0] != 0)
			{
				...
				if (FAILED(DXUTGetGlobalResourceCache().CreateTextureFromFile(pd3dDevice,
				           strPath, &pMaterials[m].pSpecularRV10)))
				{
...
//--------------------------------------------------------------------------------------
void CDXUTSDKMesh::LoadMaterials(IDirect3DDevice9* pd3dDevice,
                                 SDKMESH_MATERIAL* pMaterials, UINT numMaterials,
                                 SDKMESH_CALLBACKS9* pLoaderCallbacks)
{
	...
	if (pLoaderCallbacks && pLoaderCallbacks->pCreateTextureFromFile)
	{
		for (UINT m = 0; m < numMaterials; m++)
		{
			...
			{
				pLoaderCallbacks->pCreateTextureFromFile(pd3dDevice,
				        pMaterials[m].DiffuseTexture, &pMaterials[m].pDiffuseTexture9,
				        pLoaderCallbacks->pContext);
			}
			...
			{
				pLoaderCallbacks->pCreateTextureFromFile(pd3dDevice,
				        pMaterials[m].SpecularTexture, &pMaterials[m].pSpecularTexture9,
				        pLoaderCallbacks->pContext);
			}
		}
	}
	else
	{
		for (UINT m = 0; m < numMaterials; m++)
		{
			...
			if (pMaterials[m].DiffuseTexture[0] != 0)
			{
				...
				if (FAILED(DXUTGetGlobalResourceCache().CreateTextureFromFile(pd3dDevice,
				           strPath, &pMaterials[m].pDiffuseTexture9)))
				{
			...
			if (pMaterials[m].NormalTexture[0] != 0)
			{
				...
				if (FAILED(DXUTGetGlobalResourceCache().CreateTextureFromFile(pd3dDevice,
				           strPath, &pMaterials[m].pNormalTexture9)))
				{
			...
			if (pMaterials[m].SpecularTexture[0] != 0)
			{
				...
				if (FAILED(DXUTGetGlobalResourceCache().CreateTextureFromFile(pd3dDevice,
				           strPath, &pMaterials[m].pSpecularTexture9)))
				{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTSDKMesh::CreateVertexBuffer(ID3D10Device* pd3dDevice,
        ...
        SDKMESH_CALLBACKS10* pLoaderCallbacks)
{
	...
	{
		pLoaderCallbacks->pCreateVertexBuffer(pd3dDevice, &pHeader->pVB10, bufferDesc,
		                                      pVertices, pLoaderCallbacks->pContext);
	}
	else
	{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTSDKMesh::CreateIndexBuffer(ID3D10Device* pd3dDevice,
                                        ...
                                        SDKMESH_CALLBACKS10* pLoaderCallbacks)
{
	...
	if (pLoaderCallbacks && pLoaderCallbacks->pCreateVertexBuffer)
	{
		pLoaderCallbacks->pCreateIndexBuffer(pd3dDevice, &pHeader->pIB10, bufferDesc,
		                                     pIndices, pLoaderCallbacks->pContext);
	}
	else
	{
		D3D10_SUBRESOURCE_DATA InitData;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTSDKMesh::CreateVertexBuffer(IDirect3DDevice9* pd3dDevice,
        SDKMESH_VERTEX_BUFFER_HEADER* pHeader, void* pVertices,
        SDKMESH_CALLBACKS9* pLoaderCallbacks)
{
	HRESULT hr = S_OK;
	...
	if (pLoaderCallbacks && pLoaderCallbacks->pCreateVertexBuffer)
	{
		pLoaderCallbacks->pCreateVertexBuffer(pd3dDevice, &pHeader->pVB9,
		                                      (UINT)pHeader->SizeBytes, D3DUSAGE_WRITEONLY, 0, D3DPOOL_DEFAULT, pVertices,
		                                      pLoaderCallbacks->pContext);
	}
	else
	{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTSDKMesh::CreateIndexBuffer(IDirect3DDevice9* pd3dDevice,
                                        SDKMESH_INDEX_BUFFER_HEADER* pHeader, void* pIndices,
                                        SDKMESH_CALLBACKS9* pLoaderCallbacks)
{
	HRESULT hr = S_OK;
	...
	{
		pLoaderCallbacks->pCreateIndexBuffer(pd3dDevice, &pHeader->pIB9,
		                                     (UINT)pHeader->SizeBytes, D3DUSAGE_WRITEONLY, ibFormat, D3DPOOL_DEFAULT,
		                                     pIndices, pLoaderCallbacks->pContext);
	}
	else
	{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTSDKMesh::CreateFromFile(ID3D10Device* pDev10,
                                     ...
                                     SDKMESH_CALLBACKS10* pLoaderCallbacks10, SDKMESH_CALLBACKS9* pLoaderCallbacks9)
{
	...
	V_RETURN(DXUTFindDXSDKMediaFileCch(m_strPathW,
	                                   sizeof(m_strPathW) / sizeof(WCHAR), szFileName));
	// Open the file
	m_hFile = CreateFile(m_strPathW, FILE_READ_DATA, FILE_SHARE_READ, NULL,
	...
	WideCharToMultiByte(CP_ACP, 0, m_strPathW, -1, m_strPath, MAX_PATH, NULL,
	                    FALSE);
	// Get the file size
                                       ...
                                       SDKMESH_CALLBACKS10* pLoaderCallbacks10, SDKMESH_CALLBACKS9* pLoaderCallbacks9)
{
	...
	m_pMeshArray = (SDKMESH_MESH*)(m_pStaticMeshData +
	                               m_pMeshHeader->MeshDataOffset);
	m_pSubsetArray = (SDKMESH_SUBSET*)(m_pStaticMeshData +
	                                   m_pMeshHeader->SubsetDataOffset);
	m_pFrameArray = (SDKMESH_FRAME*)(m_pStaticMeshData +
	                                 m_pMeshHeader->FrameDataOffset);
	m_pMaterialArray = (SDKMESH_MATERIAL*)(m_pStaticMeshData +
	                                       m_pMeshHeader->MaterialDataOffset);

	// Setup subsets
	for (UINT i = 0; i < m_pMeshHeader->NumMeshes; i++)
	{
		m_pMeshArray[i].pSubsets = (UINT*)(m_pStaticMeshData +
		                                   m_pMeshArray[i].SubsetOffset);
		m_pMeshArray[i].pFrameInfluences = (UINT*)(m_pStaticMeshData +
		                                   m_pMeshArray[i].FrameInfluenceOffset);
	}
	...
	BYTE* pBufferData = pData + m_pMeshHeader->HeaderSize +
	                    m_pMeshHeader->NonBufferDataSize;
	// Get the start of the buffer data
	UINT64 BufferDataStart = m_pMeshHeader->HeaderSize +
	                         m_pMeshHeader->NonBufferDataSize;
	...
	{
		BYTE* pVertices = NULL;
		pVertices = (BYTE*)(pBufferData + (m_pVertexBufferArray[i].DataOffset -
		                                   BufferDataStart));

		if (pDev10)
		{
			CreateVertexBuffer(pDev10, &m_pVertexBufferArray[i], pVertices,
			                   pLoaderCallbacks10);
		}
		else if (pDev9)
		{
			CreateVertexBuffer(pDev9, &m_pVertexBufferArray[i], pVertices,
			                   pLoaderCallbacks9);
		}
	}
	...
	{
		BYTE* pIndices = NULL;
		pIndices = (BYTE*)(pBufferData + (m_pIndexBufferArray[i].DataOffset -
		                                  BufferDataStart));

		if (pDev10)
		{
			CreateIndexBuffer(pDev10, &m_pIndexBufferArray[i], pIndices,
			                  pLoaderCallbacks10);
		}
		else if (pDev9)
		{
	...
	if (pDev10)
	{
		LoadMaterials(pDev10, m_pMaterialArray, m_pMeshHeader->NumMaterials,
		              pLoaderCallbacks10);
	}
	else if (pDev9)
	{
		LoadMaterials(pDev9, m_pMaterialArray, m_pMeshHeader->NumMaterials,
		              pLoaderCallbacks9);
	}
...
//--------------------------------------------------------------------------------------
void CDXUTSDKMesh::TransformFrame(UINT iFrame, D3DXMATRIX* pParentWorld,
                                  double fTime)
{
	// Get the tick data
	...
	{
		SDKANIMATION_FRAME_DATA* pFrameData =
		    &m_pAnimationFrameData[ m_pFrameArray[iFrame].AnimationDataIndex ];
                              ...
                              ID3D10EffectVectorVariable* pvSpecular)
{
	...
	pd3dDevice->IASetVertexBuffers(0, pMesh->NumVertexBuffers, pVB, Strides,
	                               Offsets);
                               ...
                               ID3D10EffectVectorVariable* pvSpecular)
{
	...
	if (m_pFrameArray[iFrame].ChildFrame != INVALID_FRAME)
	{
		RenderFrame(m_pFrameArray[iFrame].ChildFrame, bAdjacent, pd3dDevice, pTechnique,
		            ptxDiffuse, ptxNormal, ptxSpecular, pvDiffuse, pvSpecular);
	}

	// Render our siblings
	if (m_pFrameArray[iFrame].SiblingFrame != INVALID_FRAME)
	{
		RenderFrame(m_pFrameArray[iFrame].SiblingFrame, bAdjacent, pd3dDevice,
		            pTechnique, ptxDiffuse, ptxNormal, ptxSpecular, pvDiffuse, pvSpecular);
	}
}
                              ...
                              D3DXHANDLE htxSpecular)
{
	...
	for (UINT p = 0; p < cPasses; ++p)
	{
		...
		for (UINT subset = 0; subset < pMesh->NumSubsets; subset++)
		{
			...
			pd3dDevice->DrawIndexedPrimitive(PrimType, VertexStart, 0, VertexCount,
			                                 IndexStart, PrimCount);
		}
                               ...
                               D3DXHANDLE htxSpecular)
{
	...
	if (m_pFrameArray[iFrame].ChildFrame != INVALID_FRAME)
	{
		RenderFrame(m_pFrameArray[iFrame].ChildFrame, pd3dDevice, pEffect, hTechnique,
		            htxDiffuse, htxNormal, htxSpecular);
	}
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTSDKMesh::Create(ID3D10Device* pDev10, LPCTSTR szFileName,
                             bool bOptimize, bool bCreateAdjacencyIndices,
                             SDKMESH_CALLBACKS10* pLoaderCallbacks)
{
	return CreateFromFile(pDev10, NULL, szFileName, bOptimize,
	                      bCreateAdjacencyIndices, pLoaderCallbacks, NULL);
}

//--------------------------------------------------------------------------------------
HRESULT CDXUTSDKMesh::Create(IDirect3DDevice9* pDev9, LPCTSTR szFileName,
                             bool bOptimize, bool bCreateAdjacencyIndices,
                             SDKMESH_CALLBACKS9* pLoaderCallbacks)
{
	return CreateFromFile(NULL, pDev9, szFileName, bOptimize,
	                      bCreateAdjacencyIndices, NULL, pLoaderCallbacks);
}

//--------------------------------------------------------------------------------------
HRESULT CDXUTSDKMesh::Create(ID3D10Device* pDev10, BYTE* pData, UINT DataBytes,
                             bool bOptimize, bool bCreateAdjacencyIndices, bool bCopyStatic,
                             SDKMESH_CALLBACKS10* pLoaderCallbacks)
{
	return CreateFromMemory(pDev10, NULL, pData, DataBytes, bOptimize,
	                        bCreateAdjacencyIndices, bCopyStatic, pLoaderCallbacks, NULL);
}

//--------------------------------------------------------------------------------------
HRESULT CDXUTSDKMesh::Create(IDirect3DDevice9* pDev9, BYTE* pData,
                             UINT DataBytes, bool bOptimize, bool bCreateAdjacencyIndices, bool bCopyStatic,
                             SDKMESH_CALLBACKS9* pLoaderCallbacks)
{
	return CreateFromMemory(NULL, pDev9, pData, DataBytes, bOptimize,
	                        bCreateAdjacencyIndices, bCopyStatic, NULL, pLoaderCallbacks);
}
...
HRESULT CDXUTSDKMesh::LoadAnimation(WCHAR* szFileName)
{
	...
	// Open the file
	HANDLE hFile = CreateFile(strPath, FILE_READ_DATA, FILE_SHARE_READ, NULL,
	                          OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL);
	...
	if (!ReadFile(hFile, &fileheader, sizeof(SDKANIMATION_FILE_HEADER),
	              &dwBytesRead, NULL))
	{
		goto Error;
	}
	...
	//allocate
	m_pAnimationData = new
	BYTE[(size_t)(sizeof(SDKANIMATION_FILE_HEADER) + fileheader.AnimationDataSize) ];
	...
	if (!ReadFile(hFile, m_pAnimationData,
	              (DWORD)(sizeof(SDKANIMATION_FILE_HEADER) + fileheader.AnimationDataSize),
	              &dwBytesRead, NULL))
	{
		goto Error;
	}
	...
	m_pAnimationFrameData = (SDKANIMATION_FRAME_DATA*)(m_pAnimationData +
	...
	{
		m_pAnimationFrameData[i].pAnimationData = (SDKANIMATION_DATA*)(
		...
		if (m_pMaterialArray)
		{
			for (UINT64 m = 0; m < m_pMeshHeader->NumMaterials; m++)
			{
				...
				else
				{
					...
					if (m_pMaterialArray[m].pDiffuseRV10
					        && !IsErrorResource(m_pMaterialArray[m].pDiffuseRV10))
					{
					...
					if (m_pMaterialArray[m].pNormalRV10
					        && !IsErrorResource(m_pMaterialArray[m].pNormalRV10))
					{
					...
					if (m_pMaterialArray[m].pSpecularRV10
					        && !IsErrorResource(m_pMaterialArray[m].pSpecularRV10))
					{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTSDKMesh::CreateAdjacencyIndices(ID3D10Device* pd3dDevice,
        float fEpsilon, BYTE* pBufferData)
{
	HRESULT hr = S_OK;
	UINT IBIndex = 0;
	UINT VBIndex = 0;
	m_pAdjacencyIndexBufferArray = new
	SDKMESH_INDEX_BUFFER_HEADER[ m_pMeshHeader->NumIndexBuffers ];
	...
	for (UINT i = 0; i < m_pMeshHeader->NumMeshes; i++)
	{
		...
		BYTE* pVertices = (BYTE*)(pBufferData +
		                          m_pVertexBufferArray[VBIndex].DataOffset);
		...
		V_RETURN(pd3dDevice->CreateBuffer(&bufferDesc, &InitData,
		                                  &m_pAdjacencyIndexBufferArray[IBIndex].pIB10));
		//cleanup
		pIndexBuffer->Unmap();
...
void CDXUTSDKMesh::Render(ID3D10Device* pd3dDevice,
                          ...
                          ID3D10EffectVectorVariable* pvSpecular)
{
	RenderFrame(0, false, pd3dDevice, pTechnique, ptxDiffuse, ptxNormal,
...
void CDXUTSDKMesh::RenderAdjacent(ID3D10Device* pd3dDevice,
                                  ...
                                  ID3D10EffectVectorVariable* pvSpecular)
{
	RenderFrame(0, true, pd3dDevice, pTechnique, ptxDiffuse, ptxNormal, ptxSpecular,
	            pvDiffuse, pvSpecular);
}
...
void CDXUTSDKMesh::Render(LPDIRECT3DDEVICE9 pd3dDevice,
                          ...
                          D3DXHANDLE htxSpecular)
{
	RenderFrame(0, pd3dDevice, pEffect, hTechnique, htxDiffuse, htxNormal,
	            htxSpecular);
}

//--------------------------------------------------------------------------------------
D3D10_PRIMITIVE_TOPOLOGY CDXUTSDKMesh::GetPrimitiveType10(
    SDKMESH_PRIMITIVE_TYPE PrimType)
{
...
//--------------------------------------------------------------------------------------
D3DPRIMITIVETYPE CDXUTSDKMesh::GetPrimitiveType9(SDKMESH_PRIMITIVE_TYPE
        PrimType)
{
...
{
	return (UINT)
	       m_pVertexBufferArray[ m_pMeshArray[ iMesh ].VertexBuffers[iVB] ].StrideBytes;
}
...
UINT CDXUTSDKMesh::GetOutstandingResources()
{
	...
	if (m_pDev10)
	{
		for (UINT i = 0; i < m_pMeshHeader->NumMaterials; i++)
		{
			...
			{
				if (!m_pMaterialArray[i].pDiffuseRV10
				        && !IsErrorResource(m_pMaterialArray[i].pDiffuseRV10))
				{
			...
			{
				if (!m_pMaterialArray[i].pNormalRV10
				        && !IsErrorResource(m_pMaterialArray[i].pNormalRV10))
				{
					outstandingResources ++;
				}
			}
			...
			{
				if (!m_pMaterialArray[i].pSpecularRV10
				        && !IsErrorResource(m_pMaterialArray[i].pSpecularRV10))
				{
					outstandingResources ++;
				}
			}
	...
	else
	{
		for (UINT i = 0; i < m_pMeshHeader->NumMaterials; i++)
		{
			if (m_pMaterialArray[i].DiffuseTexture[0] != 0)
			{
				if (!m_pMaterialArray[i].pDiffuseTexture9
				        && !IsErrorResource(m_pMaterialArray[i].pDiffuseTexture9))
				{
					outstandingResources ++;
				}
			}
			...
			if (m_pMaterialArray[i].NormalTexture[0] != 0)
			{
				if (!m_pMaterialArray[i].pNormalTexture9
				        && !IsErrorResource(m_pMaterialArray[i].pNormalTexture9))
				{
					outstandingResources ++;
				}
			}
...
UINT CDXUTSDKMesh::GetOutstandingBufferResources()
{
	...
	for (UINT i = 0; i < m_pMeshHeader->NumVertexBuffers; i++)
	{
		if (!m_pVertexBufferArray[i].pVB9
		        && !IsErrorResource(m_pVertexBufferArray[i].pVB9))
		{
			outstandingResources ++;
		}
	}
	...
	for (UINT i = 0; i < m_pMeshHeader->NumIndexBuffers; i++)
	{
		if (!m_pIndexBufferArray[i].pIB9
		        && !IsErrorResource(m_pIndexBufferArray[i].pIB9))
		{
			outstandingResources ++;
		}
	}
