...
public:
	...
	void OnBegin(int nX, int
	             nY);    // start the rotation (pass current mouse position)
	void OnMove(int nX, int
	            nY);     // continue the rotation (pass current mouse position)
	...
	static D3DXQUATERNION WINAPI QuatFromBallPoints(const D3DXVECTOR3& vFrom,
...
public:
	...
	void SetClipToBoundary(bool bClipToBoundary, D3DXVECTOR3* pvMinBoundary,
	                       D3DXVECTOR3* pvMaxBoundary) { m_bClipToBoundary = bClipToBoundary; if (pvMinBoundary) { m_vMinBoundary = *pvMinBoundary; } if (pvMaxBoundary) { m_vMaxBoundary = *pvMaxBoundary; } }
	...
	D3DXVECTOR2
	m_vMouseDelta;          // Mouse relative delta smoothed over a few frames
...
protected:
	...
	BYTE
	m_aKeys[CAM_MAX_KEYS];  // State of input - KEY_WAS_DOWN_MASK|KEY_IS_DOWN_MASK
	...
	RECT
	m_rcDrag;               // Rectangle within which a drag can be initiated.
...
protected:
	D3DXMATRIX
	m_mCameraWorld;       // World matrix of the camera (inverse of the view matrix)

	int
	m_nActiveButtonMask;  // Mask to determine which button to enable for rotation
...
public:
	...
	void SetButtonMasks(int nRotateModelButtonMask = MOUSE_LEFT_BUTTON,
	                    int nZoomButtonMask = MOUSE_WHEEL,
	                    int nRotateCameraButtonMask = MOUSE_RIGHT_BUTTON) { m_nRotateModelButtonMask = nRotateModelButtonMask, m_nZoomButtonMask = nZoomButtonMask; m_nRotateCameraButtonMask = nRotateCameraButtonMask; }
	...
	void SetRadius(float fDefaultRadius = 5.0f, float fMinRadius = 1.0f,
	               float fMaxRadius = FLT_MAX) { m_fDefaultRadius = m_fRadius = fDefaultRadius; m_fMinRadius = fMinRadius; m_fMaxRadius = fMaxRadius; m_bDragSinceLastUpdate = true; }
...
protected:
	...
	bool
	m_bDragSinceLastUpdate; // True if mouse drag has happened since last time FrameMove is called.
...
public:
	...
	HRESULT OnRender9(D3DXCOLOR color, const D3DXMATRIX* pmView,
	...
	HRESULT OnRender10(D3DXCOLOR color, const D3DXMATRIX* pmView,
	                   const D3DXMATRIX* pmProj, const D3DXVECTOR3* pEyePt);
