...
//--------------------------------------------------------------------------------------
extern void DXUTGetCallbackD3D9DeviceAcceptable(
    LPDXUTCALLBACKISD3D9DEVICEACCEPTABLE* ppCallbackIsDeviceAcceptable,
    void** ppUserContext);
extern void DXUTGetCallbackD3D10DeviceAcceptable(
    LPDXUTCALLBACKISD3D10DEVICEACCEPTABLE* ppCallbackIsDeviceAcceptable,
    void** ppUserContext);

HRESULT DXUTFindValidD3D9DeviceSettings(DXUTD3D9DeviceSettings* pOut,
                                        DXUTD3D9DeviceSettings* pIn, DXUTMatchOptions* pMatchOptions,
                                        DXUTD3D9DeviceSettings* pOptimal);
void    DXUTBuildOptimalD3D9DeviceSettings(DXUTD3D9DeviceSettings*
        pOptimalDeviceSettings, DXUTD3D9DeviceSettings* pDeviceSettingsIn,
...
float   DXUTRankD3D9DeviceCombo(CD3D9EnumDeviceSettingsCombo*
                                pDeviceSettingsCombo, DXUTD3D9DeviceSettings* pOptimalDeviceSettings,
                                D3DDISPLAYMODE* pAdapterDesktopDisplayMode);
void    DXUTBuildValidD3D9DeviceSettings(DXUTD3D9DeviceSettings*
        pValidDeviceSettings, CD3D9EnumDeviceSettingsCombo* pBestDeviceSettingsCombo,
        DXUTD3D9DeviceSettings* pDeviceSettingsIn, DXUTMatchOptions* pMatchOptions);
HRESULT DXUTFindValidD3D9Resolution(CD3D9EnumDeviceSettingsCombo*
                                    pBestDeviceSettingsCombo, D3DDISPLAYMODE displayModeIn,
                                    D3DDISPLAYMODE* pBestDisplayMode);

HRESULT DXUTFindValidD3D10DeviceSettings(DXUTD3D10DeviceSettings* pOut,
        DXUTD3D10DeviceSettings* pIn, DXUTMatchOptions* pMatchOptions,
        DXUTD3D10DeviceSettings* pOptimal);
void    DXUTBuildOptimalD3D10DeviceSettings(DXUTD3D10DeviceSettings*
        pOptimalDeviceSettings, DXUTD3D10DeviceSettings* pDeviceSettingsIn,
        DXUTMatchOptions* pMatchOptions);
bool    DXUTDoesD3D10DeviceComboMatchPreserveOptions(
...
float   DXUTRankD3D10DeviceCombo(CD3D10EnumDeviceSettingsCombo*
                                 pDeviceSettingsCombo, DXUTD3D10DeviceSettings* pOptimalDeviceSettings,
                                 DXGI_MODE_DESC* pAdapterDisplayMode);
void    DXUTBuildValidD3D10DeviceSettings(DXUTD3D10DeviceSettings*
        pValidDeviceSettings, CD3D10EnumDeviceSettingsCombo* pBestDeviceSettingsCombo,
        DXUTD3D10DeviceSettings* pDeviceSettingsIn, DXUTMatchOptions* pMatchOptions);
HRESULT DXUTFindValidD3D10Resolution(CD3D10EnumDeviceSettingsCombo*
                                     pBestDeviceSettingsCombo, DXGI_MODE_DESC displayModeIn,
                                     DXGI_MODE_DESC* pBestDisplayMode);
...
//--------------------------------------------------------------------------------------
HRESULT WINAPI DXUTFindValidDeviceSettings(DXUTDeviceSettings* pOut,
        DXUTDeviceSettings* pIn, DXUTMatchOptions* pMatchOptions)
{
	HRESULT hr = S_OK;
	...
	if (pMatchOptions->eAPIVersion == DXUTMT_PRESERVE_INPUT && pIn
	        && pIn->ver == DXUT_D3D10_DEVICE)
	{
		bAvoidD3D9 = true;
	}

	if (pMatchOptions->eAPIVersion == DXUTMT_PRESERVE_INPUT && pIn
	        && pIn->ver == DXUT_D3D9_DEVICE)
	{
		bAvoidD3D10 = true;
	}
	...
	if (pMatchOptions->eAPIVersion != DXUTMT_IGNORE_INPUT && pIn
	        && pIn->ver == DXUT_D3D9_DEVICE)
	{
		bPreferD3D9 = true;
	}
	...
	if (bAppSupportsD3D10 && !bPreferD3D9 && !bAvoidD3D10)
	{
		...
		if (bD3D10Available)
		{
			...
			DXUTBuildOptimalD3D10DeviceSettings(&optimalDeviceSettings.d3d10, &d3d10In,
			                                    pMatchOptions);
	...
	if (bAppSupportsD3D9 && !bFoundValidD3D10 && !bAvoidD3D9)
	{
		...
		DXUTBuildOptimalD3D9DeviceSettings(&optimalDeviceSettings.d3d9, &d3d9In,
		                                   pMatchOptions);
...
CD3D9Enumeration* WINAPI DXUTGetD3D9Enumeration(bool bForceEnumerate)
{
	...
	if (!g_pDXUTD3D9Enumeration->HasEnumerated() || bForceEnumerate)
	{
		...
		void* pUserContext;
		DXUTGetCallbackD3D9DeviceAcceptable(&pCallbackIsDeviceAcceptable,
		                                    &pUserContext);
...
//--------------------------------------------------------------------------------------
HRESULT CD3D9Enumeration::Enumerate(LPDXUTCALLBACKISD3D9DEVICEACCEPTABLE
                                    IsD3D9DeviceAcceptableFunc,
                                    void* pIsD3D9DeviceAcceptableFuncUserContext)
{
	CDXUTPerfEventGenerator eventGenerator(DXUT_PERFEVENTCOLOR,
	                                       L"DXUT D3D9 Enumeration");
	...
	m_pIsD3D9DeviceAcceptableFuncUserContext =
	    pIsD3D9DeviceAcceptableFuncUserContext;
	HRESULT hr;
	ClearAdapterInfoList();
	...
	const UINT allowedAdapterFormatArrayCount  = sizeof(allowedAdapterFormatArray) /
	        sizeof(allowedAdapterFormatArray[0]);
	...
	for (UINT adapterOrdinal = 0; adapterOrdinal < numAdapters; adapterOrdinal++)
	{
		...
		for (UINT iFormatList = 0; iFormatList < allowedAdapterFormatArrayCount;
		        iFormatList++)
		{
			D3DFORMAT allowedAdapterFormat = allowedAdapterFormatArray[iFormatList];
			UINT numAdapterModes = pD3D->GetAdapterModeCount(adapterOrdinal,
			                       allowedAdapterFormat);

			for (UINT mode = 0; mode < numAdapterModes; mode++)
			{
				D3DDISPLAYMODE displayMode;
				pD3D->EnumAdapterModes(adapterOrdinal, allowedAdapterFormat, mode,
				                       &displayMode);
        ...
        CGrowableArray<D3DFORMAT>* pAdapterFormatList)
{
	...
	for (UINT iDeviceType = 0; iDeviceType < devTypeArrayCount; iDeviceType++)
	{
		...
		if (pDeviceInfo->DeviceType != D3DDEVTYPE_HAL)
		{
			...
			if (FAILED(hr = m_pD3D->CreateDevice(pAdapterInfo->AdapterOrdinal,
			                                     pDeviceInfo->DeviceType, DXUTGetHWNDFocus(),
...
//--------------------------------------------------------------------------------------
HRESULT CD3D9Enumeration::EnumerateDeviceCombos(CD3D9EnumAdapterInfo*
        pAdapterInfo, CD3D9EnumDeviceInfo* pDeviceInfo,
        CGrowableArray<D3DFORMAT>* pAdapterFormatList)
{
	...
	for (int iFormat = 0; iFormat < pAdapterFormatList->GetSize(); iFormat++)
	{
		...
		for (UINT iBackBufferFormat = 0; iBackBufferFormat < backBufferFormatArrayCount;
		        iBackBufferFormat++)
		{
			...
			for (int nWindowed = 0; nWindowed < 2; nWindowed++)
			{
				...
				if (FAILED(m_pD3D->CheckDeviceType(pAdapterInfo->AdapterOrdinal,
				                                   pDeviceInfo->DeviceType,
				...
				if (m_bRequirePostPixelShaderBlending)
				{
					...
					// are not supported.
					if (FAILED(m_pD3D->CheckDeviceFormat(pAdapterInfo->AdapterOrdinal,
					                                     pDeviceInfo->DeviceType,
				...
				{
					if (!m_IsD3D9DeviceAcceptableFunc(&pDeviceInfo->Caps, adapterFormat,
					                                  backBufferFormat, FALSE != nWindowed, m_pIsD3D9DeviceAcceptableFuncUserContext))
					{
						continue;
					}
				}
...
//--------------------------------------------------------------------------------------
void CD3D9Enumeration::BuildDepthStencilFormatList(CD3D9EnumDeviceSettingsCombo*
        pDeviceCombo)
{
...
//--------------------------------------------------------------------------------------
void CD3D9Enumeration::BuildMultiSampleTypeList(CD3D9EnumDeviceSettingsCombo*
        pDeviceCombo)
{
...
//       multisample types.
//--------------------------------------------------------------------------------------
void CD3D9Enumeration::BuildDSMSConflictList(CD3D9EnumDeviceSettingsCombo*
        pDeviceCombo)
{
	...
	for (int iDS = 0; iDS < pDeviceCombo->depthStencilFormatList.GetSize(); iDS++)
	{
		...
		for (int iMS = 0; iMS < pDeviceCombo->multiSampleTypeList.GetSize(); iMS++)
		{
			...
			if (FAILED(m_pD3D->CheckDeviceMultiSampleType(pDeviceCombo->AdapterOrdinal,
...
//--------------------------------------------------------------------------------------
void CD3D9Enumeration::BuildPresentIntervalList(CD3D9EnumDeviceInfo*
        pDeviceInfo,
...
//--------------------------------------------------------------------------------------
CD3D9EnumDeviceInfo* CD3D9Enumeration::GetDeviceInfo(UINT AdapterOrdinal,
...
//
//--------------------------------------------------------------------------------------
CD3D9EnumDeviceSettingsCombo* CD3D9Enumeration::GetDeviceSettingsCombo(
    UINT AdapterOrdinal, D3DDEVTYPE DeviceType, D3DFORMAT AdapterFormat,
...
CD3D9Enumeration::GetPossibleMultisampleTypeList()
{
...
//--------------------------------------------------------------------------------------
void CD3D9Enumeration::GetPossibleVertexProcessingList(bool* pbSoftwareVP,
        bool* pbHardwareVP, bool* pbPureHarewareVP, bool* pbMixedVP)
{
...
//--------------------------------------------------------------------------------------
void CD3D9Enumeration::SetPossibleVertexProcessingList(bool bSoftwareVP,
                                        ...
                                        DXUTD3D9DeviceSettings* pOptimal)
{
	...
	for (int iAdapter = 0; iAdapter < pAdapterList->GetSize(); iAdapter++)
	{
		        ...
		        iDeviceInfo++)
		{
			        ...
			        iDeviceCombo < pDeviceInfo->deviceSettingsComboList.GetSize(); iDeviceCombo++)
			{
				...
				if (false == DXUTDoesD3D9DeviceComboMatchPreserveOptions(pDeviceSettingsCombo,
				        pIn, pMatchOptions))
				{
					continue;
				}
				...
				float fCurRanking = DXUTRankD3D9DeviceCombo(pDeviceSettingsCombo, pOptimal,
				                    &adapterDesktopDisplayMode);
	...
	DXUTBuildValidD3D9DeviceSettings(&validDeviceSettings, pBestDeviceSettingsCombo,
	                                 pIn, pMatchOptions);
	*pOut = validDeviceSettings;
	return S_OK;
}
                                        ...
                                        DXUTMatchOptions* pMatchOptions)
{
	...
	if (pMatchOptions->eAdapterFormat == DXUTMT_IGNORE_INPUT)
	{
		...
		pD3D->GetAdapterDisplayMode(pOptimalDeviceSettings->AdapterOrdinal,
		                            &adapterDesktopDisplayMode);

		if (pOptimalDeviceSettings->pp.Windowed
		        || DXUTGetD3D9ColorChannelBits(adapterDesktopDisplayMode.Format) >= 8)
		{
	...
	{
		pOptimalDeviceSettings->pp.BackBufferWidth =
		    pDeviceSettingsIn->pp.BackBufferWidth;
		pOptimalDeviceSettings->pp.BackBufferHeight =
		    pDeviceSettingsIn->pp.BackBufferHeight;
	}
	...
	{
		pOptimalDeviceSettings->pp.BackBufferFormat =
		    pOptimalDeviceSettings->AdapterFormat;    // Default to match the adapter format
	}
	else
	{
		pOptimalDeviceSettings->pp.BackBufferFormat =
		    pDeviceSettingsIn->pp.BackBufferFormat;
	}
	...
	{
		pOptimalDeviceSettings->pp.BackBufferCount =
		    2;    // Default to triple buffering for perf gain
	}
	else
	{
		pOptimalDeviceSettings->pp.BackBufferCount =
		    pDeviceSettingsIn->pp.BackBufferCount;
	}
	...
	{
		pOptimalDeviceSettings->pp.MultiSampleType =
		    pDeviceSettingsIn->pp.MultiSampleType;
		pOptimalDeviceSettings->pp.MultiSampleQuality =
		    pDeviceSettingsIn->pp.MultiSampleQuality;
	}
	...
	{
		UINT nBackBufferBits = DXUTGetD3D9ColorChannelBits(
		                           pOptimalDeviceSettings->pp.BackBufferFormat);
	...
	{
		pOptimalDeviceSettings->pp.FullScreen_RefreshRateInHz =
		    pDeviceSettingsIn->pp.FullScreen_RefreshRateInHz;
	}
...
//--------------------------------------------------------------------------------------
bool DXUTDoesD3D9DeviceComboMatchPreserveOptions(CD3D9EnumDeviceSettingsCombo*
        ...
        DXUTMatchOptions* pMatchOptions)
{
	...
	if (pMatchOptions->eVertexProcessing == DXUTMT_PRESERVE_INPUT &&
	        ...
	        &&
	        ((pDeviceSettingsCombo->pDeviceInfo->Caps.DevCaps &
	...
	if (pMatchOptions->eResolution == DXUTMT_PRESERVE_INPUT)
	{
		...
		for (int i = 0;
		        i < pDeviceSettingsCombo->pAdapterInfo->displayModeList.GetSize(); i++)
		{
			D3DDISPLAYMODE displayMode =
			    pDeviceSettingsCombo->pAdapterInfo->displayModeList.GetAt(i);
	        ...
	        pMatchOptions->eStencilFormat == DXUTMT_PRESERVE_INPUT)
	{
		if (pDeviceSettingsIn->pp.AutoDepthStencilFormat != D3DFMT_UNKNOWN &&
		        !pDeviceSettingsCombo->depthStencilFormatList.Contains(
		            pDeviceSettingsIn->pp.AutoDepthStencilFormat))
		{
			return false;
		}
	...
	{
		bool bFound = false;
		UINT dwDepthBits = DXUTGetDepthBits(
		                       pDeviceSettingsIn->pp.AutoDepthStencilFormat);

		for (int i = 0; i < pDeviceSettingsCombo->depthStencilFormatList.GetSize(); i++)
		{
			D3DFORMAT depthStencilFmt = pDeviceSettingsCombo->depthStencilFormatList.GetAt(
			                                i);
	...
	{
		bool bFound = false;
		UINT dwStencilBits = DXUTGetStencilBits(
		                         pDeviceSettingsIn->pp.AutoDepthStencilFormat);
	...
	if (pMatchOptions->ePresentInterval == DXUTMT_PRESERVE_INPUT &&
	        !pDeviceSettingsCombo->presentIntervalList.Contains(
	            pDeviceSettingsIn->pp.PresentationInterval))
	{
		return false;
	}
                              ...
                              D3DDISPLAYMODE* pAdapterDesktopDisplayMode)
{
	...
	// Adapter ordinal
	//---------------------
	if (pDeviceSettingsCombo->AdapterOrdinal ==
	        pOptimalDeviceSettings->AdapterOrdinal)
	{
	...
	if (pDeviceSettingsCombo->AdapterFormat ==
	        pOptimalDeviceSettings->AdapterFormat)
	{
		fCurRanking += fAdapterFormatWeight;
	}
	else
	{
		int nBitDepthDelta = abs((long) DXUTGetD3D9ColorChannelBits(
		                             pDeviceSettingsCombo->AdapterFormat) -
	...
	if (!pDeviceSettingsCombo->Windowed)
	{
		...
		{
			bAdapterOptimalMatch = (pDeviceSettingsCombo->AdapterFormat ==
			                        pAdapterDesktopDisplayMode->Format);
		}
		else
		{
			bAdapterOptimalMatch = (pDeviceSettingsCombo->AdapterFormat ==
	...
	// Vertex processing
	//---------------------
	if ((pOptimalDeviceSettings->BehaviorFlags &
	        D3DCREATE_HARDWARE_VERTEXPROCESSING) != 0 ||
	        (pOptimalDeviceSettings->BehaviorFlags & D3DCREATE_MIXED_VERTEXPROCESSING) != 0)
	{
		if ((pDeviceSettingsCombo->pDeviceInfo->Caps.DevCaps &
		        D3DDEVCAPS_HWTRANSFORMANDLIGHT) != 0)
		{
	...
	for (int idm = 0;
	        idm < pDeviceSettingsCombo->pAdapterInfo->displayModeList.GetSize(); idm++)
	{
		D3DDISPLAYMODE displayMode =
		    pDeviceSettingsCombo->pAdapterInfo->displayModeList.GetAt(idm);
	...
	// Back buffer format
	//---------------------
	if (pDeviceSettingsCombo->BackBufferFormat ==
	        pOptimalDeviceSettings->pp.BackBufferFormat)
	{
	...
	else
	{
		int nBitDepthDelta = abs((long) DXUTGetD3D9ColorChannelBits(
		                             pDeviceSettingsCombo->BackBufferFormat) -
		                         (long) DXUTGetD3D9ColorChannelBits(
		                             pOptimalDeviceSettings->pp.BackBufferFormat));
		float fScale = __max(0.9f - (float)nBitDepthDelta * 0.2f, 0.0f);
	...
	bool bAdapterMatchesBB = (pDeviceSettingsCombo->BackBufferFormat ==
	                          pDeviceSettingsCombo->AdapterFormat);
	...
	// Depth stencil
	//---------------------
	if (pDeviceSettingsCombo->depthStencilFormatList.Contains(
	            pOptimalDeviceSettings->pp.AutoDepthStencilFormat))
	{
	...
	//---------------------
	// If keep present interval then check that the present interval is supported by this combo
	if (pDeviceSettingsCombo->presentIntervalList.Contains(
	            pOptimalDeviceSettings->pp.PresentationInterval))
	{
                                      ...
                                      DXUTMatchOptions* pMatchOptions)
{
	IDirect3D9* pD3D = DXUTGetD3D9Object();
	D3DDISPLAYMODE adapterDesktopDisplayMode;
	pD3D->GetAdapterDisplayMode(pBestDeviceSettingsCombo->AdapterOrdinal,
	                            &adapterDesktopDisplayMode);
	...
	else if (pMatchOptions->eVertexProcessing == DXUTMT_IGNORE_INPUT)
	{
		...
		if ((pBestDeviceSettingsCombo->pDeviceInfo->Caps.DevCaps &
		        D3DDEVCAPS_HWTRANSFORMANDLIGHT) != 0)
		{
	...
	else
	{
		...
		DXUTFindValidD3D9Resolution(pBestDeviceSettingsCombo, displayModeIn,
		                            &bestDisplayMode);
	}
	...
	else
	{
		...
		else if (pMatchOptions->eMultiSample == DXUTMT_CLOSEST_TO_INPUT)
		{
			...
			for (int i = 0; i < pBestDeviceSettingsCombo->multiSampleTypeList.GetSize();
			        i++)
			{
				D3DMULTISAMPLE_TYPE type = pBestDeviceSettingsCombo->multiSampleTypeList.GetAt(
				                               i);
				...
				if (abs(type - pDeviceSettingsIn->pp.MultiSampleType) < abs(
				            bestMultiSampleType - pDeviceSettingsIn->pp.MultiSampleType))
				{
					bestMultiSampleType = type;
					bestMultiSampleQuality = __min(qualityLevels - 1,
					                               pDeviceSettingsIn->pp.MultiSampleQuality);
				}
			}
		}
	...
	depthStencilRanking.SetSize(
	    pBestDeviceSettingsCombo->depthStencilFormatList.GetSize());
	UINT dwBackBufferBitDepth = DXUTGetD3D9ColorChannelBits(
	                                pBestDeviceSettingsCombo->BackBufferFormat);
	UINT dwInputDepthBitDepth = 0;

	if (pDeviceSettingsIn)
	{
		dwInputDepthBitDepth = DXUTGetDepthBits(
		                           pDeviceSettingsIn->pp.AutoDepthStencilFormat);
	}

	for (int i = 0; i < pBestDeviceSettingsCombo->depthStencilFormatList.GetSize();
	        i++)
	{
		D3DFORMAT curDepthStencilFmt =
		    pBestDeviceSettingsCombo->depthStencilFormatList.GetAt(i);
	...
	if (pDeviceSettingsIn)
	{
		dwInputStencilBitDepth = DXUTGetStencilBits(
		                             pDeviceSettingsIn->pp.AutoDepthStencilFormat);
	}
	...
	if (nBestIndex >= 0)
	{
		bestDepthStencilFormat = pBestDeviceSettingsCombo->depthStencilFormatList.GetAt(
		                             nBestIndex);
	...
	else
	{
		...
		else
		{
			...
			if (refreshRateMatch != 0)
			{
				...
				for (int iDisplayMode = 0; iDisplayMode < pDisplayModeList->GetSize();
				        iDisplayMode++)
				{
	...
	{
		if (pBestDeviceSettingsCombo->presentIntervalList.Contains(
		            pDeviceSettingsIn->pp.PresentationInterval))
		{
			bestPresentInterval = pDeviceSettingsIn->pp.PresentationInterval;
		}
	...
	pValidDeviceSettings->AdapterOrdinal                 =
	    pBestDeviceSettingsCombo->AdapterOrdinal;
	pValidDeviceSettings->DeviceType                     =
	    pBestDeviceSettingsCombo->DeviceType;
	pValidDeviceSettings->AdapterFormat                  =
	    pBestDeviceSettingsCombo->AdapterFormat;
	...
	pValidDeviceSettings->pp.BackBufferFormat            =
	    pBestDeviceSettingsCombo->BackBufferFormat;
	...
	pValidDeviceSettings->pp.hDeviceWindow               =
	    pBestDeviceSettingsCombo->Windowed ? DXUTGetHWNDDeviceWindowed() :
	    DXUTGetHWNDDeviceFullScreen();
	pValidDeviceSettings->pp.Windowed                    =
	    pBestDeviceSettingsCombo->Windowed;
	pValidDeviceSettings->pp.EnableAutoDepthStencil      =
	    bestEnableAutoDepthStencil;
	pValidDeviceSettings->pp.AutoDepthStencilFormat      = bestDepthStencilFormat;
	pValidDeviceSettings->pp.Flags                       = dwBestFlags;
	pValidDeviceSettings->pp.FullScreen_RefreshRateInHz  =
...
CD3D10Enumeration* WINAPI DXUTGetD3D10Enumeration(bool bForceEnumerate,
        bool bEnumerateAllAdapterFormats)
{
	// Using an static class with accessor function to allow control of the construction order
	...
	{
		g_pDXUTD3D10Enumeration->SetEnumerateAllAdapterFormats(
		    bEnumerateAllAdapterFormats, false);
		LPDXUTCALLBACKISD3D10DEVICEACCEPTABLE pCallbackIsDeviceAcceptable;
		void* pUserContext;
		DXUTGetCallbackD3D10DeviceAcceptable(&pCallbackIsDeviceAcceptable,
		                                     &pUserContext);
...
// through the ConfirmDevice callback.
//--------------------------------------------------------------------------------------
HRESULT CD3D10Enumeration::Enumerate(LPDXUTCALLBACKISD3D10DEVICEACCEPTABLE
                                     IsD3D10DeviceAcceptableFunc,
                                     void* pIsD3D10DeviceAcceptableFuncUserContext)
{
	CDXUTPerfEventGenerator eventGenerator(DXUT_PERFEVENTCOLOR,
	                                       L"DXUT D3D10 Enumeration");
	HRESULT hr;
	...
	m_pIsD3D10DeviceAcceptableFuncUserContext =
	    pIsD3D10DeviceAcceptableFuncUserContext;
	ClearAdapterInfoList();
	...
	for (int index = 0; ; ++index)
	{
		...
		if (FAILED(
		            hr))  // DXGIERR_NOT_FOUND is expected when the end of the list is hit
		{
			break;
		}
	...
	{
		pAdapterInfo = m_AdapterInfoList.GetAt(i);
		StringCchCopy(pAdapterInfo->szUniqueDescription, 100,
		              pAdapterInfo->AdapterDesc.Description);

		if (!bUniqueDesc)
		{
			WCHAR sz[100];
			StringCchPrintf(sz, 100, L" (#%d)", pAdapterInfo->AdapterOrdinal);
			StringCchCat(pAdapterInfo->szUniqueDescription,
			             DXGI_MAX_DEVICE_IDENTIFIER_STRING, sz);
		}
	}
...
//--------------------------------------------------------------------------------------
HRESULT CD3D10Enumeration::EnumerateDisplayModes(CD3D10EnumOutputInfo*
        pOutputInfo)
{
	HRESULT hr = S_OK;
	...
	int allowedAdapterFormatArrayCount  = sizeof(allowedAdapterFormatArray) /
	                                      sizeof(allowedAdapterFormatArray[0]);
...
HRESULT CD3D10Enumeration::EnumerateDevices(CD3D10EnumAdapterInfo* pAdapterInfo)
{
	...
	for (UINT iDeviceType = 0; iDeviceType < devTypeArrayCount; iDeviceType++)
	{
		...
		hr = DXUT_Dynamic_D3D10CreateDevice(pAdapter, devTypeArray[iDeviceType], 0,
...
//--------------------------------------------------------------------------------------
HRESULT CD3D10Enumeration::EnumerateDeviceCombos(IDXGIFactory* pFactory,
        CD3D10EnumAdapterInfo* pAdapterInfo)
{
				...
				for (int nWindowed = 0; nWindowed < 2; nWindowed++)
				{
					...
					if (m_IsD3D10DeviceAcceptableFunc != NULL)
					{
						if (!m_IsD3D10DeviceAcceptableFunc(pAdapterInfo->AdapterOrdinal, output,
						                                   pDeviceInfo->DeviceType, backBufferFormat, FALSE != nWindowed,
						                                   m_pIsD3D10DeviceAcceptableFuncUserContext))
						{
							continue;
						}
...
//--------------------------------------------------------------------------------------
void CD3D10Enumeration::SetEnumerateAllAdapterFormats(bool
        bEnumerateAllAdapterFormats, bool bEnumerateNow)
{
...
//--------------------------------------------------------------------------------------
void CD3D10Enumeration::BuildMultiSampleQualityList(DXGI_FORMAT fmt,
        CD3D10EnumDeviceSettingsCombo* pDeviceCombo)
{
	...
	{
		DXUTGetDXGIFactory()->EnumAdapters(pDeviceCombo->pAdapterInfo->AdapterOrdinal,
		                                   &pAdapter);
	}

	if (FAILED(DXUT_Dynamic_D3D10CreateDevice(pAdapter, pDeviceCombo->DeviceType, 0,
	           NULL, D3D10_SDK_VERSION, &pd3dDevice)))
	{
		return;
	}
...
//--------------------------------------------------------------------------------------
CD3D10EnumDeviceInfo* CD3D10Enumeration::GetDeviceInfo(UINT AdapterOrdinal,
        D3D10_DRIVER_TYPE DeviceType)
{
	...
	if (pAdapterInfo)
	{
		for (int iDeviceInfo = 0; iDeviceInfo < pAdapterInfo->deviceInfoList.GetSize();
		        iDeviceInfo++)
		{
			CD3D10EnumDeviceInfo* pDeviceInfo = pAdapterInfo->deviceInfoList.GetAt(
			                                        iDeviceInfo);
...
//--------------------------------------------------------------------------------------
CD3D10EnumOutputInfo* CD3D10Enumeration::GetOutputInfo(UINT AdapterOrdinal,
        UINT Output)
{
	CD3D10EnumAdapterInfo* pAdapterInfo = GetAdapterInfo(AdapterOrdinal);
...
//--------------------------------------------------------------------------------------
CD3D10EnumDeviceSettingsCombo* CD3D10Enumeration::GetDeviceSettingsCombo(
    UINT AdapterOrdinal, D3D10_DRIVER_TYPE DeviceType, UINT Output,
        ...
        DXUTD3D10DeviceSettings* pOptimal)
{
	...
	for (int iAdapter = 0; iAdapter < pAdapterList->GetSize(); iAdapter++)
	{
		...
		// Get the desktop display mode of adapter
		DXUTGetD3D10AdapterDisplayMode(pAdapterInfo->AdapterOrdinal, 0,
		                               &adapterDisplayMode);
		        ...
		        iDeviceCombo < pAdapterInfo->deviceSettingsComboList.GetSize(); iDeviceCombo++)
		{
			...
			// Skip any combo that doesn't meet the preserve match options
			if (false == DXUTDoesD3D10DeviceComboMatchPreserveOptions(pDeviceSettingsCombo,
			        pIn, pMatchOptions))
			{
				continue;
			}
			...
			// Get a ranking number that describes how closely this device combo matches the optimal combo
			float fCurRanking = DXUTRankD3D10DeviceCombo(pDeviceSettingsCombo, pOptimal,
			                    &adapterDisplayMode);
	...
	DXUTBuildValidD3D10DeviceSettings(&validDeviceSettings,
	                                  pBestDeviceSettingsCombo, pIn, pMatchOptions);
	*pOut = validDeviceSettings;
	return S_OK;
}
        ...
        DXUTMatchOptions* pMatchOptions)
{
	...
	DXUTGetD3D10AdapterDisplayMode(pOptimalDeviceSettings->AdapterOrdinal, 0,
	                               &adapterDesktopDisplayMode);
	...
	{
		pOptimalDeviceSettings->sd.BufferDesc.Width =
		    pDeviceSettingsIn->sd.BufferDesc.Width;
		pOptimalDeviceSettings->sd.BufferDesc.Height =
		    pDeviceSettingsIn->sd.BufferDesc.Height;
	}
	...
	if (pMatchOptions->eBackBufferFormat == DXUTMT_IGNORE_INPUT)
	{
		pOptimalDeviceSettings->sd.BufferDesc.Format =
		    adapterDesktopDisplayMode.Format;    // Default to match the adapter format
	}
	else
	{
		pOptimalDeviceSettings->sd.BufferDesc.Format =
		    pDeviceSettingsIn->sd.BufferDesc.Format;
	}
	...
	if (pMatchOptions->eBackBufferCount == DXUTMT_IGNORE_INPUT)
	{
		pOptimalDeviceSettings->sd.BufferCount =
		    2;    // Default to triple buffering for perf gain
	}
	else
	{
	...
	{
		pOptimalDeviceSettings->sd.SampleDesc.Count =
		    pDeviceSettingsIn->sd.SampleDesc.Count;
		pOptimalDeviceSettings->sd.SampleDesc.Quality =
		    pDeviceSettingsIn->sd.SampleDesc.Quality;
	}
	...
	{
		pOptimalDeviceSettings->AutoCreateDepthStencil =
		    pDeviceSettingsIn->AutoCreateDepthStencil;
		pOptimalDeviceSettings->AutoDepthStencilFormat =
		    pDeviceSettingsIn->AutoDepthStencilFormat;
	}
	...
	{
		pOptimalDeviceSettings->sd.BufferDesc.RefreshRate =
		    pDeviceSettingsIn->sd.BufferDesc.RefreshRate;
	}
...
// match options against the input pDeviceSettingsIn.
//--------------------------------------------------------------------------------------
bool DXUTDoesD3D10DeviceComboMatchPreserveOptions(CD3D10EnumDeviceSettingsCombo*
        pDeviceSettingsCombo,
        DXUTD3D10DeviceSettings* pDeviceSettingsIn,
        DXUTMatchOptions* pMatchOptions)
{
	...
	if (pMatchOptions->eResolution == DXUTMT_PRESERVE_INPUT)
	{
		...
		for (int i = 0;
		        i < pDeviceSettingsCombo->pOutputInfo->displayModeList.GetSize(); i++)
		{
			DXGI_MODE_DESC displayMode =
			    pDeviceSettingsCombo->pOutputInfo->displayModeList.GetAt(i);
	...
	if (pMatchOptions->eRefreshRate == DXUTMT_PRESERVE_INPUT)
	{
		        ...
		        i < pDeviceSettingsCombo->pOutputInfo->displayModeList.GetSize(); i++)
		{
			...
			if (fabs(float(displayMode.RefreshRate.Numerator) /
			         displayMode.RefreshRate.Denominator - float(
			             pDeviceSettingsIn->sd.BufferDesc.RefreshRate.Numerator) /
                               ...
                               DXGI_MODE_DESC* pAdapterDisplayMode)
{
	...
	for (int idm = 0;
	        idm < pDeviceSettingsCombo->pOutputInfo->displayModeList.GetSize(); idm++)
	{
		DXGI_MODE_DESC displayMode =
		    pDeviceSettingsCombo->pOutputInfo->displayModeList.GetAt(idm);
	...
	{
		int nBitDepthDelta = abs((long) DXUTGetDXGIColorChannelBits(
		                             pDeviceSettingsCombo->BackBufferFormat) -
                                       ...
                                       DXUTMatchOptions* pMatchOptions)
{
	...
	else
	{
		...
		// Call a helper function to find the closest valid display mode to the optimal
		DXUTFindValidD3D10Resolution(pBestDeviceSettingsCombo, displayModeIn,
		                             &bestDisplayMode);
	}
	...
	else
	{
		...
		else if (pMatchOptions->eMultiSample == DXUTMT_CLOSEST_TO_INPUT)
		{
			...
			for (int i = 0; i < pBestDeviceSettingsCombo->multiSampleCountList.GetSize();
			        i++)
			{
				...
				// Check whether supported type is closer to the input than our current best
				if (labs(Count - pDeviceSettingsIn->sd.SampleDesc.Count) < labs(
				            bestMultiSampleCount - pDeviceSettingsIn->sd.SampleDesc.Count))
				{
					bestMultiSampleCount = Count;
					bestMultiSampleQuality = __min(Quality - 1,
					                               pDeviceSettingsIn->sd.SampleDesc.Quality);
				}
			}
		}
...
//            if( refreshRateMatch != 0 )
	...
	pValidDeviceSettings->AdapterOrdinal                 =
	    pBestDeviceSettingsCombo->AdapterOrdinal;
	pValidDeviceSettings->Output                         =
	    pBestDeviceSettingsCombo->Output;
	pValidDeviceSettings->DriverType                     =
	    pBestDeviceSettingsCombo->DeviceType;
	...
	pValidDeviceSettings->sd.BufferDesc.Format           =
	    pBestDeviceSettingsCombo->BackBufferFormat;
	pValidDeviceSettings->sd.BufferUsage                 =
	    pDeviceSettingsIn->sd.BufferUsage | DXGI_USAGE_RENDER_TARGET_OUTPUT;
	...
	pValidDeviceSettings->sd.OutputWindow                =
	    pBestDeviceSettingsCombo->Windowed ? DXUTGetHWNDDeviceWindowed() :
	    DXUTGetHWNDDeviceFullScreen();
	pValidDeviceSettings->sd.Windowed                    =
	    pBestDeviceSettingsCombo->Windowed;
	...
	pValidDeviceSettings->AutoCreateDepthStencil         =
	    bestEnableAutoDepthStencil;
	pValidDeviceSettings->AutoDepthStencilFormat         = bestDepthStencilFormat;
	pValidDeviceSettings->CreateFlags                    =
	    pDeviceSettingsIn->CreateFlags;
}
...
//--------------------------------------------------------------------------------------
HRESULT WINAPI DXUTGetD3D10AdapterDisplayMode(UINT AdapterOrdinal, UINT nOutput,
        DXGI_MODE_DESC* pModeDesc)
{
	...
	CD3D10Enumeration* pD3DEnum = DXUTGetD3D10Enumeration();
	CD3D10EnumOutputInfo* pOutputInfo = pD3DEnum->GetOutputInfo(AdapterOrdinal,
	                                    nOutput);
	...
	if (pOutputInfo)
	{
		...
		pModeDesc->Height = Desc.DesktopCoordinates.bottom -
