...
protected:
	struct STATE
	{
		...
		// D3D10 specific
		bool
		m_D3D10Available;          // if true, then D3D10 is available
		IDXGIFactory*           m_DXGIFactory;             // DXGI Factory object
		IDXGIAdapter*
		m_D3D10Adapter;            // The DXGI adapter object for the D3D10 device
		IDXGIOutput**
		m_D3D10OutputArray;        // The array of output obj for the D3D10 adapter obj
		UINT
		m_D3D10OutputArraySize;    // Number of elements in m_D3D10OutputArray
		ID3D10Device*           m_D3D10Device;             // the D3D10 rendering device
		IDXGISwapChain*         m_D3D10SwapChain;          // the D3D10 swapchain
		ID3D10Texture2D*
		m_D3D10DepthStencil;       // the D3D10 depth stencil texture (optional)
		ID3D10DepthStencilView*
		m_D3D10DepthStencilView;   // the D3D10 depth stencil view (optional)
		ID3D10RenderTargetView*
		m_D3D10RenderTargetView;   // the D3D10 render target view
		DXGI_SURFACE_DESC
		m_BackBufferSurfaceDesc10; // D3D10 back buffer surface description
		bool
		...
		LPDXUTCALLBACKISD3D10DEVICEACCEPTABLE
		m_IsD3D10DeviceAcceptableFunc;  // D3D10 is device acceptable callback
		LPDXUTCALLBACKD3D10DEVICECREATED
		m_D3D10DeviceCreatedFunc;       // D3D10 device created callback
		LPDXUTCALLBACKD3D10SWAPCHAINRESIZED
		m_D3D10SwapChainResizedFunc;    // D3D10 SwapChain reset callback
		LPDXUTCALLBACKD3D10SWAPCHAINRELEASING
		m_D3D10SwapChainReleasingFunc;  // D3D10 SwapChain lost callback
		LPDXUTCALLBACKD3D10DEVICEDESTROYED
		m_D3D10DeviceDestroyedFunc;     // D3D10 device destroyed callback
		LPDXUTCALLBACKD3D10FRAMERENDER
		m_D3D10FrameRenderFunc;         // D3D10 frame render callback
...
public:
	...
	GET_SET_ACCESSOR(LPDXUTCALLBACKISD3D10DEVICEACCEPTABLE,
	                 IsD3D10DeviceAcceptableFunc);
	GET_SET_ACCESSOR(LPDXUTCALLBACKD3D10DEVICECREATED, D3D10DeviceCreatedFunc);
	GET_SET_ACCESSOR(LPDXUTCALLBACKD3D10SWAPCHAINRESIZED,
	                 D3D10SwapChainResizedFunc);
	GET_SET_ACCESSOR(LPDXUTCALLBACKD3D10SWAPCHAINRELEASING,
	                 D3D10SwapChainReleasingFunc);
...
bool    DXUTCanDeviceBeReset(DXUTDeviceSettings* pOldDeviceSettings,
                             DXUTDeviceSettings* pNewDeviceSettings, IDirect3DDevice9* pd3d9DeviceFromApp,
                             ID3D10Device* pd3d10DeviceFromApp);
HRESULT DXUTDelayLoadDXGI();
HRESULT DXUTDelayLoadD3D9();
void    DXUTUpdateDeviceSettingsWithOverrides(DXUTDeviceSettings*
        pDeviceSettings);
...
HRESULT DXUTFindD3D9AdapterFormat(UINT AdapterOrdinal, D3DDEVTYPE DeviceType,
                                  D3DFORMAT BackBufferFormat, BOOL Windowed, D3DFORMAT* pAdapterFormat);

HRESULT DXUTSetupD3D10Views(ID3D10Device* pd3dDevice,
...
void    DXUTUpdateD3D10DeviceStats(D3D10_DRIVER_TYPE DeviceType,
...
void WINAPI DXUTSetCallbackD3D10DeviceAcceptable(
    LPDXUTCALLBACKISD3D10DEVICEACCEPTABLE pCallback, void* pUserContext)        { GetDXUTState().SetIsD3D10DeviceAcceptableFunc(pCallback); GetDXUTState().SetIsD3D10DeviceAcceptableFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackD3D10DeviceCreated(LPDXUTCALLBACKD3D10DEVICECREATED
        pCallback, void* pUserContext)                { GetDXUTState().SetD3D10DeviceCreatedFunc(pCallback); GetDXUTState().SetD3D10DeviceCreatedFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackD3D10SwapChainResized(
    LPDXUTCALLBACKD3D10SWAPCHAINRESIZED pCallback, void* pUserContext)          { GetDXUTState().SetD3D10SwapChainResizedFunc(pCallback);  GetDXUTState().SetD3D10SwapChainResizedFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackD3D10FrameRender(LPDXUTCALLBACKD3D10FRAMERENDER
        pCallback, void* pUserContext)                    { GetDXUTState().SetD3D10FrameRenderFunc(pCallback);  GetDXUTState().SetD3D10FrameRenderFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackD3D10SwapChainReleasing(
    LPDXUTCALLBACKD3D10SWAPCHAINRELEASING pCallback, void* pUserContext)      { GetDXUTState().SetD3D10SwapChainReleasingFunc(pCallback);  GetDXUTState().SetD3D10SwapChainReleasingFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackD3D10DeviceDestroyed(
    LPDXUTCALLBACKD3D10DEVICEDESTROYED pCallback, void* pUserContext)            { GetDXUTState().SetD3D10DeviceDestroyedFunc(pCallback);  GetDXUTState().SetD3D10DeviceDestroyedFuncUserContext(pUserContext); }
void DXUTGetCallbackD3D10DeviceAcceptable(LPDXUTCALLBACKISD3D10DEVICEACCEPTABLE*
...
//--------------------------------------------------------------------------------------
HRESULT WINAPI DXUTInit(bool bParseCommandLine, bool bShowMsgBoxOnError,
                        WCHAR* strExtraCommandLineParams, bool bThreadSafeDXUT)
{
                                   ...
                                   LPARAM lParam)
{
	        ...
	        (GetDXUTState().GetNotifyOnMouseMove() && uMsg == WM_MOUSEMOVE))
	{
		...
		if (pCallbackMouse)
		{
			pCallbackMouse(bLeftButton, bRightButton, bMiddleButton, bSideButton1,
			               bSideButton2, nMouseWheelDelta, xPos, yPos,
			               GetDXUTState().GetMouseFuncUserContext());
		}
	}
	...
	if (pCallbackMsgProc)
	{
		...
		LRESULT nResult = pCallbackMsgProc(hWnd, uMsg, wParam, lParam,
		                                   &bNoFurtherProcessing, GetDXUTState().GetWindowMsgFuncUserContext());
	...
	case WM_PAINT:
	{
		        ...
		        && GetDXUTState().GetDeviceObjectsReset())
		{
			...
			else
			{
				...
				if (pd3dDevice)
				{
					LPDXUTCALLBACKD3D10FRAMERENDER pCallbackFrameRender =
					...
					hr = pSwapChain->Present(0,
					                         GetDXUTState().GetCurrentDeviceSettings()->d3d10.PresentFlags);
...
//--------------------------------------------------------------------------------------
HRESULT WINAPI DXUTCreateDevice(bool bWindowed, int nSuggestedWidth,
                                int nSuggestedHeight)
{
	...
	if (GetDXUTState().GetOverrideFullScreen())
	{
		...
		if (GetDXUTState().GetOverrideWidth() == 0
		        && GetDXUTState().GetOverrideHeight() == 0)
		{
			matchOptions.eResolution = DXUTMT_IGNORE_INPUT;
		}
	}
...
HRESULT WINAPI DXUTCreateDeviceFromSettings(DXUTDeviceSettings* pDeviceSettings,
        bool bPreserveInput, bool bClipWindowToSingleAdapter)
{
	HRESULT hr;
	GetDXUTState().SetDeviceCreateCalled(true);
	...
	if (!bPreserveInput)
	{
		...
		hr = DXUTFindValidDeviceSettings(pDeviceSettings, pDeviceSettings,
		                                 &matchOptions);
                         ...
                         bool bForceRecreate, bool bClipWindowToSingleAdapter)
{
	...
	if (NULL == pd3d9DeviceFromApp && NULL == pd3d10DeviceFromApp
		...
		if (DXUTGetIsWindowedFromDS(pNewDeviceSettings))
		{
			...
			if (pOldDeviceSettings && !DXUTGetIsWindowedFromDS(pOldDeviceSettings))
			{
				...
				GetDXUTState().SetFullScreenBackBufferWidthAtModeChange(
				    DXUTGetBackBufferWidthFromDS(pOldDeviceSettings));
				GetDXUTState().SetFullScreenBackBufferHeightAtModeChange(
				    DXUTGetBackBufferHeightFromDS(pOldDeviceSettings));
	                             ...
	                             pd3d10DeviceFromApp))
	{
		...
		if (FAILED(hr))
		{
			...
			else // DXUTERR_RESETTINGDEVICE
			{
				...
				if (FAILED(DXUTChangeDevice(pNewDeviceSettings, pd3d9DeviceFromApp,
				                            pd3d10DeviceFromApp, true, bClipWindowToSingleAdapter)))
				{
	...
	if (bNeedToResize)
	{
		...
		if (bClipWindowToSingleAdapter)
		{
			...
			SetWindowPos(DXUTGetHWNDDeviceWindowed(), 0, rcResizedWindow.left,
			             rcResizedWindow.top, nWindowWidth, nWindowHeight, SWP_NOZORDER);
		}
		else
		{
			// Make a window rect with a client rect that is the same size as the backbuffer
...
{
	if (pOldDeviceSettings == NULL
	        || pOldDeviceSettings->ver != pNewDeviceSettings->ver)
	{
		return false;
	}
	...
	else
	{
		if (DXUTGetD3D10Device() &&
		        ...
		        (pOldDeviceSettings->d3d10.AdapterOrdinal ==
		         pNewDeviceSettings->d3d10.AdapterOrdinal) &&
		        (pOldDeviceSettings->d3d10.DriverType     ==
		         pNewDeviceSettings->d3d10.DriverType) &&
		        (pOldDeviceSettings->d3d10.CreateFlags    ==
		         pNewDeviceSettings->d3d10.CreateFlags) &&
		        (pOldDeviceSettings->d3d10.sd.SampleDesc.Count ==
		         pNewDeviceSettings->d3d10.sd.SampleDesc.Count) &&
		        (pOldDeviceSettings->d3d10.sd.SampleDesc.Quality ==
		         pNewDeviceSettings->d3d10.sd.SampleDesc.Quality))
		{
			return true;
		}
...
void DXUTUpdateDeviceSettingsWithOverrides(DXUTDeviceSettings* pDeviceSettings)
{
	...
	else
	{
		if (GetDXUTState().GetOverrideAdapterOrdinal() != -1)
		{
			pDeviceSettings->d3d10.AdapterOrdinal =
			    GetDXUTState().GetOverrideAdapterOrdinal();
		}
	...
	if (pDeviceInfo)
	{
		        ...
		        iDeviceCombo < pDeviceInfo->deviceSettingsComboList.GetSize(); iDeviceCombo++)
		{
			CD3D9EnumDeviceSettingsCombo* pDeviceSettingsCombo =
			    pDeviceInfo->deviceSettingsComboList.GetAt(iDeviceCombo);
...
// DXUTCreateDeviceFromSettings()
//--------------------------------------------------------------------------------------
HRESULT WINAPI DXUTSetD3D10Device(ID3D10Device* pd3dDevice,
                                  IDXGISwapChain* pSwapChain)
{
	                   ...
	                   reinterpret_cast< void** >(&pDXGIDevice))))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			CGrowableArray<CD3D10EnumAdapterInfo*>* pAdapterInfoList =
...
HRESULT DXUTCreate3DEnvironment10(ID3D10Device* pd3dDeviceFromApp)
{
	...
	pDXGIFactory->MakeWindowAssociation(DXUTGetHWNDFocus(),
	...
	if (pd3dDeviceFromApp == NULL)
	{
		...
		{
			hr = pDXGIFactory->EnumAdapters(pNewDeviceSettings->d3d10.AdapterOrdinal,
			                                &pAdapter);
		}
		...
		hr = pDXGIFactory->CreateSwapChain(pd3dDevice, &pNewDeviceSettings->d3d10.sd,
		                                   &pSwapChain);
	...
	// If switching to REF, set the exit code to 10.  If switching to HAL and exit code was 10, then set it back to 0.
	if (pNewDeviceSettings->d3d10.DriverType == D3D10_DRIVER_TYPE_REFERENCE
	        && GetDXUTState().GetExitCode() == 0)
	{
		GetDXUTState().SetExitCode(10);
	}
	else if (pNewDeviceSettings->d3d10.DriverType == D3D10_DRIVER_TYPE_HARDWARE
	         && GetDXUTState().GetExitCode() == 10)
	{
		GetDXUTState().SetExitCode(0);
	}
	...
	// Update the device stats text
	CD3D10Enumeration* pd3dEnum = DXUTGetD3D10Enumeration();
	CD3D10EnumAdapterInfo* pAdapterInfo = pd3dEnum->GetAdapterInfo(
	        pNewDeviceSettings->d3d10.AdapterOrdinal);
	DXUTUpdateD3D10DeviceStats(pNewDeviceSettings->d3d10.DriverType,
	                           &pAdapterInfo->AdapterDesc);
	// Call the app's device created callback if non-NULL
	const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc =
	    DXUTGetDXGIBackBufferSurfaceDesc();
	GetDXUTState().SetInsideDeviceCallback(true);
	LPDXUTCALLBACKD3D10DEVICECREATED pCallbackDeviceCreated =
	    GetDXUTState().GetD3D10DeviceCreatedFunc();
	hr = S_OK;
	...
	GetDXUTState().SetInsideDeviceCallback(true);
	LPDXUTCALLBACKD3D10SWAPCHAINRESIZED pCallbackSwapChainResized =
	    GetDXUTState().GetD3D10SwapChainResizedFunc();
	hr = S_OK;

	if (pCallbackSwapChainResized != NULL)
	{
		hr = pCallbackSwapChainResized(DXUTGetD3D10Device(), pSwapChain,
	...
	if (GetDXUTState().GetDeviceObjectsReset())
	{
		...
		if (pCallbackSwapChainReleasing != NULL)
		{
			pCallbackSwapChainReleasing(
			    GetDXUTState().GetD3D10SwapChainReleasingFuncUserContext());
		}
	...
	if (SCDesc.Windowed != pDeviceSettings->d3d10.sd.Windowed)
	{
		...
		else
		{
			...
			pSwapChain->ResizeBuffers(pDeviceSettings->d3d10.sd.BufferCount,
			                          ...
			                          0);
			IDXGIOutput* pOutput =
			    GetDXUTState().GetD3D10OutputArray()[pDeviceSettings->d3d10.Output];
...
void DXUTCleanup3DEnvironment10(bool bReleaseSettings)
{
	...
	if (pd3dDevice != NULL)
	{
		...
		{
			LPDXUTCALLBACKD3D10DEVICEDESTROYED pCallbackDeviceDestroyed =
			    GetDXUTState().GetD3D10DeviceDestroyedFunc();

			if (pCallbackDeviceDestroyed != NULL)
			{
				pCallbackDeviceDestroyed(
				    GetDXUTState().GetD3D10DeviceDestroyedFuncUserContext());
			}
		...
		DXGI_SURFACE_DESC* pBackBufferSurfaceDesc =
		    GetDXUTState().GetBackBufferSurfaceDesc10();
...
HRESULT WINAPI DXUTSetTimer(LPDXUTCALLBACKTIMER pCallbackTimer,
                            float fTimeoutInSecs, UINT* pnIDEvent, void* pCallbackUserContext)
{
...
void DXUTDisplayErrorMessage(HRESULT hr)
{
	...
	case DXUTERR_NODIRECT3D:
	{
		...
		{
			StringCchCopy(strBuffer, ARRAYSIZE(strBuffer),
	...
	case DXUTERR_MEDIANOTFOUND:          nExitCode = 4;
		StringCchCopy(strBuffer, ARRAYSIZE(strBuffer),
		              L"Could not find required media."); break;
	...
	case DXUTERR_DEVICEREMOVED:          nExitCode = 11;
		StringCchCopy(strBuffer, ARRAYSIZE(strBuffer),
		              L"The Direct3D device was removed."); break;
...
HRESULT WINAPI DXUTToggleREF()
{
	...
	else
	{
		...
		hr = DXUTGetD3D10Device()->QueryInterface(__uuidof(*pD3D10STR),
		        (LPVOID*)&pD3D10STR);
...
HMONITOR DXUTGetMonitorFromAdapter(DXUTDeviceSettings* pDeviceSettings)
{
	...
	{
		CD3D10Enumeration* pD3DEnum = DXUTGetD3D10Enumeration();
		CD3D10EnumOutputInfo* pOutputInfo = pD3DEnum->GetOutputInfo(
		                                        pDeviceSettings->d3d10.AdapterOrdinal, pDeviceSettings->d3d10.Output);
        ...
        UINT* pAdapterOrdinal)
{
	*pAdapterOrdinal = 0;
	...
	else
	{
		...
		CD3D10Enumeration* pd3dEnum = DXUTGetD3D10Enumeration();
		CGrowableArray<CD3D10EnumAdapterInfo*>* pAdapterList =
		    pd3dEnum->GetAdapterInfoList();
		...
		for (int iAdapter = 0; iAdapter < pAdapterList->GetSize(); ++iAdapter)
		{
			...
			for (int o = 0; o < pAdapterInfo->outputInfoList.GetSize(); ++o)
			{
				...
				if (wcsncmp(pOutputInfo->Desc.DeviceName, mi.szDevice,
				            sizeof(mi.szDevice) / sizeof(mi.szDevice[0])) == 0)
				{
					*pAdapterOrdinal = pAdapterInfo->AdapterOrdinal;
					return S_OK;
				}
...
void DXUTUpdateBackBufferDesc()
{
	...
	else
	{
		HRESULT hr;
		ID3D10Texture2D* pBackBuffer;
		hr = GetDXUTState().GetD3D10SwapChain()->GetBuffer(0, __uuidof(*pBackBuffer),
		        (LPVOID*)&pBackBuffer);
		...
		if (!DXUTIsWindowed() && pd3dDevice)
		{
			if (GetDXUTState().GetShowCursorWhenFullScreen())
			{
				...
				HCURSOR hCursor = (HCURSOR)(ULONG_PTR)GetClassLongPtr(
				                      DXUTGetHWNDDeviceFullScreen(), GCLP_HCURSOR);
...
void DXUTUpdateStaticFrameStats()
{
	...
	if (DXUTIsD3D9(pDeviceSettings))
	{
		...
		CD3D9EnumDeviceSettingsCombo* pDeviceSettingsCombo =
		    pd3dEnum->GetDeviceSettingsCombo(pDeviceSettings->d3d9.AdapterOrdinal,
		                                     pDeviceSettings->d3d9.DeviceType, pDeviceSettings->d3d9.AdapterFormat,
		                                     pDeviceSettings->d3d9.pp.BackBufferFormat, pDeviceSettings->d3d9.pp.Windowed);
		...
		{
			StringCchPrintf(strDepthFmt, 100, L" (%s)",
		...
		{
		case D3DMULTISAMPLE_NONMASKABLE: StringCchCopy(strMultiSample, 100,
			        L" (Nonmaskable Multisample)"); break;

		case D3DMULTISAMPLE_NONE:        StringCchCopy(strMultiSample, 100, L""); break;

		default:                         StringCchPrintf(strMultiSample, 100,
			        L" (%dx Multisample)", pPP->MultiSampleType); break;
		}
	...
	else
	{
		...
		CD3D10EnumDeviceSettingsCombo* pDeviceSettingsCombo =
		    pd3dEnum->GetDeviceSettingsCombo(pDeviceSettings->d3d10.AdapterOrdinal,
		                                     pDeviceSettings->d3d10.DriverType, pDeviceSettings->d3d10.Output,
		                                     pDeviceSettings->d3d10.sd.BufferDesc.Format,
		                                     pDeviceSettings->d3d10.sd.Windowed);
		...
		WCHAR strFmt[100];
		StringCchCopy(strFmt, 100,
		              DXUTDXGIFormatToString(pDeviceSettingsCombo->BackBufferFormat, false));
		WCHAR strMultiSample[100];
		StringCchPrintf(strMultiSample, 100, L" (MS%u, Q%u)",
		                pDeviceSettings->d3d10.sd.SampleDesc.Count,
...
LPCWSTR WINAPI DXUTGetFrameStats(bool bShowFPS)
{
	...
	StringCchPrintf(pstrFrameStats, 256, GetDXUTState().GetStaticFrameStats(),
	                pstrFPS);
