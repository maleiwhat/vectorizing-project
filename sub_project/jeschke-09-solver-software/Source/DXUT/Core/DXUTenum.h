...
HRESULT WINAPI DXUTFindValidDeviceSettings(DXUTDeviceSettings* pOut,
        DXUTDeviceSettings* pIn = NULL, DXUTMatchOptions* pMatchOptions = NULL);


//--------------------------------------------------------------------------------------
// Functions to get bit depth from formats
//--------------------------------------------------------------------------------------
HRESULT WINAPI DXUTGetD3D10AdapterDisplayMode(UINT AdapterOrdinal, UINT Output,
        DXGI_MODE_DESC* pModeDesc);  // TODO: refactor?
...
public:
	...
	void SetResolutionMinMax(UINT nMinWidth, UINT nMinHeight, UINT nMaxWidth,
	                         UINT nMaxHeight);
	...
	void SetPossibleVertexProcessingList(bool bSoftwareVP, bool bHardwareVP,
	                                     bool bPureHarewareVP, bool bMixedVP);
	...
	HRESULT Enumerate(LPDXUTCALLBACKISD3D9DEVICEACCEPTABLE
	                  IsD3D9DeviceAcceptableFunc = NULL,
	...
	CD3D9EnumDeviceInfo*                     GetDeviceInfo(UINT AdapterOrdinal,
	        D3DDEVTYPE DeviceType);
	CD3D9EnumDeviceSettingsCombo*            GetDeviceSettingsCombo(
	    DXUTD3D9DeviceSettings* pD3D9DeviceSettings) { return GetDeviceSettingsCombo(pD3D9DeviceSettings->AdapterOrdinal, pD3D9DeviceSettings->DeviceType, pD3D9DeviceSettings->AdapterFormat, pD3D9DeviceSettings->pp.BackBufferFormat, pD3D9DeviceSettings->pp.Windowed); }
	CD3D9EnumDeviceSettingsCombo*            GetDeviceSettingsCombo(
...
public:
	...
	CGrowableArray<D3DDISPLAYMODE>
	displayModeList; // Array of supported D3DDISPLAYMODEs
	CGrowableArray<CD3D9EnumDeviceInfo*>
	deviceInfoList; // Array of CD3D9EnumDeviceInfo* with unique supported DeviceTypes
};
...
struct CD3D9EnumDeviceSettingsCombo
{
	...
	multiSampleTypeList; // List of D3DMULTISAMPLE_TYPEs
	CGrowableArray<DWORD>
	multiSampleQualityList; // List of number of quality levels for each multisample type
	CGrowableArray<UINT> presentIntervalList; // List of D3DPRESENT flags
	CGrowableArray<CD3D9EnumDSMSConflict>
	DSMSConflictList; // List of CD3D9EnumDSMSConflict
...
public:
	...
	void SetEnumerateAllAdapterFormats(bool bEnumerateAllAdapterFormats,
	                                   bool bEnumerateNow = true);

	// Call Enumerate() to enumerate available D3D10 adapters, devices, modes, etc.
	bool HasEnumerated() { return m_bHasEnumerated; }
	HRESULT Enumerate(LPDXUTCALLBACKISD3D10DEVICEACCEPTABLE
	...
	CD3D10EnumDeviceInfo*                    GetDeviceInfo(UINT AdapterOrdinal,
	        D3D10_DRIVER_TYPE DeviceType);
	CD3D10EnumOutputInfo*                    GetOutputInfo(UINT AdapterOrdinal,
	        UINT Output);
	CD3D10EnumDeviceSettingsCombo*           GetDeviceSettingsCombo(
	    DXUTD3D10DeviceSettings* pDeviceSettings) { return GetDeviceSettingsCombo(pDeviceSettings->AdapterOrdinal, pDeviceSettings->DriverType, pDeviceSettings->Output, pDeviceSettings->sd.BufferDesc.Format, pDeviceSettings->sd.Windowed); }
	CD3D10EnumDeviceSettingsCombo*           GetDeviceSettingsCombo(
...
private:
	...
	HRESULT EnumerateDeviceCombos(IDXGIFactory* pFactory,
...
CD3D10Enumeration* WINAPI DXUTGetD3D10Enumeration(bool bForceEnumerate = false,
        bool EnumerateAllAdapterFormats = false);
...
public:
	...
	CGrowableArray<CD3D10EnumOutputInfo*>
	outputInfoList; // Array of CD3D10EnumOutputInfo*
	CGrowableArray<CD3D10EnumDeviceInfo*>
	deviceInfoList; // Array of CD3D10EnumDeviceInfo*
...
struct CD3D10EnumDeviceSettingsCombo
{
	...
	CGrowableArray<UINT>
	multiSampleCountList; // List of valid sampling counts (multisampling)
	CGrowableArray<UINT>
	multiSampleQualityList; // List of number of quality levels for each multisample count
