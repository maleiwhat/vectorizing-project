...
void HKCV::Brightness_Contrast(const Mat& _src, Mat& _dst, double _bright, double _contrast)
{
	/// The input should be channel 1 OR channel 3 with type uchar (8U)
	if((_src.channels() != 1 && _src.channels() != 3) || _src.depth() != CV_8U )
		CV_Error(CV_StsBadArg, "_src has incorrect channel (not 1 or 3) and type (not CV_8U)");
...
void HKCV::Unsharp_Mask(const cv::Mat& _src, cv::Mat& _dst, double _r, double _a, double _t, cv::Mat& _mask)
{
	double	sigma = _r+1.0;
	Mat		blur, lct, srpn;
...
int HKCV::Quantize(const Mat& img3f, Mat &idx1i, Mat &_color3f, Mat &_colorNum, int _binNum, double ratio)
{
	...
	clrTmp[0]=clrNums[0]-0.0001f; clrTmp[1]=clrNums[1]-0.0001f; clrTmp[2]=clrNums[2]-0.0001f;  
...
void HKCV::Find_Contours(const cv::Mat& _img, VecPnt2is& _cntrs, cv::Rect& _rect)
{
	/// The input should be channel 1 with type uchar (8U)
	if(_img.channels() != 1 || _img.depth() != CV_8U )
		CV_Error(CV_StsBadArg, "_img has incorrect channel (not 1) and type (not CV_8U)");
...
void HKCV::RGB2lab(const cv::Mat& _bgr, cv::Mat& _lab)
{
	...
	if(_bgr.channels() != 3 || _bgr.depth() != CV_32F )
		CV_Error(CV_StsBadArg, "_bgr has incorrect channel (not 3) and type (not CV_32F)");
...
void HKCV::lab2RGB(const cv::Mat& _lab, cv::Mat& _bgr)
{
	...
	if(_lab.channels() != 3 || _lab.depth() != CV_32F )
		CV_Error(CV_StsBadArg, "_lab has incorrect channel (not 3) and type (not CV_32F)");
