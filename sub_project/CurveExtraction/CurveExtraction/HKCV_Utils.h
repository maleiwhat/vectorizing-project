...
template<class T> inline T CvPntDist(const cv::Point_<T>& p1,
                                     const cv::Point_<T>& p2) {return sqrt(sqr(p1.x - p2.x) + sqr(p1.y - p2.y));}
template<class T> inline T CvPntSqrDist(const cv::Point_<T>& p1,
                                        const cv::Point_<T>& p2) {return sqr(p1.x - p2.x) + sqr(p1.y - p2.y);}

/// OpenCV Vec utilities
template<class T> inline T CvVec3Dist(const cv::Vec<T, 3>& v1,
                                      const cv::Vec<T, 3>& v2) {return sqrt(sqr(v1[0] - v2[0]) + sqr(v1[1] - v2[1]) + sqr(v1[2] - v2[2]));}
template<class T> inline T CvVec3SqrDist(const cv::Vec<T, 3>& v1,
        const cv::Vec<T, 3>& v2) {return sqr(v1[0] - v2[0]) + sqr(v1[1] - v2[1]) + sqr(v1[2] - v2[2]);}
//template<class T1, class T2> inline void operator /= (cv::Vec<T1, 3> &v1, const T2 v2) { v1[0] /= v2; v1[1] /= v2; v1[2] /= v2; }
template<class T> inline void operator /= (cv::Vec<T, 3>& v1,
        const cv::Scalar s) { v1[0] /= s[0]; v1[1] /= s[1]; v1[2] /= s[2]; }
template<class T> inline void operator *= (cv::Vec<T, 3>& v1,
        const cv::Scalar s) { v1[0] *= s[0]; v1[1] *= s[1]; v1[2] *= s[2]; }
...
{
	_os << "Rect: [" << _rect.x << "," << _rect.y << "," << _rect.width << "," <<
	    _rect.height << "]";
	return _os;
}
...
inline void cvMat2Array(const cv::Mat& _mat, double* _array)
{
	...
	if (!_mat.data || _mat.channels() != 1)
	{
		CV_Error(CV_StsBadArg,
		         "_mat has incorrect data or channel (only support single channel for now...)");
		return;
	}
