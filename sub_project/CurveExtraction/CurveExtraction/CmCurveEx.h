...
public:
	...
	const cv::Mat& CalSecDer(int kSize = 5, float linkEndBound = 0.01f, float linkStartBound = 0.1f);
	const cv::Mat& CalFirDer(int kSize = 5, float linkEndBound = 0.01f, float linkStartBound = 0.1f);
...
private:
	...
	bool goNext(cv::Point &pnt, float& ornt, CEdge& crtEdge, int orntInd, bool isBackward);
	bool jumpNext(cv::Point &pnt, float& ornt, CEdge& crtEdge, int orntInd, bool isBackward);
