...
{
    cv::Mat fwtf = m_img1f.clone();
    //cv::resize(m_img1f.clone(), fwtf, m_img1f.size() * 2, 0, 0, cv::INTER_CUBIC);
//     m_pDer2f.create(m_h * 2, m_w * 2, CV_32FC1);
//     m_pOrnt1f.create(m_h * 2, m_w * 2, CV_32FC1);
    cv::Mat dxx, dxy, dyy;
    Sobel(fwtf, dxx, CV_32F, 2, 0, kSize);
    Sobel(fwtf, dxy, CV_32F, 1, 1, kSize);
    Sobel(fwtf, dyy, CV_32F, 0, 2, kSize);
    imshow("dxx1", dxx);
    imshow("dyy1", dyy);
    cv::Mat dxx2, dxy2, dyy2;
//     Sobel(fwtf, dxx2, CV_32F, 1, 0, 3);
//     Sobel(fwtf, dxy2, CV_32F, 1, 1, 3);
//     Sobel(fwtf, dyy2, CV_32F, 0, 1, 3);
//  cv::add(dxx, dxx2, dxx);
//  cv::add(dxy, dxy2, dxy);
//  cv::add(dyy, dyy2, dyy);
//     for(int i = 2; i < dyy.rows - 2; ++i)
//     {
//         for(int j = 2; j < dyy.cols - 2; ++j)
//         {
//             dyy.at<float>(i, j) = abs(fwtf.at<float>(i, j) - fwtf.at<float>(i - 1, j));
//         }
//     }
//     for(int i = 2; i < dxx.rows - 2; ++i)
//     {
//         for(int j = 2; j < dxx.cols - 2; ++j)
//         {
//             dxx.at<float>(i, j) = abs(fwtf.at<float>(i, j) - fwtf.at<float>(i, j - 1));
//         }
//     }
//     for(int i = 2; i < dxy.rows - 2; ++i)
//     {
//         for(int j = 2; j < dxy.cols - 2; ++j)
//         {
//          dxy.at<float>(i, j) = abs(fwtf.at<float>(i, j) - fwtf.at<float>(i - 1, j));
//          dxy.at<float>(i, j) += abs(fwtf.at<float>(i, j) - fwtf.at<float>(i, j - 1));
// //             dxy.at<float>(i, j) = abs(m_img1f.at<float>(i, j) - m_img1f.at<float>(i + 1, j + 1)) +
// //                                   abs(m_img1f.at<float>(i, j) - m_img1f.at<float>(i - 1, j + 1));
//         }
//     }
//     imshow("dxx", dxx);
//     imshow("dyy", dyy);
//     imshow("dxy", dxy);
    ...
    for(int y = 0; y < m_h ; y++)
    {
        ...
        for(int x = 0; x < m_w ; x++)
        {
            ...
            pDer2[x] = eigval[0] < 0 ? 0 : eigval[0]; //計算二階導數
        }
    }
    cv::Size ss = m_pDer2f.size();
    ss.width /= 2;
    ss.height /= 2;
//  cv::resize(m_pDer2f.clone(), m_pDer2f, ss, 0, 0, cv::INTER_CUBIC);
//  cv::resize(m_pOrnt1f.clone(), m_pOrnt1f, ss, 0, 0, cv::INTER_CUBIC);
    for(int y = 0; y < m_h; y++)
    {
        float* pOrnt = m_pOrnt1f.ptr<float>(y);
        for(int x = 0; x < m_w; x++)
    ...
    cv::Mat showder = m_pDer2f.clone();
    normalize(showder, showder, 0, 10, cv::NORM_MINMAX);
    for(int y = 0; y < m_h; y++)
    {
        float* pOrnt = m_pOrnt1f.ptr<float>(y);
        for(int x = 0; x < m_w; x++)
    ...
    showder.convertTo(showder, CV_8UC1, 255);
    imwrite("showder.png", showder);
    imshow("m_pDer2f", showder);
