...
bool LinearlyIndependent(MMatrix_row& mat)
{
	...
	while ((y_no_indep = std::find(isIndependentVector.begin(),
	                               isIndependentVector.end(), false))
...
#ifdef USE_CLAPACK
void svd(const boost::numeric::ublas::matrix<double>& A,
         boost::numeric::ublas::matrix<double>& U,
         boost::numeric::ublas::diagonal_matrix<double>& D,
         boost::numeric::ublas::matrix<double>& VT)
{
	namespace ublas = boost::numeric::ublas;
	std::vector<double>                        s((std::min)(A.size1(), A.size2()));
	ublas::matrix<double, ublas::column_major> CA(A), CU(A.size1(), A.size1()),
	      CVT(A.size2(), A.size2());
	int                                        info;
	...
	BOOST_UBLAS_CHECK(
	    ublas::detail::expression_type_check(ublas::prod(ublas::matrix<double>
	            (ublas::prod(CCU, CD)), CCVT), A),
