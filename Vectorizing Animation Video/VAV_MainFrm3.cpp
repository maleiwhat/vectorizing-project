				...
				for(int t = 0; t < timgs.size(); ++t)
				{
					...
					for(int i = 0; i < fgfi.curves2.size(); ++i)
					{
						if(fgfi.curves2[i].size() < 5)
						{
							...
							fgfi.ocolor2.left.erase(fgfi.ocolor2.left.begin() + i);
							fgfi.ocolor2.right.erase(fgfi.ocolor2.right.begin() + i);
							--i;
						}
					}
...
// 						cv::imwrite(path, simg);
			...
			for(int i = 0; i < fi.curves2.size(); ++i)
			{
				if(fi.curves2[i].size() < 5)
				{
					...
					fi.ocolor2.left.erase(fi.ocolor2.left.begin() + i);
					fi.ocolor2.right.erase(fi.ocolor2.right.begin() + i);
					--i;
				}
			}
			BackGround bgdata;
