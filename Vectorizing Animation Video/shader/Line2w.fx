...
VS_OUT VS(VS_IN vIn)
{
	...
	vOut.p22 = (vOut.p22/float2(width,height)*scale)*2-1+float2(centerX/width,-centerY/height)*scale;
	vOut.p33 = (vOut.p33/float2(width,height)*scale)*2-1+float2(centerX/width,-centerY/height)*scale;
	vOut.p1 += offset;
	vOut.p2 += offset;
	vOut.p3 += offset;
	vOut.p4 += offset;
...
//discard;
	...
	return float4(pIn.color.x, pIn.color.y, pIn.color.z, pow(pIn.color.w, 0.5));
}
