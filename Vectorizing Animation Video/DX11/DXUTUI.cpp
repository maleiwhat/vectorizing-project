...
void CALLBACK KTSGOnGUIEvent(UINT nEvent, int nControlID,
                             CDXUTControl* pControl, void* pUserContext);
bool CALLBACK KTSGIsD3D11DeviceAcceptable(const CD3D11EnumAdapterInfo*
        AdapterInfo, UINT Output, const CD3D11EnumDeviceInfo* DeviceInfo,
        DXGI_FORMAT BackBufferFormat, bool bWindowed, void* pUserContext);

HRESULT CALLBACK KTSGOnD3D11ResizedSwapChain(ID3D11Device* pd3dDevice,
        IDXGISwapChain* pSwapChain,
        const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext)
{
	g_ktsg_ui->m_DialogResourceManager.OnD3D11ResizedSwapChain(pd3dDevice,
	        pBackBufferSurfaceDesc) ;
	g_ktsg_ui->m_D3DSettingsDlg.OnD3D11ResizedSwapChain(pd3dDevice,
	        pBackBufferSurfaceDesc) ;
	return true;
}
...
DXUTUI::DXUTUI()
{
	...
	DXUTInit(true,
	         true);                   // Use this line instead to try to create a hardware device
...
void DXUTUI::CreateDevice(int winWidth, int winHeight)
{
	...
	m_DialogResourceManager.OnD3D11CreateDevice(DXUTGetD3D11Device(),
...
void UIDialog::SetCmd(int no, int id, const char* text, CommandType type,
