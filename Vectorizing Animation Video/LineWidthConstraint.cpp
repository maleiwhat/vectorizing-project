...
void LineWidthConstraint::BuildModel()
{
    ...
    for(int i = 0; i < m_Widths1.size(); ++i)
    {
        ...
        A.coeffRef(c, 3) = m_len * m_len * m_len;
        A.coeffRef(c, 4) = m_len * m_len * m_len * m_len;
        c++;
    }
    At = A.transpose();
    Ata = At * A;
...
{
    double xx = x1[0] + x1[1] * dis + x1[2] * dis * dis + x1[3] * dis* dis* dis + x1[4] * dis* dis* dis* dis;
    double yy = x2[0] + x2[1] * dis + x2[2] * dis * dis + x2[3] * dis* dis* dis + x2[4] * dis* dis* dis* dis;
