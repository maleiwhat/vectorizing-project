...
void Transpose(MMatrix_col& m_out, const MMatrix_row& m_in)
{
	...
	for (MMatrix_row::const_iterator1 it1 = m_in.begin1(); it1 != m_in.end1();
	        ++it1)
	{
...
void Transpose(MMatrix_row& m_out, const MMatrix_col& m_in)
{
	...
	for (MMatrix_col::const_iterator2 it1 = m_in.begin2(); it1 != m_in.end2();
	        ++it1)
	{
...
void Sparse_Multiply(MATRIX_TYPE& out, const MMatrix_row& A,
                     const MMatrix_col& B)
{
	out.clear();
...
void svd(const Matrix& A, Matrix& U, DMatrix& D, Matrix& VT)
{
	...
	BOOST_UBLAS_CHECK(
	    ublas::detail::expression_type_check(ublas::prod(Matrix(ublas::prod(CCU, CD)),
	            CCVT), A),
