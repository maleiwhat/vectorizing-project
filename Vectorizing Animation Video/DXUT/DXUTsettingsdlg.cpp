...
WCHAR*              DXUTMultisampleTypeToString(D3DMULTISAMPLE_TYPE
        MultiSampleType);
...
CD3DSettingsDlg::~CD3DSettingsDlg()
{
	...
	CDXUTComboBox* pComboBox = m_Dialog.GetComboBox(
	                               DXUTSETTINGSDLG_D3D11_REFRESH_RATE);

	if (pComboBox)
		for (UINT i = 0; i < pComboBox->GetNumItems(); ++i)
		{
			DXGI_RATIONAL* pRate = reinterpret_cast<DXGI_RATIONAL*>(pComboBox->GetItemData(
			                           i));
			delete pRate;
		}
}
...
//--------------------------------------------------------------------------------------
void CD3DSettingsDlg::Init(CDXUTDialogResourceManager* pManager,
                           LPCWSTR szControlTextureFileName)
{
	assert(pManager);
	m_Dialog.Init(pManager, false,
	              szControlTextureFileName);    // Don't register this dialog.
	m_RevertModeDialog.Init(pManager, false,
	                        szControlTextureFileName);     // Don't register this dialog.
	m_pActiveDialog = &m_Dialog;
	CreateControls();
}
...
//--------------------------------------------------------------------------------------
void CD3DSettingsDlg::Init(CDXUTDialogResourceManager* pManager,
                           LPCWSTR pszControlTextureResourcename,
                           HMODULE hModule)
{
	assert(pManager);
	m_Dialog.Init(pManager, false, pszControlTextureResourcename,
	              hModule);    // Don't register this dialog.
	m_RevertModeDialog.Init(pManager, false, pszControlTextureResourcename,
	                        hModule); // Don't register this dialog
	m_pActiveDialog = &m_Dialog;
	CreateControls();
}
...
void CD3DSettingsDlg::CreateControls()
{
	...
	if (pElement)
	{
		...
		m_Dialog.AddStatic(DXUTSETTINGSDLG_STATIC, L"Direct3D Settings", 10, 5, 400, 50,
		                   false, &pStatic);
	...
	m_Dialog.AddStatic(DXUTSETTINGSDLG_D3D11_FEATURE_LEVEL_LABEL, L"Feature Level",
	                   10, 60, 180, 23);
	...
	m_Dialog.AddCheckBox(DXUTSETTINGSDLG_DEVICECLIP,
	                     L"Clip to device when window spans across multiple monitors",
	                     250, 136, 500, 16);
	m_Dialog.AddRadioButton(DXUTSETTINGSDLG_WINDOWED,
	                        DXUTSETTINGSDLG_WINDOWED_GROUP, L"Windowed",
	                        360, 157, 100, 16);
	m_Dialog.AddRadioButton(DXUTSETTINGSDLG_FULLSCREEN,
	                        DXUTSETTINGSDLG_WINDOWED_GROUP, L"Full Screen",
	                        220, 157, 100, 16);
	// DXUTSETTINGSDLG_ADAPTER_FORMAT
	m_Dialog.AddStatic(DXUTSETTINGSDLG_ADAPTER_FORMAT_LABEL, L"Adapter Format", 10,
	                   175, 180, 23);
	m_Dialog.AddComboBox(DXUTSETTINGSDLG_ADAPTER_FORMAT, 200, 175, 300, 23);
	// DXUTSETTINGSDLG_RESOLUTION
	m_Dialog.AddStatic(DXUTSETTINGSDLG_RESOLUTION_LABEL, L"Resolution", 10, 200,
	                   180, 23);
	...
	m_Dialog.AddCheckBox(DXUTSETTINGSDLG_RESOLUTION_SHOW_ALL,
	                     L"Show All Aspect Ratios", 420, 200, 200, 23, false);
	// DXUTSETTINGSDLG_REFRESH_RATE
	m_Dialog.AddStatic(DXUTSETTINGSDLG_REFRESH_RATE_LABEL, L"Refresh Rate", 10, 225,
	                   180, 23);
	m_Dialog.AddComboBox(DXUTSETTINGSDLG_REFRESH_RATE, 200, 225, 300, 23);
	// DXUTSETTINGSDLG_BACK_BUFFER_FORMAT
	m_Dialog.AddStatic(DXUTSETTINGSDLG_BACK_BUFFER_FORMAT_LABEL,
	                   L"Back Buffer Format", 10, 260, 180, 23);
	m_Dialog.AddComboBox(DXUTSETTINGSDLG_BACK_BUFFER_FORMAT, 200, 260, 300, 23);
	// DXUTSETTINGSDLG_DEPTH_STENCIL
	m_Dialog.AddStatic(DXUTSETTINGSDLG_DEPTH_STENCIL_LABEL, L"Depth/Stencil Format",
	                   10, 285, 180, 23);
	m_Dialog.AddComboBox(DXUTSETTINGSDLG_DEPTH_STENCIL, 200, 285, 300, 23);
	// DXUTSETTINGSDLG_MULTISAMPLE_TYPE
	m_Dialog.AddStatic(DXUTSETTINGSDLG_MULTISAMPLE_TYPE_LABEL, L"Multisample Type",
	                   10, 310, 180, 23);
	m_Dialog.AddComboBox(DXUTSETTINGSDLG_MULTISAMPLE_TYPE, 200, 310, 300, 23);
	// DXUTSETTINGSDLG_MULTISAMPLE_QUALITY
	m_Dialog.AddStatic(DXUTSETTINGSDLG_MULTISAMPLE_QUALITY_LABEL,
	                   L"Multisample Quality", 10, 335, 180, 23);
	m_Dialog.AddComboBox(DXUTSETTINGSDLG_MULTISAMPLE_QUALITY, 200, 335, 300, 23);
	// DXUTSETTINGSDLG_VERTEX_PROCESSING
	m_Dialog.AddStatic(DXUTSETTINGSDLG_VERTEX_PROCESSING_LABEL,
	                   L"Vertex Processing", 10, 360, 180, 23);
	m_Dialog.AddComboBox(DXUTSETTINGSDLG_VERTEX_PROCESSING, 200, 360, 300, 23);
	// DXUTSETTINGSDLG_PRESENT_INTERVAL
	m_Dialog.AddStatic(DXUTSETTINGSDLG_PRESENT_INTERVAL_LABEL, L"Vertical Sync", 10,
	                   385, 180, 23);
	m_Dialog.AddComboBox(DXUTSETTINGSDLG_PRESENT_INTERVAL, 200, 385, 300, 23);
	// DXUTSETTINGSDLG_D3D11_ADAPTER_OUTPUT
	m_Dialog.AddStatic(DXUTSETTINGSDLG_D3D11_ADAPTER_OUTPUT_LABEL,
	                   L"Adapter Output", 10, 175, 180, 23);
	m_Dialog.AddComboBox(DXUTSETTINGSDLG_D3D11_ADAPTER_OUTPUT, 200, 175, 300, 23);
	// DXUTSETTINGSDLG_D3D11_RESOLUTION
	m_Dialog.AddStatic(DXUTSETTINGSDLG_D3D11_RESOLUTION_LABEL, L"Resolution", 10,
	                   200, 180, 23);
	...
	// DXUTSETTINGSDLG_D3D11_REFRESH_RATE
	m_Dialog.AddStatic(DXUTSETTINGSDLG_D3D11_REFRESH_RATE_LABEL, L"Refresh Rate",
	                   10, 225, 180, 23);
	m_Dialog.AddComboBox(DXUTSETTINGSDLG_D3D11_REFRESH_RATE, 200, 225, 300, 23);
	// DXUTSETTINGSDLG_D3D11_BACK_BUFFER_FORMAT
	m_Dialog.AddStatic(DXUTSETTINGSDLG_D3D11_BACK_BUFFER_FORMAT_LABEL,
	                   L"Back Buffer Format", 10, 260, 180, 23);
	m_Dialog.AddComboBox(DXUTSETTINGSDLG_D3D11_BACK_BUFFER_FORMAT, 200, 260, 300,
	                     23);
	// DXUTSETTINGSDLG_D3D11_MULTISAMPLE_COUNT
	m_Dialog.AddStatic(DXUTSETTINGSDLG_D3D11_MULTISAMPLE_COUNT_LABEL,
	                   L"Multisample Count", 10, 285, 180, 23);
	m_Dialog.AddComboBox(DXUTSETTINGSDLG_D3D11_MULTISAMPLE_COUNT, 200, 285, 300,
	                     23);
	// DXUTSETTINGSDLG_D3D11_MULTISAMPLE_QUALITY
	m_Dialog.AddStatic(DXUTSETTINGSDLG_D3D11_MULTISAMPLE_QUALITY_LABEL,
	                   L"Multisample Quality", 10, 310, 180, 23);
	m_Dialog.AddComboBox(DXUTSETTINGSDLG_D3D11_MULTISAMPLE_QUALITY, 200, 310, 300,
	                     23);
	// DXUTSETTINGSDLG_D3D11_PRESENT_INTERVAL
	m_Dialog.AddStatic(DXUTSETTINGSDLG_D3D11_PRESENT_INTERVAL_LABEL,
	                   L"Vertical Sync", 10, 335, 180, 23);
	m_Dialog.AddComboBox(DXUTSETTINGSDLG_D3D11_PRESENT_INTERVAL, 200, 335, 300, 23);
	// DXUTSETTINGSDLG_D3D11_DEBUG_DEVICE
	m_Dialog.AddCheckBox(DXUTSETTINGSDLG_D3D11_DEBUG_DEVICE, L"Create Debug Device",
	                     200, 365, 180, 23);
	// DXUTSETTINGSDLG_OK, DXUTSETTINGSDLG_CANCEL
	m_Dialog.AddButton(DXUTSETTINGSDLG_OK, L"OK", 230, 440, 73, 31);
	m_Dialog.AddButton(DXUTSETTINGSDLG_CANCEL, L"Cancel", 315, 440, 73, 31, 0,
	                   true);
	...
	if (pElement)
	{
		...
		// Title
		CDXUTStatic* pStatic = NULL;
		m_RevertModeDialog.AddStatic(DXUTSETTINGSDLG_STATIC,
		                             L"Do you want to keep these display settings?", 10, 5,
		                             640, 50, false, &pStatic);
		pElement = pStatic->GetElement(0);
		pElement->iFont = 1;
		pElement->dwTextFormat = DT_TOP | DT_LEFT;
		// Timeout static text control
		m_RevertModeDialog.AddStatic(DXUTSETTINGSDLG_STATIC_MODE_CHANGE_TIMEOUT, L"",
		                             10, 90, 640, 30,
		                             false, &pStatic);
		pElement = pStatic->GetElement(0);
		pElement->iFont = 0;
	...
	m_RevertModeDialog.AddButton(DXUTSETTINGSDLG_MODE_CHANGE_ACCEPT, L"Yes", 230,
	                             50, 73, 31);
	m_RevertModeDialog.AddButton(DXUTSETTINGSDLG_MODE_CHANGE_REVERT, L"No", 315, 50,
	                             73, 31, 0, true);
}
...
HRESULT CD3DSettingsDlg::Refresh()
{
	...
	case DXUT_D3D9_DEVICE:
	{
		...
		SetDeviceClip(0 != (g_DeviceSettings.d3d9.pp.Flags &
		                    D3DPRESENTFLAG_DEVICECLIP));
		AddAdapterFormat(g_DeviceSettings.d3d9.AdapterFormat);
		AddResolution(g_DeviceSettings.d3d9.pp.BackBufferWidth,
		...
		else if (g_DeviceSettings.d3d9.BehaviorFlags &
		...
		CGrowableArray <CD3D9EnumAdapterInfo*>* pAdapterInfoList =
		    pD3DEnum->GetAdapterInfoList();

		if (pAdapterInfoList->GetSize() == 0)
		{
			return DXUT_ERR_MSGBOX(L"CD3DSettingsDlg::OnCreatedDevice",
		...
		pAdapterCombo->SetSelectedByData(ULongToPtr(
		                                     g_DeviceSettings.d3d9.AdapterOrdinal));
	...
	case DXUT_D3D11_DEVICE:
	{
		...
		AddD3D11AdapterOutput(pOutputInfo->Desc.DeviceName,
		...
		CDXUTComboBox* pFeatureLevelBox = m_Dialog.GetComboBox(
		                                      DXUTSETTINGSDLG_D3D11_FEATURE_LEVEL);
		...
		pFeatureLevelBox->SetSelectedByData(ULongToPtr(
		                                        g_DeviceSettings.d3d11.DeviceFeatureLevel));
		// Get the adapters list from CD3D11Enumeration object
		CGrowableArray <CD3D11EnumAdapterInfo*>* pAdapterInfoList =
		    pD3DEnum->GetAdapterInfoList();
		...
		pAdapterCombo->SetSelectedByData(ULongToPtr(
		                                     g_DeviceSettings.d3d11.AdapterOrdinal));
		hr = OnAPIVersionChanged(true);
...
{
	CDXUTComboBox* pRefreshRateComboBox = m_Dialog.GetComboBox(
	        DXUTSETTINGSDLG_D3D11_REFRESH_RATE);
	...
	for (UINT i = 0; i < pRefreshRateComboBox->GetNumItems(); ++i)
	{
		...
		if (pRate && pRate->Numerator == RefreshRate.Numerator
		        && pRate->Denominator == RefreshRate.Denominator)
		{
...
//--------------------------------------------------------------------------------------
LRESULT CD3DSettingsDlg::MsgProc(HWND hWnd, UINT uMsg, WPARAM wParam,
                                 LPARAM lParam)
{
        ...
        const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc)
{
	...
	m_Dialog.SetBackgroundColors(D3DCOLOR_ARGB(255, 98, 138, 206),
	                             ...
	                             D3DCOLOR_ARGB(255, 10, 73, 179));
	m_RevertModeDialog.SetLocation(0, 0);
	m_RevertModeDialog.SetSize(pBackBufferSurfaceDesc->Width,
...
void CD3DSettingsDlg::ShowControlSet(DXUTDeviceVersion ver)
{
	...
	case DXUT_D3D9_DEVICE:
		...
		m_Dialog.GetControl(DXUTSETTINGSDLG_MULTISAMPLE_QUALITY_LABEL)->SetVisible(
		    true);
		...
		m_Dialog.GetControl(DXUTSETTINGSDLG_D3D11_FEATURE_LEVEL_LABEL)->SetVisible(
		    false);
		m_Dialog.GetControl(DXUTSETTINGSDLG_D3D11_ADAPTER_OUTPUT)->SetVisible(false);
		m_Dialog.GetControl(DXUTSETTINGSDLG_D3D11_ADAPTER_OUTPUT_LABEL)->SetVisible(
		    false);
		...
		m_Dialog.GetControl(DXUTSETTINGSDLG_D3D11_REFRESH_RATE_LABEL)->SetVisible(
		    false);
		m_Dialog.GetControl(DXUTSETTINGSDLG_D3D11_BACK_BUFFER_FORMAT)->SetVisible(
		    false);
		m_Dialog.GetControl(DXUTSETTINGSDLG_D3D11_BACK_BUFFER_FORMAT_LABEL)->SetVisible(
		    false);
		m_Dialog.GetControl(DXUTSETTINGSDLG_D3D11_MULTISAMPLE_COUNT)->SetVisible(false);
		m_Dialog.GetControl(DXUTSETTINGSDLG_D3D11_MULTISAMPLE_COUNT_LABEL)->SetVisible(
		    false);
		m_Dialog.GetControl(DXUTSETTINGSDLG_D3D11_MULTISAMPLE_QUALITY)->SetVisible(
		    false);
		m_Dialog.GetControl(
		    DXUTSETTINGSDLG_D3D11_MULTISAMPLE_QUALITY_LABEL)->SetVisible(false);
		m_Dialog.GetControl(DXUTSETTINGSDLG_D3D11_PRESENT_INTERVAL)->SetVisible(false);
		m_Dialog.GetControl(DXUTSETTINGSDLG_D3D11_PRESENT_INTERVAL_LABEL)->SetVisible(
		    false);
	...
	case DXUT_D3D11_DEVICE:
		...
		m_Dialog.GetControl(DXUTSETTINGSDLG_BACK_BUFFER_FORMAT_LABEL)->SetVisible(
		    false);
		...
		m_Dialog.GetControl(DXUTSETTINGSDLG_D3D11_MULTISAMPLE_QUALITY)->SetVisible(
		    true);
		m_Dialog.GetControl(
		    DXUTSETTINGSDLG_D3D11_MULTISAMPLE_QUALITY_LABEL)->SetVisible(true);
...
//--------------------------------------------------------------------------------------
void WINAPI CD3DSettingsDlg::StaticOnModeChangeTimer(UINT nIDEvent,
        void* pUserContext)
{
	...
	{
		CDXUTControl* pControl = pD3DSettings->m_RevertModeDialog.GetControl(
		                             DXUTSETTINGSDLG_MODE_CHANGE_REVERT);
		assert(pControl);
		pD3DSettings->m_RevertModeDialog.SendEvent(EVENT_BUTTON_CLICKED, false,
		        pControl);
	}
...
HRESULT CD3DSettingsDlg::SetDeviceSettingsFromUI()
{
	...
	g_DeviceSettings.d3d9.DeviceType = (D3DDEVTYPE)PtrToUlong(
	                                       pComboBox->GetSelectedData());
	...
	g_DeviceSettings.d3d9.AdapterFormat = (D3DFORMAT)PtrToUlong(
	        pComboBox->GetSelectedData());
	...
	else
	{
		...
		g_DeviceSettings.d3d9.pp.BackBufferFormat = (D3DFORMAT)PtrToUlong(
		            pComboBox->GetSelectedData());
		// DXUTSETTINGSDLG_RESOLUTION
		...
		g_DeviceSettings.d3d9.pp.FullScreen_RefreshRateInHz = PtrToUlong(
		            pComboBox->GetSelectedData());
	}

	// DXUTSETTINGSDLG_DEPTH_STENCIL
	pComboBox = m_Dialog.GetComboBox(DXUTSETTINGSDLG_DEPTH_STENCIL);
	g_DeviceSettings.d3d9.pp.AutoDepthStencilFormat = (D3DFORMAT)PtrToUlong(
	            pComboBox->GetSelectedData());
	return S_OK;
}
...
CD3DSettingsDlg::GetCurrentD3D11DeviceSettingsCombo()
{
	CD3D11Enumeration* pD3DEnum = DXUTGetD3D11Enumeration();
...
HRESULT CD3DSettingsDlg::OnFeatureLevelChanged()
{
	...
	if (g_DeviceSettings.ver == DXUT_D3D11_DEVICE)
	{
		...
		hr = DXUTSnapDeviceSettingsToEnumDevice(&g_DeviceSettings, true,
		                                        GetSelectedFeatureLevel());
		//}
		CD3D11Enumeration* pD3DEnum = DXUTGetD3D11Enumeration();
		CGrowableArray <CD3D11EnumAdapterInfo*>* pAdapterInfoList =
		    pD3DEnum->GetAdapterInfoList();
		...
		pAdapterComboBox->SetSelectedByData(ULongToPtr(
		                                        g_DeviceSettings.d3d11.AdapterOrdinal));
		CDXUTCheckBox* pCheckBox = m_Dialog.GetCheckBox(
		                               DXUTSETTINGSDLG_D3D11_DEBUG_DEVICE);
		pCheckBox->SetChecked(0 != (g_DeviceSettings.d3d11.CreateFlags &
...
HRESULT CD3DSettingsDlg::OnAdapterChanged()
{
	...
	case DXUT_D3D9_DEVICE:
	{
		...
		// DXUTSETTINGSDLG_DEVICE_TYPE
		CDXUTComboBox* pDeviceTypeComboBox = m_Dialog.GetComboBox(
		        DXUTSETTINGSDLG_DEVICE_TYPE);
		...
		pDeviceTypeComboBox->SetSelectedByData(ULongToPtr(
		        g_DeviceSettings.d3d9.DeviceType));
	...
	case DXUT_D3D11_DEVICE:
	{
		...
		for (int iDeviceInfo = 0; iDeviceInfo < pAdapterInfo->deviceInfoList.GetSize();
		        iDeviceInfo++)
		{
			CD3D11EnumDeviceInfo* pDeviceInfo = pAdapterInfo->deviceInfoList.GetAt(
			                                        iDeviceInfo);
	...
	case DXUT_D3D9_DEVICE:
	{
		...
		for (int idc = 0; idc < pDeviceInfo->deviceSettingsComboList.GetSize(); idc++)
		{
			CD3D9EnumDeviceSettingsCombo* pDeviceSettingsCombo =
			    pDeviceInfo->deviceSettingsComboList.GetAt(idc);
		...
		m_Dialog.SetControlEnabled(DXUTSETTINGSDLG_FULLSCREEN,
		                           bHasFullScreenDeviceCombo);
...
HRESULT CD3DSettingsDlg::OnWindowedFullScreenChanged()
{
	...
	m_Dialog.SetControlEnabled(DXUTSETTINGSDLG_D3D11_ADAPTER_OUTPUT_LABEL,
	                           !bWindowed);
	m_Dialog.SetControlEnabled(DXUTSETTINGSDLG_D3D11_RESOLUTION_LABEL, !bWindowed);
	m_Dialog.SetControlEnabled(DXUTSETTINGSDLG_D3D11_REFRESH_RATE_LABEL,
	                           !bWindowed);
	...
	case DXUT_D3D9_DEVICE:
	{
		...
		bool bDeviceClip = (0x0 != (g_DeviceSettings.d3d9.pp.Flags &
		                            D3DPRESENTFLAG_DEVICECLIP));
		...
		// DXUTSETTINGSDLG_ADAPTER_FORMAT
		CDXUTComboBox* pAdapterFormatComboBox = m_Dialog.GetComboBox(
		        DXUTSETTINGSDLG_ADAPTER_FORMAT);
		...
		{
			for (int iSettingsCombo = 0;
			        iSettingsCombo < pDeviceInfo->deviceSettingsComboList.GetSize();
			        iSettingsCombo++)
			{
				CD3D9EnumDeviceSettingsCombo* pSettingsCombo =
		...
		// DXUTSETTINGSDLG_RESOLUTION
		CDXUTComboBox* pResolutionComboBox = m_Dialog.GetComboBox(
		        DXUTSETTINGSDLG_RESOLUTION);
	...
	case DXUT_D3D11_DEVICE:
	{
		...
		CD3D11Enumeration* pD3DEnum = DXUTGetD3D11Enumeration();
		CDXUTComboBox* pOutputComboBox = m_Dialog.GetComboBox(
		                                     DXUTSETTINGSDLG_D3D11_ADAPTER_OUTPUT);
		pOutputComboBox->RemoveAllItems();
		CD3D11EnumAdapterInfo* pAdapterInfo = pD3DEnum->GetAdapterInfo(
		        g_DeviceSettings.d3d11.AdapterOrdinal);

		for (int ioutput = 0; ioutput < pAdapterInfo->outputInfoList.GetSize();
		        ++ioutput)
		{
...
HRESULT CD3DSettingsDlg::OnAdapterOutputChanged()
{
	...
	case DXUT_D3D11_DEVICE:
	{
		...
		const DXGI_RATIONAL RefreshRate =
		    g_DeviceSettings.d3d11.sd.BufferDesc.RefreshRate;
		CD3D11EnumAdapterInfo* pAdapterInfo = GetCurrentD3D11AdapterInfo();
		...
		// DXUTSETTINGSDLG_D3D11_BACK_BUFFER_FORMAT
		CDXUTComboBox* pBackBufferFormatComboBox = m_Dialog.GetComboBox(
		            DXUTSETTINGSDLG_D3D11_BACK_BUFFER_FORMAT
		        );
		...
		{
			CD3D11EnumDeviceSettingsCombo* pDeviceCombo =
			    pAdapterInfo->deviceSettingsComboList.GetAt(idc);
		...
		pBackBufferFormatComboBox->SetSelectedByData(ULongToPtr(
		            g_DeviceSettings.d3d11.sd.BufferDesc.Format));
...
HRESULT CD3DSettingsDlg::OnAdapterFormatChanged()
{
	...
	case DXUT_D3D9_DEVICE:
	{
		...
		bool bShowAll = m_Dialog.GetCheckBox(
		                    DXUTSETTINGSDLG_RESOLUTION_SHOW_ALL)->GetChecked();
		// Get the desktop aspect ratio
		D3DDISPLAYMODE dmDesktop;
		DXUTGetDesktopResolution(g_DeviceSettings.d3d9.AdapterOrdinal, &dmDesktop.Width,
		                         &dmDesktop.Height);
...
HRESULT CD3DSettingsDlg::OnBackBufferFormatChanged()
{
	...
	case DXUT_D3D9_DEVICE:
	{
		...
		DXUTGetD3D9Enumeration()->GetPossibleVertexProcessingList(&bAllowSoftwareVP,
		        &bAllowHardwareVP,
		...
		for (int idc = 0; idc < pDeviceInfo->deviceSettingsComboList.GetSize(); idc++)
		{
			...
			{
				CDXUTComboBox* pDepthStencilComboBox = m_Dialog.GetComboBox(
				        DXUTSETTINGSDLG_DEPTH_STENCIL);
				pDepthStencilComboBox->RemoveAllItems();
				pDepthStencilComboBox->SetEnabled((
				                                      g_DeviceSettings.d3d9.pp.EnableAutoDepthStencil == TRUE));

				if (g_DeviceSettings.d3d9.pp.EnableAutoDepthStencil)
				{
					for (int ifmt = 0; ifmt < pDeviceCombo->depthStencilFormatList.GetSize();
					        ifmt++)
					{
				...
				if (bAllowHardwareVP
				        && pDeviceInfo->Caps.DevCaps & D3DDEVCAPS_HWTRANSFORMANDLIGHT)
				{
					AddVertexProcessingType(D3DCREATE_HARDWARE_VERTEXPROCESSING);
				}
				...
				CDXUTComboBox* pPresentIntervalComboBox = m_Dialog.GetComboBox(
				            DXUTSETTINGSDLG_PRESENT_INTERVAL);
				pPresentIntervalComboBox->RemoveAllItems();
				pPresentIntervalComboBox->AddItem(L"On",
				                                  ULongToPtr(D3DPRESENT_INTERVAL_DEFAULT));
				pPresentIntervalComboBox->AddItem(L"Off",
				                                  ULongToPtr(D3DPRESENT_INTERVAL_IMMEDIATE));
	...
	case DXUT_D3D11_DEVICE:
	{
		...
		for (int idc = 0; idc < pAdapterInfo->deviceSettingsComboList.GetSize(); idc++)
		{
			...
			{
				CDXUTComboBox* pMultisampleCountCombo = m_Dialog.GetComboBox(
				        DXUTSETTINGSDLG_D3D11_MULTISAMPLE_COUNT
				                                        );
				...
				pPresentIntervalComboBox->SetSelectedByData(ULongToPtr(
				            g_DeviceSettings.d3d11.SyncInterval));
				hr = OnPresentIntervalChanged();
...
HRESULT CD3DSettingsDlg::OnDepthStencilBufferFormatChanged()
{
	...
	CDXUTComboBox* pMultisampleTypeCombo = m_Dialog.GetComboBox(
	        DXUTSETTINGSDLG_MULTISAMPLE_TYPE);
	pMultisampleTypeCombo->RemoveAllItems();

	for (int ims = 0; ims < pDeviceSettingsCombo->multiSampleTypeList.GetSize();
	        ims++)
	{
		D3DMULTISAMPLE_TYPE msType = pDeviceSettingsCombo->multiSampleTypeList.GetAt(
		                                 ims);
		bool bConflictFound = false;

		for (int iConf = 0; iConf < pDeviceSettingsCombo->DSMSConflictList.GetSize();
		        iConf++)
		{
			CD3D9EnumDSMSConflict DSMSConf = pDeviceSettingsCombo->DSMSConflictList.GetAt(
			                                     iConf);
	...
	CDXUTComboBox* pMultisampleQualityCombo = m_Dialog.GetComboBox(
	            DXUTSETTINGSDLG_MULTISAMPLE_TYPE);
	pMultisampleQualityCombo->SetSelectedByData(ULongToPtr(
	            g_DeviceSettings.d3d9.pp.MultiSampleType));
	hr = OnMultisampleTypeChanged();
...
HRESULT CD3DSettingsDlg::OnMultisampleTypeChanged()
{
	...
	case DXUT_D3D9_DEVICE:
	{
		...
		for (int iType = 0; iType < pDeviceSettingsCombo->multiSampleTypeList.GetSize();
		        iType++)
		{
			D3DMULTISAMPLE_TYPE msType = pDeviceSettingsCombo->multiSampleTypeList.GetAt(
			                                 iType);
		...
		pMultisampleQualityCombo->SetSelectedByData(ULongToPtr(
		            g_DeviceSettings.d3d9.pp.MultiSampleQuality));
	...
	case DXUT_D3D11_DEVICE:
	{
		...
		for (int iCount = 0;
		        iCount < pDeviceSettingsCombo->multiSampleCountList.GetSize(); iCount++)
		{
		...
		pMultisampleQualityCombo->SetSelectedByData(ULongToPtr(
		            g_DeviceSettings.d3d11.sd.SampleDesc.Quality));
		hr = OnMultisampleQualityChanged();
...
HRESULT CD3DSettingsDlg::OnMultisampleQualityChanged()
{
	...
	case DXUT_D3D11_DEVICE:
		g_DeviceSettings.d3d11.sd.SampleDesc.Quality =
		    GetSelectedD3D11MultisampleQuality();
		break;
	}
...
{
	CDXUTRadioButton* pRadioButton = m_Dialog.GetRadioButton(
	                                     DXUTSETTINGSDLG_WINDOWED);
...
//-------------------------------------------------------------------------------------
void CD3DSettingsDlg::GetSelectedD3D11Resolution(DWORD* pdwWidth,
        DWORD* pdwHeight)
{
	CDXUTComboBox* pComboBox = m_Dialog.GetComboBox(
	                               DXUTSETTINGSDLG_D3D11_RESOLUTION);
...
void CD3DSettingsDlg::AddD3D11FeatureLevel(D3D_FEATURE_LEVEL fl)
{
	...
	case D3D_FEATURE_LEVEL_9_1:
	{
		...
		{
			pComboBox->AddItem(L"D3D_FEATURE_LEVEL_9_1",
			                   ULongToPtr(D3D_FEATURE_LEVEL_9_1));
		}
	}
	break;
	...
	case D3D_FEATURE_LEVEL_9_2:
	{
		...
		{
			pComboBox->AddItem(L"D3D_FEATURE_LEVEL_9_2",
			                   ULongToPtr(D3D_FEATURE_LEVEL_9_2));
		}
	}
	break;
	...
	case D3D_FEATURE_LEVEL_9_3:
	{
		...
		{
			pComboBox->AddItem(L"D3D_FEATURE_LEVEL_9_3",
			                   ULongToPtr(D3D_FEATURE_LEVEL_9_3));
		}
	}
	break;
	...
	case D3D_FEATURE_LEVEL_10_0:
	{
		...
		{
			pComboBox->AddItem(L"D3D_FEATURE_LEVEL_10_0",
			                   ULongToPtr(D3D_FEATURE_LEVEL_10_0));
		}
	}
	break;
	...
	case D3D_FEATURE_LEVEL_10_1:
	{
		...
		{
			pComboBox->AddItem(L"D3D_FEATURE_LEVEL_10_1",
			                   ULongToPtr(D3D_FEATURE_LEVEL_10_1));
		}
	}
	break;
	...
	case D3D_FEATURE_LEVEL_11_0:
	{
		...
		{
			pComboBox->AddItem(L"D3D_FEATURE_LEVEL_11_0",
			                   ULongToPtr(D3D_FEATURE_LEVEL_11_0));
		}
	}
	break;
	}
...
void CD3DSettingsDlg::AddD3D11RefreshRate(DXGI_RATIONAL RefreshRate)
{
	...
	{
		swprintf_s(strRefreshRate, 50, L"%d Hz",
...
void CD3DSettingsDlg::AddVertexProcessingType(DWORD dwType)
{
	...
	{
		pComboBox->AddItem(DXUTVertexProcessingTypeToString(dwType),
		                   ULongToPtr(dwType));
	}
}
...
HRESULT CD3DSettingsDlg::UpdateD3D11Resolutions()
{
	...
	DXUTGetDesktopResolution(g_DeviceSettings.d3d11.AdapterOrdinal,
	                         &dmDesktop.Width, &dmDesktop.Height);
	float fDesktopAspectRatio = dmDesktop.Width / (float)dmDesktop.Height;
	...
	const DWORD dwCurResolution = MAKELONG(
	                                  g_DeviceSettings.d3d11.sd.BufferDesc.Width,
	                                  g_DeviceSettings.d3d11.sd.BufferDesc.Height);
	pResolutionComboBox->SetSelectedByData(ULongToPtr(dwCurResolution));
	bool bWindowed = IsWindowed();
...
{
	const WCHAR StrTimeout[] =
	    L"Reverting to previous display settings in %d seconds";
	const DWORD CchBuf = sizeof(StrTimeout) / sizeof(WCHAR) + 16;
	WCHAR buf[CchBuf];
	swprintf_s(buf, CchBuf, StrTimeout, nSecRemaining);
	CDXUTStatic* pStatic = m_RevertModeDialog.GetStatic(
	                           DXUTSETTINGSDLG_STATIC_MODE_CHANGE_TIMEOUT);
	pStatic->SetText(buf);
}
