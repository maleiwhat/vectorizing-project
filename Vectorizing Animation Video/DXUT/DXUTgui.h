...
struct DXUTBlendColor
{
	...
	D3DCOLOR
	States[ MAX_CONTROL_STATES ]; // Modulate colors for all possible control states
	D3DXCOLOR Current;
};
...
public:
	void    SetTexture(UINT iTexture, RECT* prcTexture,
	                   D3DCOLOR defaultTextureColor = D3DCOLOR_ARGB(255, 255, 255,
	                           255));
...
public:
	...
	// Control creation
	HRESULT             AddStatic(int ID, LPCWSTR strText, int x, int y, int width,
	                              int height, bool bIsDefault = false,
	                              CDXUTStatic** ppCreated = NULL);
	HRESULT             AddButton(int ID, LPCWSTR strText, int x, int y, int width,
	                              int height, UINT nHotkey = 0,
	                              bool bIsDefault = false, CDXUTButton** ppCreated = NULL);
	HRESULT             AddCheckBox(int ID, LPCWSTR strText, int x, int y,
	                                int width, int height, bool bChecked = false,
	                                UINT nHotkey = 0, bool bIsDefault = false, CDXUTCheckBox** ppCreated = NULL);
	HRESULT             AddRadioButton(int ID, UINT nButtonGroup, LPCWSTR strText,
	...
	HRESULT             AddComboBox(int ID, int x, int y, int width, int height,
	                                UINT nHotKey = 0, bool bIsDefault =
	                                    false, CDXUTComboBox** ppCreated = NULL);
	HRESULT             AddSlider(int ID, int x, int y, int width, int height,
	...
	HRESULT             AddEditBox(int ID, LPCWSTR strText, int x, int y, int width,
	...
	void                SendEvent(UINT nEvent, bool bTriggeredByUser,
	                              CDXUTControl* pControl);
	...
	HRESULT             DrawPolyLine(POINT* apPoints, UINT nNumPoints,
	                                 D3DCOLOR color);
	HRESULT             DrawSprite(CDXUTElement* pElement, RECT* prcDest,
	                               float fDepth);
	HRESULT             DrawSprite9(CDXUTElement* pElement, RECT* prcDest);
	HRESULT             DrawSprite11(CDXUTElement* pElement, RECT* prcDest,
	                                 float fDepth);
	HRESULT             CalcTextRect(LPCWSTR strText, CDXUTElement* pElement,
	                                 RECT* prcDest, int nCount = -1);
	HRESULT             DrawText(LPCWSTR strText, CDXUTElement* pElement,
	                             RECT* prcDest, bool bShadow = false,
	                             int nCount = -1, bool bCenter = false);
	HRESULT             DrawText9(LPCWSTR strText, CDXUTElement* pElement,
	                              RECT* prcDest, bool bShadow = false,
	                              int nCount = -1);
	HRESULT             DrawText11(ID3D11Device* pd3dDevice,
	...
	{
		SetBackgroundColors(colorAllCorners, colorAllCorners, colorAllCorners,
		                    colorAllCorners);
	}
	void                SetBackgroundColors(D3DCOLOR colorTopLeft,
	...
	{
		wcscpy_s(m_wszCaption, sizeof(m_wszCaption) / sizeof(m_wszCaption[0]),
		         pwszText);
	}
	...
	void                SetCallback(PCALLBACKDXUTGUIEVENT pCallback,
...
public:
	...
	void    BeginSprites11();
	void    EndSprites11(ID3D11Device* pd3dDevice,
...
public:
	...
	void            Scroll(int
	                       nDelta);      // Scroll by nDelta items (plus or minus)
	void            ShowItem(int
...
protected:
	...
	void
	Cap();  // Clips position at boundaries. Ensures it stays within legal range.
...
private:
	HRESULT
	Analyse();      // Uniscribe -- Analyse() analyses the string in the buffer
...
private:
	...
	static HRESULT WINAPI   Dummy_ScriptStringAnalyse(HDC, const void*, int, int,
	...
	static HRESULT WINAPI   Dummy_ScriptStringCPtoX(SCRIPT_STRING_ANALYSIS, int,
	        BOOL, int*)
	{
		return E_NOTIMPL;
	}
	static HRESULT WINAPI   Dummy_ScriptStringXtoCP(SCRIPT_STRING_ANALYSIS, int,
	        int*, int*)
	{
		return E_NOTIMPL;
	}
	...
	// Function pointers
	static                  HRESULT(WINAPI* _ScriptApplyDigitSubstitution)(
	...
	static                  HRESULT(WINAPI* _ScriptStringAnalyse)(HDC, const void*,
	        ...
	        SCRIPT_STRING_ANALYSIS*);
	static                  HRESULT(WINAPI* _ScriptStringCPtoX)(
	    SCRIPT_STRING_ANALYSIS, int, BOOL, int*);
	static                  HRESULT(WINAPI* _ScriptStringXtoCP)(
	    SCRIPT_STRING_ANALYSIS, int, int*, int*);
	static                  HRESULT(WINAPI* _ScriptStringFree)(
	    SCRIPT_STRING_ANALYSIS*);
