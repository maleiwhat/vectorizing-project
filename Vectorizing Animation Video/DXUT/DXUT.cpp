...
protected:
	struct STATE
	{
		...
		D3DSURFACE_DESC
		m_BackBufferSurfaceDesc9;  // D3D9 back buffer surface description
		D3DCAPS9
		m_Caps;                    // D3D caps for current device

		// D3D11 specific
		IDXGIFactory1*           m_DXGIFactory;             // DXGI Factory object
		IDXGIAdapter1*
		m_DXGIAdapter;            // The DXGI adapter object for the D3D11 device
		IDXGIOutput**
		m_DXGIOutputArray;        // The array of output obj for the D3D11 adapter obj
		UINT
		m_DXGIOutputArraySize;    // Number of elements in m_D3D11OutputArray
		IDXGISwapChain*         m_DXGISwapChain;          // the D3D11 swapchain
		DXGI_SURFACE_DESC
		m_BackBufferSurfaceDescDXGI; // D3D11 back buffer surface description
		bool
		m_RenderingOccluded;       // Rendering is occluded by another window
		bool
		m_DoNotStoreBufferSize;    // Do not store the buffer size on WM_SIZE messages

		// D3D11 specific
		bool
		m_D3D11Available;          // if true, then D3D11 is available
		ID3D11Device*       m_D3D11Device;             // the D3D11 rendering device
		ID3D11DeviceContext*
		m_D3D11DeviceContext;      // the D3D11 immediate device context
		D3D_FEATURE_LEVEL
		m_D3D11FeatureLevel;       // the D3D11 feature level that this device supports
		ID3D11Texture2D*
		m_D3D11DepthStencil;       // the D3D11 depth stencil texture (optional)
		ID3D11DepthStencilView*
		m_D3D11DepthStencilView;   // the D3D11 depth stencil view (optional)
		ID3D11RenderTargetView*
		m_D3D11RenderTargetView;   // the D3D11 render target view
		...
		WINDOWPLACEMENT
		m_WindowedPlacement;// record of windowed HWND position/show state/etc
		...
		STICKYKEYS
		m_StartupStickyKeys;     // StickyKey settings upon startup so they can be restored later
		TOGGLEKEYS
		m_StartupToggleKeys;     // ToggleKey settings upon startup so they can be restored later
		FILTERKEYS
		m_StartupFilterKeys;     // FilterKey settings upon startup so they can be restored later
		...
		D3D_FEATURE_LEVEL
		m_OverrideForceFeatureLevel; // if != -1, then overrid to use a featurelevel
		...
		m_ModifyDeviceSettingsFunc;     // modify Direct3D device settings callback
		LPDXUTCALLBACKDEVICEREMOVED
		m_DeviceRemovedFunc;            // Direct3D device removed callback
		LPDXUTCALLBACKFRAMEMOVE
		m_FrameMoveFunc;                // frame move callback
		LPDXUTCALLBACKKEYBOARD
		m_KeyboardFunc;                 // keyboard callback
		LPDXUTCALLBACKMOUSE
		m_MouseFunc;                    // mouse callback
		LPDXUTCALLBACKMSGPROC
		m_WindowMsgFunc;                // window messages callback

		LPDXUTCALLBACKISD3D9DEVICEACCEPTABLE
		m_IsD3D9DeviceAcceptableFunc;   // D3D9 is device acceptable callback
		LPDXUTCALLBACKD3D9DEVICECREATED
		m_D3D9DeviceCreatedFunc;        // D3D9 device created callback
		LPDXUTCALLBACKD3D9DEVICERESET
		m_D3D9DeviceResetFunc;          // D3D9 device reset callback
		LPDXUTCALLBACKD3D9DEVICELOST
		m_D3D9DeviceLostFunc;           // D3D9 device lost callback
		LPDXUTCALLBACKD3D9DEVICEDESTROYED
		m_D3D9DeviceDestroyedFunc;      // D3D9 device destroyed callback
		LPDXUTCALLBACKD3D9FRAMERENDER
		m_D3D9FrameRenderFunc;          // D3D9 frame render callback

		LPDXUTCALLBACKISD3D11DEVICEACCEPTABLE
		m_IsD3D11DeviceAcceptableFunc;  // D3D11 is device acceptable callback
		LPDXUTCALLBACKD3D11DEVICECREATED
		m_D3D11DeviceCreatedFunc;       // D3D11 device created callback
		LPDXUTCALLBACKD3D11SWAPCHAINRESIZED
		m_D3D11SwapChainResizedFunc;    // D3D11 SwapChain reset callback
		LPDXUTCALLBACKD3D11SWAPCHAINRELEASING
		m_D3D11SwapChainReleasingFunc;  // D3D11 SwapChain lost callback
		LPDXUTCALLBACKD3D11DEVICEDESTROYED
		m_D3D11DeviceDestroyedFunc;     // D3D11 device destroyed callback
		LPDXUTCALLBACKD3D11FRAMERENDER
		m_D3D11FrameRenderFunc;         // D3D11 frame render callback
...
public:
	...
	GET_SET_ACCESSOR(LPDXUTCALLBACKISD3D9DEVICEACCEPTABLE,
	                 IsD3D9DeviceAcceptableFunc);
	...
	GET_SET_ACCESSOR(LPDXUTCALLBACKISD3D11DEVICEACCEPTABLE,
	                 IsD3D11DeviceAcceptableFunc);
	GET_SET_ACCESSOR(LPDXUTCALLBACKD3D11DEVICECREATED, D3D11DeviceCreatedFunc);
	GET_SET_ACCESSOR(LPDXUTCALLBACKD3D11SWAPCHAINRESIZED,
	                 D3D11SwapChainResizedFunc);
	GET_SET_ACCESSOR(LPDXUTCALLBACKD3D11SWAPCHAINRELEASING,
	                 D3D11SwapChainReleasingFunc);
...
LRESULT CALLBACK DXUTStaticWndProc(HWND hWnd, UINT uMsg, WPARAM wParam,
                                   LPARAM lParam);
...
HRESULT DXUTSnapDeviceSettingsToEnumDevice(DXUTDeviceSettings* pDeviceSettings,
        bool forceEnum, D3D_FEATURE_LEVEL forceFL = D3D_FEATURE_LEVEL(0));
...
HRESULT DXUTGetAdapterOrdinalFromMonitor(HMONITOR hMonitor,
        UINT* pAdapterOrdinal);
HRESULT DXUTGetOutputOrdinalFromMonitor(HMONITOR hMonitor,
                                        UINT* pOutputOrdinal);
...
HRESULT DXUTSetD3D9DeviceCursor(IDirect3DDevice9* pd3dDevice, HCURSOR hCursor,
                                bool bAddWatermark);
void DXUTUpdateD3D9DeviceStats(D3DDEVTYPE DeviceType, DWORD BehaviorFlags,
                               D3DADAPTER_IDENTIFIER9* pAdapterIdentifier);
HRESULT DXUTFindD3D9AdapterFormat(UINT AdapterOrdinal, D3DDEVTYPE DeviceType,
                                  D3DFORMAT BackBufferFormat,
                                  BOOL Windowed, D3DFORMAT* pAdapterFormat);

// Direct3D 11
HRESULT DXUTCreateD3D11Views(ID3D11Device* pd3dDevice,
                             ID3D11DeviceContext* pd3dDeviceContext, DXUTDeviceSettings* pDeviceSettings);
...
void DXUTUpdateD3D11DeviceStats(D3D_DRIVER_TYPE DeviceType,
...
// General callbacks
void WINAPI DXUTSetCallbackDeviceChanging(LPDXUTCALLBACKMODIFYDEVICESETTINGS
        pCallback, void* pUserContext)                  { GetDXUTState().SetModifyDeviceSettingsFunc(pCallback); GetDXUTState().SetModifyDeviceSettingsFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackDeviceRemoved(LPDXUTCALLBACKDEVICEREMOVED pCallback,
        void* pUserContext)                          { GetDXUTState().SetDeviceRemovedFunc(pCallback); GetDXUTState().SetDeviceRemovedFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackFrameMove(LPDXUTCALLBACKFRAMEMOVE pCallback,
                                     void* pUserContext)                                  { GetDXUTState().SetFrameMoveFunc(pCallback);  GetDXUTState().SetFrameMoveFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackKeyboard(LPDXUTCALLBACKKEYBOARD pCallback,
                                    void* pUserContext)                                    { GetDXUTState().SetKeyboardFunc(pCallback);  GetDXUTState().SetKeyboardFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackMouse(LPDXUTCALLBACKMOUSE pCallback,
                                 bool bIncludeMouseMove, void* pUserContext)                  { GetDXUTState().SetMouseFunc(pCallback); GetDXUTState().SetNotifyOnMouseMove(bIncludeMouseMove);  GetDXUTState().SetMouseFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackMsgProc(LPDXUTCALLBACKMSGPROC pCallback,
                                   void* pUserContext)                                      { GetDXUTState().SetWindowMsgFunc(pCallback);  GetDXUTState().SetWindowMsgFuncUserContext(pUserContext); }

// Direct3D 9 callbacks
void WINAPI DXUTSetCallbackD3D9DeviceAcceptable(
    LPDXUTCALLBACKISD3D9DEVICEACCEPTABLE pCallback, void* pUserContext)          { GetDXUTState().SetIsD3D9DeviceAcceptableFunc(pCallback); GetDXUTState().SetIsD3D9DeviceAcceptableFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackD3D9DeviceCreated(LPDXUTCALLBACKD3D9DEVICECREATED
        pCallback, void* pUserContext)                  { GetDXUTState().SetD3D9DeviceCreatedFunc(pCallback); GetDXUTState().SetD3D9DeviceCreatedFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackD3D9DeviceReset(LPDXUTCALLBACKD3D9DEVICERESET
        pCallback, void* pUserContext)                      { GetDXUTState().SetD3D9DeviceResetFunc(pCallback);  GetDXUTState().SetD3D9DeviceResetFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackD3D9DeviceLost(LPDXUTCALLBACKD3D9DEVICELOST
        pCallback, void* pUserContext)                        { GetDXUTState().SetD3D9DeviceLostFunc(pCallback);  GetDXUTState().SetD3D9DeviceLostFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackD3D9DeviceDestroyed(LPDXUTCALLBACKD3D9DEVICEDESTROYED
        pCallback, void* pUserContext)              { GetDXUTState().SetD3D9DeviceDestroyedFunc(pCallback);  GetDXUTState().SetD3D9DeviceDestroyedFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackD3D9FrameRender(LPDXUTCALLBACKD3D9FRAMERENDER
        pCallback, void* pUserContext)                      { GetDXUTState().SetD3D9FrameRenderFunc(pCallback);  GetDXUTState().SetD3D9FrameRenderFuncUserContext(pUserContext); }
void DXUTGetCallbackD3D9DeviceAcceptable(LPDXUTCALLBACKISD3D9DEVICEACCEPTABLE*
        ppCallback, void** ppUserContext)             { *ppCallback = GetDXUTState().GetIsD3D9DeviceAcceptableFunc(); *ppUserContext = GetDXUTState().GetIsD3D9DeviceAcceptableFuncUserContext(); }

// Direct3D 11 callbacks
void WINAPI DXUTSetCallbackD3D11DeviceAcceptable(
    LPDXUTCALLBACKISD3D11DEVICEACCEPTABLE pCallback, void* pUserContext)        { GetDXUTState().SetIsD3D11DeviceAcceptableFunc(pCallback); GetDXUTState().SetIsD3D11DeviceAcceptableFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackD3D11DeviceCreated(LPDXUTCALLBACKD3D11DEVICECREATED
        pCallback, void* pUserContext)                { GetDXUTState().SetD3D11DeviceCreatedFunc(pCallback); GetDXUTState().SetD3D11DeviceCreatedFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackD3D11SwapChainResized(
    LPDXUTCALLBACKD3D11SWAPCHAINRESIZED pCallback, void* pUserContext)          { GetDXUTState().SetD3D11SwapChainResizedFunc(pCallback);  GetDXUTState().SetD3D11SwapChainResizedFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackD3D11FrameRender(LPDXUTCALLBACKD3D11FRAMERENDER
        pCallback, void* pUserContext)                    { GetDXUTState().SetD3D11FrameRenderFunc(pCallback);  GetDXUTState().SetD3D11FrameRenderFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackD3D11SwapChainReleasing(
    LPDXUTCALLBACKD3D11SWAPCHAINRELEASING pCallback, void* pUserContext)      { GetDXUTState().SetD3D11SwapChainReleasingFunc(pCallback);  GetDXUTState().SetD3D11SwapChainReleasingFuncUserContext(pUserContext); }
void WINAPI DXUTSetCallbackD3D11DeviceDestroyed(
    LPDXUTCALLBACKD3D11DEVICEDESTROYED pCallback, void* pUserContext)            { GetDXUTState().SetD3D11DeviceDestroyedFunc(pCallback);  GetDXUTState().SetD3D11DeviceDestroyedFuncUserContext(pUserContext); }
void DXUTGetCallbackD3D11DeviceAcceptable(LPDXUTCALLBACKISD3D11DEVICEACCEPTABLE*
        ppCallback, void** ppUserContext)           { *ppCallback = GetDXUTState().GetIsD3D11DeviceAcceptableFunc(); *ppUserContext = GetDXUTState().GetIsD3D11DeviceAcceptableFuncUserContext(); }
...
//--------------------------------------------------------------------------------------
HRESULT WINAPI DXUTCreateWindow(const WCHAR* strWindowTitle,
                                HINSTANCE hInstance,
                                HICON hIcon, HMENU hMenu, int x, int y)
{
	...
	if (DXUTGetHWNDFocus() == NULL)
	{
		...
		if (hIcon ==
		        NULL)  // If the icon is NULL, then use the first one found in the exe
		{
		...
		HWND hWnd = CreateWindow(L"Direct3DWindowClass", strWindowTitle,
		                         WS_OVERLAPPEDWINDOW,
                             ...
                             HWND hWndDeviceWindowed, bool bHandleMessages)
{
	...
	if (hWndFocus == NULL || hWndDeviceFullScreen == NULL
	...
	HINSTANCE hInstance = (HINSTANCE)(LONG_PTR)GetWindowLongPtr(hWndFocus,
	                      GWLP_HINSTANCE);
	GetDXUTState().SetHInstance(hInstance);
	GetDXUTState().SetWindowCreatedWithDefaultPositions(false);
	GetDXUTState().SetWindowCreated(true);
                                   ...
                                   LPARAM lParam)
{
	        ...
	        uMsg == WM_SYSKEYUP)
	{
		...
		if (pCallbackKeyboard)
		{
			pCallbackKeyboard((UINT)wParam, bKeyDown, bAltDown,
			                  GetDXUTState().GetKeyboardFuncUserContext());
		}
	}
	        ...
	        (GetDXUTState().GetNotifyOnMouseMove() && uMsg == WM_MOUSEMOVE))
	{
		...
		if (pCallbackMouse)
			pCallbackMouse(bLeftButton, bRightButton, bMiddleButton, bSideButton1,
			               bSideButton2, nMouseWheelDelta,
	...
	case WM_PAINT:
	{
		...
		if (DXUTIsRenderingPaused() &&
		        GetDXUTState().GetDeviceObjectsCreated()
		        && GetDXUTState().GetDeviceObjectsReset())
		{
			HRESULT hr;
			...
			if (DXUTIsCurrentDeviceD3D9())
			{
				...
				if (pd3dDevice)
				{
					LPDXUTCALLBACKD3D9FRAMERENDER pCallbackFrameRender =
			...
			else
			{
				...
				if (pd3dDevice)
				{
					LPDXUTCALLBACKD3D11FRAMERENDER pCallbackFrameRender =
					...
					hr = pSwapChain->Present(0,
					                         GetDXUTState().GetCurrentDeviceSettings()->d3d11.PresentFlags);
	...
	case WM_ACTIVATEAPP:
		         ...
		         && DXUTIsActive())  // Handle only if previously active
		{
			...
			if (!DXUTIsWindowed())
			{
				...
				{
					DXUTPause(true,
					          true);    // Pause while we're minimized (take care not to pause twice by handling this message twice)
				}
...
HRESULT WINAPI DXUTMainLoop(HACCEL hAccel)
{
	...
	if (GetDXUTState().GetInsideDeviceCallback()
	        || GetDXUTState().GetInsideMainloop())
	{
	...
	if (!GetDXUTState().GetDXUTInited() || !GetDXUTState().GetWindowCreated()
...
//======================================================================================
//======================================================================================
HRESULT WINAPI DXUTCreateDevice(D3D_FEATURE_LEVEL reqFL,  bool bWindowed,
                                int nSuggestedWidth, int nSuggestedHeight)
{
	...
	if (hr ==  DXUTERR_NODIRECT3D11
	        && GetDXUTState().GetMessageWhenD3D11NotAvailable())
	{
		OSVERSIONINFOEX osv;
		...
		if ((osv.dwMajorVersion > 6)
		        ...
		        || (osv.dwMajorVersion == 6 && osv.dwMinorVersion == 0
		            && osv.dwBuildNumber > 6002))
		{
			MessageBox(0, L"Direct3D 11 components were not found.", L"Error",
			           MB_ICONEXCLAMATION);
		...
		{
			MessageBox(0,
			           L"Direct3D 11 components were not found. Please install the latest Service Pack.\n"\
		...
		{
			MessageBox(0, L"Direct3D 11 is not supported on this OS.", L"Error",
			           MB_ICONEXCLAMATION);
		}
	}
...
//--------------------------------------------------------------------------------------
HRESULT WINAPI DXUTCreateDeviceFromSettings(DXUTDeviceSettings* pDeviceSettings,
        bool bPreserveInput,
        bool bClipWindowToSingleAdapter)
{
	HRESULT hr;
	...
	hr = DXUTChangeDevice(pDeviceSettings, NULL, NULL, false,
...
{
	HRESULT hr = S_OK;
	DXUTDeviceSettings* pOldDeviceSettings =
	    GetDXUTState().GetCurrentDeviceSettings();
	...
	memcpy(pNewDeviceSettingsOnHeap, pNewDeviceSettings,
	       sizeof(DXUTDeviceSettings));
	...
	LPDXUTCALLBACKMODIFYDEVICESETTINGS pCallbackModifyDeviceSettings =
	    GetDXUTState().GetModifyDeviceSettingsFunc();
	...
	if (pCallbackModifyDeviceSettings && pd3d9DeviceFromApp == NULL)
	{
		...
		if (GetDXUTState().GetD3D9() == NULL
		        && GetDXUTState().GetDXGIFactory() ==
		        NULL) // if DXUTShutdown() was called in the modify callback, just return
		{
			SAFE_DELETE(pNewDeviceSettings);
			return S_FALSE;
		}

		DXUTSnapDeviceSettingsToEnumDevice(pNewDeviceSettingsOnHeap,
		                                   false); // modify the app specified settings to the closed enumerated settigns
	...
	if (DXUTIsCurrentDeviceD3D9())
	{
		if (DXUTGetIsWindowedFromDS(pNewDeviceSettings))
		{
			...
			if (pOldDeviceSettings && !DXUTGetIsWindowedFromDS(pOldDeviceSettings))
			{
				...
				GetDXUTState().SetFullScreenBackBufferWidthAtModeChange(
				    DXUTGetBackBufferWidthFromDS(
				        pOldDeviceSettings));
				GetDXUTState().SetFullScreenBackBufferHeightAtModeChange(
				    DXUTGetBackBufferHeightFromDS(
				        pOldDeviceSettings));
				// Restore windowed mode style
				SetWindowLong(DXUTGetHWNDDeviceWindowed(), GWL_STYLE,
				              GetDXUTState().GetWindowedStyleAtModeChange());
			}
		...
		else
		{
			...
			if (pOldDeviceSettings == NULL || (pOldDeviceSettings
			                                   && DXUTGetIsWindowedFromDS(pOldDeviceSettings)))
			{
				...
				{
					GetDXUTState().SetWindowBackBufferWidthAtModeChange(
					    DXUTGetBackBufferWidthFromDS(
					        pOldDeviceSettings));
					GetDXUTState().SetWindowBackBufferHeightAtModeChange(
					    DXUTGetBackBufferHeightFromDS(
					        pOldDeviceSettings));
				}
			}
	        ...
	        DXUTGetIsWindowedFromDS(pNewDeviceSettings))
	{
		...
		HWND hWndInsertAfter = GetDXUTState().GetTopmostWhileWindowed() ? HWND_TOPMOST :
		                       HWND_NOTOPMOST;
	        ...
	        !bKeepCurrentWindowSize)                       // only resize if pp.BackbufferWidth/Height were not 0
	{
		...
		if (IsIconic(DXUTGetHWNDDeviceWindowed()))
		{
			...
			else
			{
				...
				nClientWidth = (UINT)(wp.rcNormalPosition.right - wp.rcNormalPosition.left -
				                      nFrameWidth);
				nClientHeight = (UINT)(wp.rcNormalPosition.bottom - wp.rcNormalPosition.top -
				                       nFrameHeight);
			}
		}
		else
		...
		if (bClipWindowToSingleAdapter && !IsIconic(DXUTGetHWNDDeviceWindowed()))
		{
			...
			HMONITOR hWindowMonitor = DXUTMonitorFromWindow(DXUTGetHWND(),
	...
	if (bNeedToResize)
	{
		...
		if (IsZoomed(
		            DXUTGetHWNDDeviceWindowed()))    // doing the IsIconic() check first also handles the WPF_RESTORETOMAXIMIZED case
		{
			ShowWindow(DXUTGetHWNDDeviceWindowed(), SW_RESTORE);
		}
		...
		if (bClipWindowToSingleAdapter)
		{
			...
			DXUTGetMonitorInfo(DXUTMonitorFromWindow(DXUTGetHWND(),
			                   MONITOR_DEFAULTTOPRIMARY), &miWindow);
			...
			AdjustWindowRect(&rcResizedWindow, GetWindowLong(DXUTGetHWNDDeviceWindowed(),
			                 GWL_STYLE),
			                 GetDXUTState().GetMenu() != NULL);
			...
			// after resetting the device rather than beforehand to ensure
			// that the monitor resolution is correct and does not limit the size of the new window.
			SetWindowPos(DXUTGetHWNDDeviceWindowed(), 0, rcResizedWindow.left,
			             rcResizedWindow.top, nWindowWidth,
		...
		else
		{
			...
			AdjustWindowRect(&rcWindow, GetWindowLong(DXUTGetHWNDDeviceWindowed(),
			                 GWL_STYLE), GetDXUTState().GetMenu() != NULL);
			// Resize the window.  It is important to adjust the window size
			// after resetting the device rather than beforehand to ensure
			// that the monitor resolution is correct and does not limit the size of the new window.
			int cx = (int)(rcWindow.right - rcWindow.left);
			int cy = (int)(rcWindow.bottom - rcWindow.top);
			SetWindowPos(DXUTGetHWNDDeviceWindowed(), 0, 0, 0, cx, cy,
		        ...
		        nClientHeight != DXUTGetBackBufferHeightFromDS(pNewDeviceSettings))
		{
			...
			hr = DXUTChangeDevice(&deviceSettings, NULL, NULL, false,
			                      bClipWindowToSingleAdapter);
...
HRESULT DXUTDelayLoadDXGI()
{
	...
	{
		DXUT_Dynamic_CreateDXGIFactory1(__uuidof(IDXGIFactory1),
		                                (LPVOID*)&pDXGIFactory);
...
void DXUTUpdateDeviceSettingsWithOverrides(DXUTDeviceSettings* pDeviceSettings)
{
	...
	if (GetDXUTState().GetOverrideHeight() != 0)
	{
		...
		pDeviceSettings->d3d11.sd.BufferDesc.Height =
		    GetDXUTState().GetOverrideHeight();
	}

	if (GetDXUTState().GetOverrideAdapterOrdinal() != -1)
	{
		pDeviceSettings->d3d9.AdapterOrdinal =
		    GetDXUTState().GetOverrideAdapterOrdinal();
	...
	{
		pDeviceSettings->d3d11.DeviceFeatureLevel = (D3D_FEATURE_LEVEL)
		        GetDXUTState().GetOverrideForceFeatureLevel();
	}
}
...
HRESULT WINAPI DXUTSetD3D9Device(IDirect3DDevice9* pd3dDevice)
{
	...
	DXUTFindD3D9AdapterFormat(DeviceSettings.d3d9.AdapterOrdinal,
...
HRESULT DXUTCreate3DEnvironment9(IDirect3DDevice9* pd3dDeviceFromApp)
{
	...
	if (pd3dDeviceFromApp == NULL)
	{
		...
		assert(pD3D != NULL);
		hr = pD3D->CreateDevice(pNewDeviceSettings->d3d9.AdapterOrdinal,
		                        pNewDeviceSettings->d3d9.DeviceType,
	...
	if (pNewDeviceSettings->d3d9.DeviceType == D3DDEVTYPE_REF
	        && GetDXUTState().GetExitCode() == 0)
	{
		GetDXUTState().SetExitCode(10);
	}
	else if (pNewDeviceSettings->d3d9.DeviceType == D3DDEVTYPE_HAL
	         && GetDXUTState().GetExitCode() == 10)
	{
	...
	CD3D9EnumAdapterInfo* pAdapterInfo = pd3dEnum->GetAdapterInfo(
	        pNewDeviceSettings->d3d9.AdapterOrdinal);
	...
	LPDXUTCALLBACKD3D9DEVICECREATED pCallbackDeviceCreated =
	    GetDXUTState().GetD3D9DeviceCreatedFunc();
	hr = S_OK;
	...
	if (FAILED(hr))
	{
		...
		return (hr == DXUTERR_MEDIANOTFOUND) ? DXUTERR_MEDIANOTFOUND :
	...
	GetDXUTState().SetInsideDeviceCallback(true);
	LPDXUTCALLBACKD3D9DEVICERESET pCallbackDeviceReset =
	    GetDXUTState().GetD3D9DeviceResetFunc();
	hr = S_OK;
...
HRESULT DXUTReset3DEnvironment9()
{
	...
	{
		GetDXUTState().SetInsideDeviceCallback(true);
		LPDXUTCALLBACKD3D9DEVICELOST pCallbackDeviceLost =
		    GetDXUTState().GetD3D9DeviceLostFunc();
...
void DXUTRender3DEnvironment9()
{
	...
	if (GetDXUTState().GetDeviceLost() && !GetDXUTState().GetRenderingPaused())
	{
		...
		if (FAILED(hr = pd3dDevice->TestCooperativeLevel()))
		{
			...
			if (DXUTIsWindowed())
			{
				...
				IDirect3D9* pD3D = DXUTGetD3D9Object();
				DXUTDeviceSettings* pDeviceSettings = GetDXUTState().GetCurrentDeviceSettings();
				pD3D->GetAdapterDisplayMode(pDeviceSettings->d3d9.AdapterOrdinal,
	...
	{
		pCallbackFrameMove(fTime, fElapsedTime,
		                   GetDXUTState().GetFrameMoveFuncUserContext());
...
void DXUTCleanup3DEnvironment9(bool bReleaseSettings)
{
	...
	if (pd3dDevice != NULL)
	{
		...
		{
			LPDXUTCALLBACKD3D9DEVICEDESTROYED pCallbackDeviceDestroyed =
			    GetDXUTState().GetD3D9DeviceDestroyedFunc();

			if (pCallbackDeviceDestroyed != NULL)
			{
				pCallbackDeviceDestroyed(
				    GetDXUTState().GetD3D9DeviceDestroyedFuncUserContext());
			}
		...
		D3DSURFACE_DESC* pBackBufferSurfaceDesc =
		    GetDXUTState().GetBackBufferSurfaceDesc9();
                                  ...
                                  BOOL Windowed, D3DFORMAT* pAdapterFormat)
{
	...
	assert(pd3dEnum != NULL);
	CD3D9EnumDeviceInfo* pDeviceInfo = pd3dEnum->GetDeviceInfo(AdapterOrdinal,
	                                   DeviceType);

	if (pDeviceInfo)
	{
		for (int iDeviceCombo = 0;
		        iDeviceCombo < pDeviceInfo->deviceSettingsComboList.GetSize(); iDeviceCombo++)
		{
			CD3D9EnumDeviceSettingsCombo* pDeviceSettingsCombo =
			    pDeviceInfo->deviceSettingsComboList.GetAt(
			        iDeviceCombo);
...
HRESULT DXUTCreate3DEnvironment11(ID3D11Device* pd3d11DeviceFromApp)
{
	...
	if (pd3d11DeviceFromApp == NULL)
	{
		...
		{
			hr = pDXGIFactory->EnumAdapters1(pNewDeviceSettings->d3d11.AdapterOrdinal,
			                                 &pAdapter);
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr) && pDXGIDev)
			{
				if (pAdapter == NULL)
				{
					...
					V_RETURN(pTempAdapter->QueryInterface(__uuidof(IDXGIAdapter1),
					                                      (LPVOID*) &pAdapter));
		...
		hr = pDXGIFactory->CreateSwapChain(pd3d11Device, &pNewDeviceSettings->d3d11.sd,
		                                   &pSwapChain);
	...
	if (pNewDeviceSettings->d3d11.DriverType == D3D_DRIVER_TYPE_REFERENCE
	        && GetDXUTState().GetExitCode() == 0)
	{
		GetDXUTState().SetExitCode(10);
	}
	else if (pNewDeviceSettings->d3d11.DriverType == D3D_DRIVER_TYPE_HARDWARE
	         && GetDXUTState().GetExitCode() == 10)
	{
		GetDXUTState().SetExitCode(0);
	}
	...
	assert(pd3dEnum != NULL);
	CD3D11EnumAdapterInfo* pAdapterInfo = pd3dEnum->GetAdapterInfo(
	        pNewDeviceSettings->d3d11.AdapterOrdinal);
	DXUTUpdateD3D11DeviceStats(pNewDeviceSettings->d3d11.DriverType,
	                           &pAdapterInfo->AdapterDesc);
	// Call the app's device created callback if non-NULL
	const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc =
	    DXUTGetDXGIBackBufferSurfaceDesc();
	GetDXUTState().SetInsideDeviceCallback(true);
	LPDXUTCALLBACKD3D11DEVICECREATED pCallbackDeviceCreated =
	    GetDXUTState().GetD3D11DeviceCreatedFunc();
	hr = S_OK;
	...
	hr = DXUTCreateD3D11Views(pd3d11Device, pd3dImmediateContext,
	                          pNewDeviceSettings);
	...
	GetDXUTState().SetInsideDeviceCallback(true);
	LPDXUTCALLBACKD3D11SWAPCHAINRESIZED pCallbackSwapChainResized =
	    GetDXUTState().GetD3D11SwapChainResizedFunc();
	hr = S_OK;

	if (pCallbackSwapChainResized != NULL)
		hr = pCallbackSwapChainResized(DXUTGetD3D11Device(), pSwapChain,
...
void DXUTRender3DEnvironment11()
{
	...
	if (DXUTIsRenderingPaused() || !DXUTIsActive()
	        || GetDXUTState().GetRenderingOccluded())
	{
	...
	UINT SyncInterval =
	    GetDXUTState().GetCurrentDeviceSettings()->d3d11.SyncInterval;
	// Show the frame on the primary surface.
...
void ClearD3D11DeviceContext(ID3D11DeviceContext* pd3dDeviceContext)
{
	...
	// IA clear
	pd3dDeviceContext->IASetVertexBuffers(0, 16, pBuffers, StrideOffset,
	                                      StrideOffset);
...
void DXUTCleanup3DEnvironment11(bool bReleaseSettings)
{
	...
	if (pd3dDevice != NULL)
	{
		...
		if (GetDXUTState().GetDeviceObjectsReset())
		{
			...
			if (pCallbackSwapChainReleasing != NULL)
			{
				pCallbackSwapChainReleasing(
				    GetDXUTState().GetD3D11SwapChainReleasingFuncUserContext());
			}
		...
		{
			LPDXUTCALLBACKD3D11DEVICEDESTROYED pCallbackDeviceDestroyed =
			    GetDXUTState().GetD3D11DeviceDestroyedFunc();

			if (pCallbackDeviceDestroyed != NULL)
			{
				pCallbackDeviceDestroyed(
				    GetDXUTState().GetD3D11DeviceDestroyedFuncUserContext());
			}
		...
		ID3D11DeviceContext* pd3d11DeviceContext =
		    GetDXUTState().GetD3D11DeviceContext();
		...
		DXGI_SURFACE_DESC* pBackBufferSurfaceDesc =
		    GetDXUTState().GetBackBufferSurfaceDescDXGI();
...
//--------------------------------------------------------------------------------------
LRESULT CALLBACK DXUTLowLevelKeyboardProc(int nCode, WPARAM wParam,
        LPARAM lParam)
{
...
//--------------------------------------------------------------------------------------
void WINAPI DXUTSetShortcutKeySettings(bool bAllowWhenFullscreen,
...
//--------------------------------------------------------------------------------------
HRESULT WINAPI DXUTSetTimer(LPDXUTCALLBACKTIMER pCallbackTimer,
                            float fTimeoutInSecs, UINT* pnIDEvent,
                            void* pCallbackUserContext)
{
...
void DXUTDisplayErrorMessage(HRESULT hr)
{
	...
	case DXUTERR_NODIRECT3D:
	{
		...
		if (DXUTDoesAppSupportD3D11() && !DXUTDoesAppSupportD3D9())
		{
			wcscpy_s(strBuffer, ARRAYSIZE(strBuffer),
			         L"Could not initialize Direct3D 11. ");
		}
		else
		{
			wcscpy_s(strBuffer, ARRAYSIZE(strBuffer),
	...
	case DXUTERR_NOCOMPATIBLEDEVICES:
		...
		{
			wcscpy_s(strBuffer, ARRAYSIZE(strBuffer),
			         L"Direct3D does not work over a remote session.");
		}
		else
		{
			wcscpy_s(strBuffer, ARRAYSIZE(strBuffer),
			         L"Could not find any compatible Direct3D devices.");
		}

		break;

	case DXUTERR_MEDIANOTFOUND:          nExitCode = 4;
		wcscpy_s(strBuffer, ARRAYSIZE(strBuffer), L"Could not find required media.");
		break;

	case DXUTERR_NONZEROREFCOUNT:        nExitCode = 5;
		wcscpy_s(strBuffer, ARRAYSIZE(strBuffer),
		         L"The Direct3D device has a non-zero reference count, meaning some objects were not released.");
		break;

	case DXUTERR_CREATINGDEVICE:         nExitCode = 6;
		wcscpy_s(strBuffer, ARRAYSIZE(strBuffer),
		         L"Failed creating the Direct3D device."); break;

	case DXUTERR_RESETTINGDEVICE:        nExitCode = 7;
		wcscpy_s(strBuffer, ARRAYSIZE(strBuffer),
		         L"Failed resetting the Direct3D device."); break;

	case DXUTERR_CREATINGDEVICEOBJECTS:  nExitCode = 8;
		wcscpy_s(strBuffer, ARRAYSIZE(strBuffer),
		         L"An error occurred in the device create callback function."); break;

	case DXUTERR_RESETTINGDEVICEOBJECTS: nExitCode = 9;
		wcscpy_s(strBuffer, ARRAYSIZE(strBuffer),
		         L"An error occurred in the device reset callback function."); break;

		// nExitCode 10 means the app exited using a REF device
	case DXUTERR_DEVICEREMOVED:          nExitCode = 11;
		wcscpy_s(strBuffer, ARRAYSIZE(strBuffer), L"The Direct3D device was removed.");
		break;

	default: bFound = false; nExitCode = 1;
	...
	if (bFound && bShowMsgBoxOnError)
	{
		...
		{
			MessageBox(DXUTGetHWND(), strBuffer, L"DXUT Application",
			           MB_ICONERROR | MB_OK);
		}
		else
		{
			MessageBox(DXUTGetHWND(), strBuffer, DXUTGetWindowTitle(),
			           MB_ICONERROR | MB_OK);
		}
	}
}
...
HRESULT WINAPI DXUTToggleFullScreen()
{
	...
	if (deviceSettings.ver == DXUT_D3D11_DEVICE)
	{
		...
		if (!deviceSettings.d3d11.sd.Windowed)
		{
			...
			DXUTGetD3D11AdapterDisplayMode(deviceSettings.d3d11.AdapterOrdinal, 0,
			                               &adapterDesktopDisplayMode);
	...
	if (FAILED(hr) && (hr != E_ABORT))
	{
		...
		HRESULT hr2 = DXUTChangeDevice(&orginalDeviceSettings, NULL,  NULL, false,
		                               false);
...
void DXUTCheckForDXGIFullScreenSwitch()
{
	...
	if (!DXUTIsD3D9(pDeviceSettings))
	{
		...
		if (bIsWindowed != SCDesc.Windowed)
		{
			...
			{
				GetDXUTState().SetWindowBackBufferWidthAtModeChange(
				    deviceSettings.d3d11.sd.BufferDesc.Width);
				GetDXUTState().SetWindowBackBufferHeightAtModeChange(
				    deviceSettings.d3d11.sd.BufferDesc.Height);
			}
			else
			{
				GetDXUTState().SetFullScreenBackBufferWidthAtModeChange(
...
void DXUTCheckForWindowSizeChange()
{
	...
	if (DXUTIsD3D9(&deviceSettings))
	{
		...
		if ((UINT)rcCurrentClient.right != DXUTGetBackBufferWidthFromDS(
		            &deviceSettings) ||
...
void DXUTCheckForWindowChangingMonitors()
{
	...
	if (!GetDXUTState().GetAutoChangeAdapter() ||
	        GetDXUTState().GetIgnoreSizeChange() || !GetDXUTState().GetDeviceCreated()
	        || !DXUTIsWindowed())
	{
		return;
	}
...
HMONITOR DXUTGetMonitorFromAdapter(DXUTDeviceSettings* pDeviceSettings)
{
	...
	else if (pDeviceSettings->ver == DXUT_D3D11_DEVICE)
	{
		CD3D11Enumeration* pD3DEnum = DXUTGetD3D11Enumeration();
		assert(pD3DEnum != NULL);
		CD3D11EnumOutputInfo* pOutputInfo = pD3DEnum->GetOutputInfo(
		                                        pDeviceSettings->d3d11.AdapterOrdinal,
		...
		return DXUTMonitorFromRect(&pOutputInfo->Desc.DesktopCoordinates,
        ...
        UINT* pAdapterOrdinal)
{
	*pAdapterOrdinal = 0;
	...
	if (DXUTIsCurrentDeviceD3D9())
	{
		CD3D9Enumeration* pd3dEnum = DXUTGetD3D9Enumeration();
		IDirect3D9* pD3D = DXUTGetD3D9Object();
		CGrowableArray <CD3D9EnumAdapterInfo*>* pAdapterList =
		    pd3dEnum->GetAdapterInfoList();

		for (int iAdapter = 0; iAdapter < pAdapterList->GetSize(); iAdapter++)
		{
			CD3D9EnumAdapterInfo* pAdapterInfo = pAdapterList->GetAt(iAdapter);
			HMONITOR hAdapterMonitor = pD3D->GetAdapterMonitor(
			                               pAdapterInfo->AdapterOrdinal);
	...
	else
	{
		...
		CD3D11Enumeration* pd3dEnum = DXUTGetD3D11Enumeration();
		CGrowableArray <CD3D11EnumAdapterInfo*>* pAdapterList =
		    pd3dEnum->GetAdapterInfoList();
		...
		for (int iAdapter = 0; iAdapter < pAdapterList->GetSize(); ++iAdapter)
		{
			...
			for (int o = 0; o < pAdapterInfo->outputInfoList.GetSize(); ++o)
			{
				...
				if (wcsncmp(pOutputInfo->Desc.DeviceName, mi.szDevice,
				            sizeof(mi.szDevice) / sizeof
				            (mi.szDevice[0])) == 0)
				{
					*pAdapterOrdinal = pAdapterInfo->AdapterOrdinal;
					return S_OK;
...
HRESULT DXUTHandleDeviceRemoved()
{
	...
	LPDXUTCALLBACKDEVICEREMOVED pDeviceRemovedFunc =
	    GetDXUTState().GetDeviceRemovedFunc();

	if (pDeviceRemovedFunc)
	{
		bLookForNewDevice = pDeviceRemovedFunc(
		                        GetDXUTState().GetDeviceRemovedFuncUserContext());
	}
...
void DXUTUpdateBackBufferDesc()
{
	if (DXUTIsCurrentDeviceD3D9())
	{
		...
		hr = GetDXUTState().GetD3D9Device()->GetBackBuffer(0, 0,
	...
	if (DXUTIsCurrentDeviceD3D9())
	{
		...
		if (!DXUTIsWindowed() && pd3dDevice)
		{
			if (GetDXUTState().GetShowCursorWhenFullScreen())
			{
				...
				HCURSOR hCursor = (HCURSOR)(ULONG_PTR)GetClassLongPtr(
...
void DXUTUpdateStaticFrameStats()
{
	...
	if (DXUTIsD3D9(pDeviceSettings))
	{
		...
		CD3D9EnumDeviceSettingsCombo* pDeviceSettingsCombo =
		    pd3dEnum->GetDeviceSettingsCombo(
		...
		if (pDeviceSettingsCombo->AdapterFormat ==
		        pDeviceSettingsCombo->BackBufferFormat)
		{
			wcscpy_s(strFmt, 100, DXUTD3DFormatToString(pDeviceSettingsCombo->AdapterFormat,
			         false));
		}
		else
		{
		...
		{
			swprintf_s(strDepthFmt, 100, L" (%s)",
			           DXUTD3DFormatToString(pPP->AutoDepthStencilFormat, false));
		}
		else
		{
			// No depth buffer
	...
	else
	{
		...
		CD3D11EnumDeviceSettingsCombo* pDeviceSettingsCombo =
		    pd3dEnum->GetDeviceSettingsCombo(
		        pDeviceSettings->d3d11.AdapterOrdinal, pDeviceSettings->d3d11.DriverType,
		        pDeviceSettings->d3d11.Output,
		        pDeviceSettings->d3d11.sd.BufferDesc.Format,
		        pDeviceSettings->d3d11.sd.Windowed);
		...
		WCHAR strFmt[100];
		wcscpy_s(strFmt, 100, DXUTDXGIFormatToString(
		             pDeviceSettingsCombo->BackBufferFormat, false));
		WCHAR strMultiSample[100];
		swprintf_s(strMultiSample, 100, L" (MS%u, Q%u)",
		           pDeviceSettings->d3d11.sd.SampleDesc.Count,
		...
		swprintf_s(pstrStaticFrameStats, 256, L"D3D11 %%sVsync %s (%dx%d), %s%s",
		           ...
		           pDeviceSettings->d3d11.sd.BufferDesc.Width,
		           pDeviceSettings->d3d11.sd.BufferDesc.Height,
	...
	if (DeviceType == D3DDEVTYPE_HAL)
	{
		...
		if (pDeviceSettingsCombo)
		{
			wcscat_s(pstrDeviceStats, 256,
			         pDeviceSettingsCombo->pAdapterInfo->szUniqueDescription);
		}
		else
		{
			const int cchDesc = sizeof(pAdapterIdentifier->Description);
			WCHAR szDescription[cchDesc];
			MultiByteToWideChar(CP_ACP, 0, pAdapterIdentifier->Description, -1,
			                    szDescription, cchDesc);
...
HRESULT WINAPI DXUTGetD3D9DeviceCaps(DXUTDeviceSettings* pDeviceSettings,
                                     D3DCAPS9* pCaps)
{
	IDirect3D9* pD3D = DXUTGetD3D9Object();
	assert(pD3D != NULL);
	return pD3D->GetDeviceCaps(pDeviceSettings->d3d9.AdapterOrdinal,
	                           pDeviceSettings->d3d9.DeviceType, pCaps);
}
...
bool WINAPI DXUTIsKeyDown(BYTE vKey)
{
	...
	if (vKey >= 0xA0
	        && vKey <=
	        0xA5)   // VK_LSHIFT, VK_RSHIFT, VK_LCONTROL, VK_RCONTROL, VK_LMENU, VK_RMENU
	{
		return GetAsyncKeyState(vKey) !=
		       0;    // these keys only are tracked via GetAsyncKeyState()
	}
	else if (vKey >= 0x01 && vKey <= 0x06
	         && vKey != 0x03)  // mouse buttons (VK_*BUTTON)
	{
...
void WINAPI DXUTSetHotkeyHandling(bool bAltEnterToToggleFullscreen,
                                  bool bEscapeToQuit, bool bPauseToToggleTimePause)
{
...
void WINAPI DXUTSetCursorSettings(bool bShowCursorWhenFullScreen,
...
void DXUTApplyDefaultDeviceSettings(DXUTDeviceSettings* modifySettings)
{
	...
	modifySettings->d3d11.sd.BufferDesc.ScanlineOrdering =
	...
	modifySettings->d3d11.sd.SampleDesc.Count = 4;
	modifySettings->d3d11.sd.SampleDesc.Quality = 1;
	...
	modifySettings->d3d9.pp.BackBufferHeight = 1080;
	modifySettings->d3d9.pp.BackBufferWidth = 1920;
...
//--------------------------------------------------------------------------------------
HRESULT DXUTSnapDeviceSettingsToEnumDevice(DXUTDeviceSettings* pDeviceSettings,
        bool forceEnum,  D3D_FEATURE_LEVEL forceFL)
{
	bool bAppSupportsD3D9 = DXUTDoesAppSupportD3D9();
	bool bAppSupportsD3D11 = DXUTDoesAppSupportD3D11();
	...
	if (bAppSupportsD3D11 && pDeviceSettings->ver == DXUT_D3D11_DEVICE)
	{
		...
		for (int iDeviceCombo = 0;
		        iDeviceCombo < pAdapterInfo->deviceSettingsComboList.GetSize(); iDeviceCombo++)
		{
			CD3D11EnumDeviceSettingsCombo* tempDeviceSettingsCombo =
			    pAdapterInfo->deviceSettingsComboList.GetAt(iDeviceCombo);
			DXGI_MODE_DESC adapterDisplayMode;
			DXUTGetD3D11AdapterDisplayMode(pAdapterInfo->AdapterOrdinal, 0,
			                               &adapterDisplayMode);
			int bestMode;
			int bestMSAA;
			float score = DXUTRankD3D11DeviceCombo(tempDeviceSettingsCombo,
			                                       &(pDeviceSettings->d3d11), &adapterDisplayMode, bestMode, bestMSAA);
		...
		if (pDeviceSettingsCombo->pOutputInfo != NULL)
		{
			...
			bestDisplayMode = pDeviceSettingsCombo->pOutputInfo->displayModeList.GetAt(
			                      bestModeIndex);
			...
			if (!pDeviceSettingsCombo->Windowed)
			{
				...
				pDeviceSettings->d3d11.sd.BufferDesc.RefreshRate.Numerator =
				    bestDisplayMode.RefreshRate.Numerator;
				pDeviceSettings->d3d11.sd.BufferDesc.RefreshRate.Denominator =
				    bestDisplayMode.RefreshRate.Denominator;
				pDeviceSettings->d3d11.sd.BufferDesc.Scaling = bestDisplayMode.Scaling;
				pDeviceSettings->d3d11.sd.BufferDesc.ScanlineOrdering =
				    bestDisplayMode.ScanlineOrdering;
			}
		}

		if (pDeviceSettings->d3d11.DeviceFeatureLevel == 0)
		{
			pDeviceSettings->d3d11.DeviceFeatureLevel =
			    pDeviceSettingsCombo->pDeviceInfo->SelectedLevel;
		}

		pDeviceSettings->d3d11.sd.SampleDesc.Count =
		    pDeviceSettingsCombo->multiSampleCountList.GetAt(bestMSAAIndex);

		if (pDeviceSettings->d3d11.sd.SampleDesc.Quality >
		        pDeviceSettingsCombo->multiSampleQualityList.GetAt(bestMSAAIndex) - 1)
		{
			pDeviceSettings->d3d11.sd.SampleDesc.Quality =
			    pDeviceSettingsCombo->multiSampleQualityList.GetAt(bestMSAAIndex) - 1;
		}
	...
	if (bAppSupportsD3D9)
	{
		...
		for (int iAdapter = 0; iAdapter < pAdapterList->GetSize(); iAdapter++)
		{
			...
			pD3D->GetAdapterDisplayMode(pAdapterInfo->AdapterOrdinal,
			                            &adapterDesktopDisplayMode);

			// Enum all the device types supported by this adapter to find the best device settings
			for (int iDeviceInfo = 0; iDeviceInfo < pAdapterInfo->deviceInfoList.GetSize();
			        iDeviceInfo++)
			{
				CD3D9EnumDeviceInfo* pDeviceInfo = pAdapterInfo->deviceInfoList.GetAt(
				                                       iDeviceInfo);
		...
		pDeviceSettings->d3d9.AdapterFormat =
		    pBestDeviceSettingsCombo->AdapterFormat;//D3DFMT_X8R8G8B8;
		pDeviceSettings->d3d9.AdapterOrdinal = pBestDeviceSettingsCombo->AdapterOrdinal
		                                       ;//0;
		pDeviceSettings->d3d9.DeviceType = pBestDeviceSettingsCombo->DeviceType;
		pDeviceSettings->d3d9.pp.BackBufferFormat =
		    pBestDeviceSettingsCombo->BackBufferFormat;//D3DFMT_X8R8G8B8;
		...
		if (!pBestDeviceSettingsCombo->Windowed)
		{
			D3DDISPLAYMODE displayMode =
			    pBestDeviceSettingsCombo->pAdapterInfo->displayModeList.GetAt(bestModeIndex);
		...
		pDeviceSettings->d3d9.pp.hDeviceWindow = pBestDeviceSettingsCombo->Windowed ?
		        DXUTGetHWNDDeviceWindowed() : DXUTGetHWNDDeviceFullScreen();

		if (pDeviceSettings->d3d9.pp.MultiSampleQuality >
		        pBestDeviceSettingsCombo->multiSampleQualityList.GetAt(bestMSAAIndex) - 1)
		{
			pDeviceSettings->d3d9.pp.MultiSampleQuality =
			    pBestDeviceSettingsCombo->multiSampleQualityList.GetAt(bestMSAAIndex) - 1;
		}

		pDeviceSettings->d3d9.pp.MultiSampleType =
		    pBestDeviceSettingsCombo->multiSampleTypeList.GetAt(bestMSAAIndex);
