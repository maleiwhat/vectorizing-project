...
UINT(WINAPI* _GetReadingString)(HIMC himc, UINT uReadingBufLen,
                                LPWSTR lpwReadingBuf, PINT pnErrorIndex,
                                BOOL* pfIsVertical, PUINT puMaxReadingLen);
BOOL (WINAPI* _ShowReadingWindow)(HIMC himc, BOOL bShow);

// Callbacks
void (CALLBACK* ImeUiCallback_DrawRect)(int x1, int y1, int x2, int y2,
                                        DWORD color);
void (CALLBACK* ImeUiCallback_DrawFans)(const IMEUI_VERTEX* paVertex,
                                        UINT uNum);
...
static LRESULT(WINAPI* _SendMessage)(HWND hwnd, UINT msg, WPARAM wp,
                                     LPARAM lp) = SendMessageA;
static DWORD (* _GetCandidateList)(HIMC himc, DWORD dwIndex,
...
static int
g_iReadingError;    // Used only in case of horizontal reading window
...
protected:
	...
	static void MakeReadingInformationString(ITfReadingInformationUIElement*
	        preading);
	...
	static int
	m_nCandidateRefCount;    // Some IME shows multiple candidate lists but the Library doesn't support multiple candidate list.
...
DWORD                           CTsfUiLessMode::m_dwUIElementSinkCookie =
    TF_INVALID_COOKIE;
DWORD                           CTsfUiLessMode::m_dwAlpnSinkCookie =
    TF_INVALID_COOKIE;
DWORD                           CTsfUiLessMode::m_dwOpenModeSinkCookie =
    TF_INVALID_COOKIE;
DWORD                           CTsfUiLessMode::m_dwConvModeSinkCookie =
    TF_INVALID_COOKIE;
...
static void AW_SendCompString()
{
	...
	iLen = WideCharToMultiByte(g_uCodePage, 0, g_szCompositionString, -1,
	                           (LPSTR)szCompStr, COUNTOF(szCompStr), NULL,
	                           NULL) - 1; // don't need to send NUL terminator;
...
static DWORD AW_GetCandidateList(HIMC himc, DWORD dwIndex,
                                 LPCANDIDATELIST* ppCandList)
{
	...
	if (dwBufLen)
	{
		...
		if (pCandList)
		{
			...
			if (dwBufLen)
			{
				...
				{
					wideBufLen += MultiByteToWideChar(g_uCodePage, 0,
					                                  (LPSTR)pCandList + pCandList->dwOffset[i], -1, NULL, 0) * sizeof(WCHAR);
				}
				...
				*ppCandList = (LPCANDIDATELIST)ImeUiCallback_Malloc(wideBufLen);
				...
				for (i = 0; i < (int)pCandList->dwCount; i++)
				{
					...
					pwz += MultiByteToWideChar(g_uCodePage, 0,
					                           (LPSTR)pCandList + pCandList->dwOffset[i], -1, pwz, 256);
				}
...
static LONG WINAPI AW_ImmGetCompositionString(HIMC himc, DWORD dwIndex,
        LPVOID lpBuf, DWORD dwBufLen)
{
	...
	case GCS_COMPSTR:
		...
		{
			pszMb[dwRet] = 0;
			dwRet = (DWORD)MultiByteToWideChar(g_uCodePage, 0, pszMb, -1, (LPWSTR)lpBuf,
			                                   dwBufLen);
	...
	case GCS_COMPATTR:
	{
		...
		DWORD dwRet2 = ImmGetCompositionStringA(himc, GCS_COMPSTR, pszMb2,
		                                        sizeof(pszMb2));
...
static DWORD WA_GetCandidateList(HIMC himc, DWORD dwIndex,
                                 LPCANDIDATELIST* ppCandList)
{
...
static LONG WINAPI WA_ImmGetCompositionString(HIMC himc, DWORD dwIndex,
        LPVOID lpBuf, DWORD dwBufLen)
{
	...
	case GCS_COMPSTR:
		if (dwRet)
		{
			...
			dwRet = (DWORD)WideCharToMultiByte(g_uCodePage, 0, pwzUc, -1, (LPSTR)lpBuf,
			                                   dwBufLen, NULL,
			                                   NULL);
	...
	case GCS_CURSORPOS:
	{
		...
		DWORD dwRet2 = ImmGetCompositionStringW(himc, GCS_COMPSTR, pwzUc2,
		                                        sizeof(pwzUc2));
		...
		char pszMb[bufSize];
		int iRc = WideCharToMultiByte(g_uCodePage, 0, pwzUc2, dwRet2, pszMb,
		                              sizeof(pszMb), NULL, NULL);
		assert(iRc >
		       0);   //WideCharToMultiByte returns 0 if it failed, and it should *never* be negative in the first place

		if (iRc >=
		        bufSize)  //if we wrote more bytes than the length of the buffer, we need to terminate it
		{
...
static void SendCompString()
{
	...
	if (ImeUiCallback_OnChar)
	{
		...
		pwz = szUnicode;
		iLen = MultiByteToWideChar(g_uCodePage, 0, g_szCompositionString, -1, szUnicode,
		                           COUNTOF(szUnicode)) - 1;
#endif
...
static DWORD GetCandidateList(HIMC himc, DWORD dwIndex,
                              LPCANDIDATELIST* ppCandList)
{
...
static void CancelCompString(HWND hwnd, bool bUseBackSpace = true,
                             int iNewStrLen = 0)
{
	if (g_dwIMELevel != 3)
	{
...
static void DrawImeIndicator()
{
	...
	if (gSkinIME.symbolFont)
	{
		...
		gSkinIME.symbolFont->SetColor((((DWORD)gSkinIME.symbolTranslucence) << 24) |
		                              gSkinIME.symbolColorText);
		//
		// draw the proper symbol over the fan
		//
		DWORD w, h;
		LPCTSTR cszSymbol = (g_dwState == IMEUI_STATE_ON) ? g_pszIndicatior :
		                    g_aszIndicator[0];
...
static void DrawCompositionString(bool bDrawCompAttr)
{
	...
	if (GETPRIMLANG() != LANG_KOREAN
	        || g_bCaretDraw)    // Korean uses composition attribute as blinking block caret
		for (i = 0; i < len; i += cType)
		{
			...
			if (cType == 2
			        && g_szCompositionString[i + 1])  // in case we have 0 in trailbyte, we don't count it.
			{
...
static void DrawCandidateList()
{
	...
	{
		g_CaretInfo.pFont->GetTextExtent(g_szReadingString, (DWORD*)&largest.cx,
		                                 (DWORD*)&largest.cy);
	}
	else
	{
	...
	else if (g_bVerticalCand)
	{
		...
		if (!g_bReadingWindow)
		{
			...
			if (ImeUiCallback_DrawRect)
			{
				ImeUiCallback_DrawRect(left, top, seperateLineX - 1, bottom,
				                       gSkinIME.candColorBase);
				ImeUiCallback_DrawRect(seperateLineX, top, right, bottom,
				                       gSkinIME.candColorBase);
			}
		}
	}
	...
	if (g_bReadingWindow && g_bHorizontalReading)
	{
		...
		if (iStart >= 0)
		{
			...
			{
				g_CaretInfo.pFont->SetColor(0xff000000 + (~((0x00ffffff) &
...
//
LPARAM ImeUi_ProcessMessage(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM& lParam,
                            bool* trapped)
{
	HIMC himc;
	int len;
...
void ImeUi_SetCompStringAppearance(CImeUiFont_Base* pFont, DWORD color,
                                   const RECT* prc)
{
	if (!g_bInitialized)
	{
...
void ImeUi_SetState(DWORD dwState)
{
	...
	if (NULL != (himc = _ImmGetContext(g_hwndCurr)))
	{
		...
		case LANG_KOREAN:
			...
			// toggle Korean IME
			if ((bOn && g_dwState != IMEUI_STATE_ON) || (!bOn
			        && g_dwState == IMEUI_STATE_ON))
			{
...
bool ImeUi_Initialize(HWND hwnd, bool bDisable)
{
	...
	if (g_hImmDll)
	{
		_ImmLockIMC = (LPINPUTCONTEXT2(WINAPI*)(HIMC hIMC))GetProcAddress(g_hImmDll,
		              "ImmLockIMC");
		_ImmUnlockIMC = (BOOL (WINAPI*)(HIMC hIMC))GetProcAddress(g_hImmDll,
		                "ImmUnlockIMC");
		_ImmLockIMCC = (LPVOID (WINAPI*)(HIMCC hIMCC))GetProcAddress(g_hImmDll,
		               "ImmLockIMCC");
		_ImmUnlockIMCC = (BOOL (WINAPI*)(HIMCC hIMCC))GetProcAddress(g_hImmDll,
		                 "ImmUnlockIMCC");
		BOOL (WINAPI * _ImmDisableTextFrameService)(DWORD) = (BOOL (WINAPI*)(
		            DWORD))GetProcAddress(g_hImmDll,
...
#define LCID_INVARIANT MAKELCID(MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), SORT_DEFAULT)
		...
		if ((CompareStringA(LCID_INVARIANT, NORM_IGNORECASE, szTmp, -1,
		                    _CHT_IMEFILENAME, -1) != 2)
		        && (CompareStringA(LCID_INVARIANT, NORM_IGNORECASE, szTmp, -1,
		                           _CHT_IMEFILENAME2, -1) != 2)
		        && (CompareStringA(LCID_INVARIANT, NORM_IGNORECASE, szTmp, -1,
		                           _CHT_IMEFILENAME3, -1) != 2)
		        && (CompareStringA(LCID_INVARIANT, NORM_IGNORECASE, szTmp, -1, _CHS_IMEFILENAME,
		                           -1) != 2)
		        && (CompareStringA(LCID_INVARIANT, NORM_IGNORECASE, szTmp, -1,
		                           _CHS_IMEFILENAME2, -1) != 2)
		   )
		{
			goto error;
		}
...
static void GetReadingString(HWND hWnd)
{
	...
	if (_GetReadingString)
	{
		...
		if (dwlen)
		{
			...
			dwlen = _GetReadingString(himc, dwlen, wstr, (PINT)&dwerr, &bVertical,
			                          &uMaxUiLen);
		}
	...
	else // IMEs that doesn't implement Reading String API
	{
		...
		case IMEID_CHT_VER50: // 5.0.x.x // WinME
			p = *(LPBYTE*)((LPBYTE)_ImmLockIMCC(lpIMC->hPrivate) + 3 *
			               4);         // PCKeyCtrlManager

			if (!p) { break; }

			p = *(LPBYTE*)((LPBYTE)p + 1 * 4 + 5 * 4 + 4 *
			               2);       // = PCReading = &STypingInfo

			if (!p) { break; }

			dwlen = *(DWORD*)(p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4 +
			                  16);              //m_dwDisplayStringLength;
			dwerr = *(DWORD*)(p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4 + 16 + 1 *
			                  4);          //m_dwDisplayErrorStart;
		...
		case IMEID_CHS_VER53: // 5.3.x.x // SCIME2k or MSPY3 (w/OfficeXP and Whistler)
			...
			p = *(LPBYTE*)((LPBYTE)p + 1 * 4 + 5 *
			               4);                             // = PCReading = &STypingInfo

			if (!p) { break; }

			dwlen = *(DWORD*)(p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4 + 16 *
			                  2);              //m_dwDisplayStringLength;
			dwerr = *(DWORD*)(p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4 + 16 * 2 + 1 *
			                  4);          //m_dwDisplayErrorStart;
			wstr = (WCHAR*)(p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4);
			unicode = TRUE;
			break;
		...
		case IMEID_CHS_VER42: // 4.2.x.x // SCIME98 or MSPY2 (w/Office2k, Win2k, WinME, etc)
		{
			...
			p = *(LPBYTE*)((LPBYTE)_ImmLockIMCC(lpIMC->hPrivate) + 1 * 4 + 1 * 4 + 6 *
			               4);         // = PCReading = &STypintInfo

			if (!p) { break; }

			dwlen = *(DWORD*)(p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4 + 16 *
			                  nTcharSize);       //m_dwDisplayStringLength;
			dwerr = *(DWORD*)(p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4 + 16 * nTcharSize + 1 *
			                  4);    //m_dwDisplayErrorStart;
			wstr = (WCHAR*)(p + 1 * 4 + (16 * 2 + 2 * 4) + 5 *
			                4);                       //m_tszDisplayString
	...
	else
	{
		...
		for (i = 0, j = 0; (DWORD)i < dwlen;
		        i++, j++)    // dwlen > 0, if known IME : yutakah
		{
			if (dwerr <= (DWORD)i && g_dwSelection == (DWORD) - 1)
			{
...
void ImeUi_FinalizeString(bool bSend)
{
	...
	if (g_bUILessMode)
	{
		...
		ImmSetCompositionString(himc, SCS_SETSTR, TEXT(""), sizeof(TCHAR), TEXT(""),
		                        sizeof(TCHAR));
	}
...
static void GetReadingWindowOrientation(DWORD dwId)
{
	g_bHorizontalReading = (g_hklCurrent == _CHS_HKL)
	                       || (g_hklCurrent == _CHT_HKL_NEW_CHANG_JIE) || (dwId == 0);
	...
	if (!g_bHorizontalReading && IMEID_LANG(dwId) == LANG_CHT)
	{
		...
		StringCchCopyA(szRegPath, COUNTOF(szRegPath),
		               "software\\microsoft\\windows\\currentversion\\");
		strcat_s(szRegPath, COUNTOF(szRegPath), (dwVer >= MAKEIMEVERSION(5,
		         1)) ? "MSTCIPH" : "TINTLGNT");
		...
		if (lRc == ERROR_SUCCESS)
		{
			...
			lRc = RegQueryValueExA(hkey, "keyboard mapping", NULL, &dwType,
...
void ImeUi_ToggleLanguageBar(BOOL bRestore)
{
	...
	if (iShowStatusWindow == -1)
	{
		iShowStatusWindow = IsNT() && g_osi.dwMajorVersion >= 5 &&
		                    (g_osi.dwMinorVersion > 1 || (g_osi.dwMinorVersion == 1
		                            && lstrlenA(g_osi.szCSDVersion))) ? 1 : 0;
	}
...
static void CheckInputLocale()
{
	...
	int iRc = GetLocaleInfoA(MAKELCID(GETLANG(), SORT_DEFAULT),
	                         LOCALE_IDEFAULTANSICODEPAGE, szCodePage,
	...
	if (m_TsfSink)
	{
		...
		if (SUCCEEDED(hr = m_tm->QueryInterface(__uuidof(ITfSource), (void**)&srcTm)))
		{
			                                     ...
			                                     &m_dwUIElementSinkCookie)))
			{
				...
				if (SUCCEEDED(hr = srcTm->AdviseSink(__uuidof(
				        ITfInputProcessorProfileActivationSink),
...
void CTsfUiLessMode::ReleaseSinks()
{
	...
	// Remove all sinks
	if (m_tm && SUCCEEDED(m_tm->QueryInterface(__uuidof(ITfSource),
	                      (void**)&source)))
	{
        ...
        BOOL* pbShow)
{
	...
	*pbShow = FALSE;
	...
	if (!g_bCandList
	        && SUCCEEDED(pElement->QueryInterface(__uuidof(ITfReadingInformationUIElement),
	                     (void**)&preading)))
	{
...
void CTsfUiLessMode::UpdateImeState(BOOL bResetCompartmentEventSink)
{
	...
	if (GetCompartments(&pcm, &pTfOpenMode, &pTfConvMode))
	{
		...
		if (valOpenMode.vt == VT_I4)
		{
			if (g_bChineseIME)
			{
				g_dwState = valOpenMode.lVal != 0
				            && valConvMode.lVal != 0 ? IMEUI_STATE_ON : IMEUI_STATE_ENGLISH;
			}
			else
			{
...
STDAPI CTsfUiLessMode::CUIElementSink::OnActivated(DWORD dwProfileType,
        LANGID langid, REFCLSID clsid, REFGUID catid,
        REFGUID guidProfile, HKL hkl, DWORD dwFlags)
{
	...
	{
		g_bChineseIME = (dwProfileType & TF_PROFILETYPE_INPUTPROCESSOR)
		                && langid == LANG_CHT;
    ...
    ITfReadingInformationUIElement* preading)
{
	...
	if (SUCCEEDED(preading->GetString(&bstr)))
	{
		if (bstr)
		{
			...
			szStr[0] = 0;
			int iRc = WideCharToMultiByte(CP_ACP, 0, bstr, -1, szStr, sizeof(szStr), NULL,
			                              NULL);
...
void CTsfUiLessMode::MakeCandidateStrings(ITfCandidateListUIElement* pcandidate)
{
	...
	for (UINT i = dwPageStart, j = 0; (DWORD)i < g_dwCount
	        && j < g_uCandPageSize; i++, j++)
	{
        ...
        ITfCompartment* pTfConvMode)
{
	                   ...
	                   (void**)&srcOpenMode)))
	{
		...
		if (bRemoveOnly
		        || SUCCEEDED(hr = srcOpenMode->AdviseSink(IID_ITfCompartmentEventSink,
		                          ...
		                          &m_dwOpenModeSinkCookie)))
		{
			                   ...
			                   (void**)&srcConvMode)))
			{
				...
				// Setup sink for open mode (toggle state) change
				if (bRemoveOnly
				        || SUCCEEDED(hr = srcConvMode->AdviseSink(IID_ITfCompartmentEventSink,
				                          (ITfCompartmentEventSink*)m_TsfSink,
