...
//--------------------------------------------------------------------------------------
void CDXUTSDKMesh::LoadMaterials(ID3D11Device* pd3dDevice,
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTSDKMesh::CreateVertexBuffer(ID3D11Device* pd3dDevice,
        ...
        void* pVertices, SDKMESH_CALLBACKS11* pLoaderCallbacks)
{
	...
	{
		pLoaderCallbacks->pCreateVertexBuffer(pd3dDevice, &pHeader->pVB11, bufferDesc,
		                                      pVertices,
		                                      pLoaderCallbacks->pContext);
	}
	else
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTSDKMesh::CreateIndexBuffer(ID3D11Device* pd3dDevice,
                                        ...
                                        void* pIndices, SDKMESH_CALLBACKS11* pLoaderCallbacks)
{
	...
	{
		pLoaderCallbacks->pCreateIndexBuffer(pd3dDevice, &pHeader->pIB11, bufferDesc,
		                                     pIndices,
		                                     pLoaderCallbacks->pContext);
	}
	else
                                       ...
                                       SDKMESH_CALLBACKS9* pLoaderCallbacks9)
{
	...
	if (pDev11)
	{
		LoadMaterials(pDev11, m_pMaterialArray, m_pMeshHeader->NumMaterials,
		              pLoaderCallbacks11);
	}
	else if (pDev9)
	{
	...
	for (UINT meshi = 0; meshi < m_pMeshHeader->NumMeshes; ++meshi)
	{
		...
		{
			pSubset = GetSubset(meshi,
			                    subset);   //&m_pSubsetArray[ currentMesh->pSubsets[subset] ];
			PrimType = GetPrimitiveType11((SDKMESH_PRIMITIVE_TYPE)pSubset->PrimitiveType);
			assert(PrimType ==
			       D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);  // only triangle lists are handled.
			...
			UINT stride = (UINT)
			              m_pVertexBufferArray[currentMesh->VertexBuffers[0]].StrideBytes;
			assert(stride % 4 == 0);
			stride /= 4;
                              ...
                              UINT iSpecularSlot)
{
	...
	pd3dDeviceContext->IASetVertexBuffers(0, pMesh->NumVertexBuffers, pVB, Strides,
	                                      Offsets);
	...
	for (UINT subset = 0; subset < pMesh->NumSubsets; subset++)
	{
		...
		if (iDiffuseSlot != INVALID_SAMPLER_SLOT
		        && !IsErrorResource(pMat->pDiffuseRV11))
		{
		...
		if (iSpecularSlot != INVALID_SAMPLER_SLOT
		        && !IsErrorResource(pMat->pSpecularRV11))
		{
			pd3dDeviceContext->PSSetShaderResources(iSpecularSlot, 1,
			                                        &pMat->pSpecularRV11);
		}
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTSDKMesh::Create(ID3D11Device* pDev11, LPCTSTR szFileName,
                             bool bCreateAdjacencyIndices,
                             SDKMESH_CALLBACKS11* pLoaderCallbacks)
{
	return CreateFromFile(pDev11,  NULL, szFileName, bCreateAdjacencyIndices,
...
HRESULT CDXUTSDKMesh::Create(IDirect3DDevice9* pDev9, LPCTSTR szFileName,
                             ...
                             SDKMESH_CALLBACKS9* pLoaderCallbacks)
{
	return CreateFromFile(NULL,  pDev9, szFileName, bCreateAdjacencyIndices, NULL,
	                      pLoaderCallbacks);
}

//--------------------------------------------------------------------------------------
HRESULT CDXUTSDKMesh::Create(ID3D11Device* pDev11, BYTE* pData, UINT DataBytes,
                             bool bCreateAdjacencyIndices,
                             bool bCopyStatic, SDKMESH_CALLBACKS11* pLoaderCallbacks)
{
	return CreateFromMemory(pDev11, NULL, pData, DataBytes, bCreateAdjacencyIndices,
	                        bCopyStatic,
	                        pLoaderCallbacks,  NULL);
}
...
HRESULT CDXUTSDKMesh::Create(IDirect3DDevice9* pDev9, BYTE* pData,
                             UINT DataBytes, bool bCreateAdjacencyIndices,
                             bool bCopyStatic, SDKMESH_CALLBACKS9* pLoaderCallbacks)
{
	return CreateFromMemory(NULL, pDev9, pData, DataBytes, bCreateAdjacencyIndices,
	                        bCopyStatic, NULL,
	                        pLoaderCallbacks);
}
		...
		if (m_pMaterialArray)
		{
			for (UINT64 m = 0; m < m_pMeshHeader->NumMaterials; m++)
			{
				...
				else if (m_pDev11)
				{
					...
					if (m_pMaterialArray[m].pDiffuseRV11
					        && !IsErrorResource(m_pMaterialArray[m].pDiffuseRV11))
					{
					...
					if (m_pMaterialArray[m].pNormalRV11
					        && !IsErrorResource(m_pMaterialArray[m].pNormalRV11))
					{
					...
					if (m_pMaterialArray[m].pSpecularRV11
					        && !IsErrorResource(m_pMaterialArray[m].pSpecularRV11))
					{
...
void CDXUTSDKMesh::TransformMesh(D3DXMATRIX* pWorld, double fTime)
{
	if (m_pAnimationHeader == NULL
	        || FTT_RELATIVE == m_pAnimationHeader->FrameTransformType)
	{
...
void CDXUTSDKMesh::Render(ID3D11DeviceContext* pd3dDeviceContext,
                          ...
                          UINT iSpecularSlot)
{
	RenderFrame(0, false, pd3dDeviceContext, iDiffuseSlot, iNormalSlot,
	            iSpecularSlot);
}
...
void CDXUTSDKMesh::RenderAdjacent(ID3D11DeviceContext* pd3dDeviceContext,
                                  ...
                                  UINT iSpecularSlot)
{
	RenderFrame(0, true, pd3dDeviceContext, iDiffuseSlot, iNormalSlot,
	            iSpecularSlot);
}
...
//--------------------------------------------------------------------------------------
D3D11_PRIMITIVE_TOPOLOGY CDXUTSDKMesh::GetPrimitiveType11(
...
UINT CDXUTSDKMesh::GetOutstandingResources()
{
	...
	if (m_pDev11)
	{
		for (UINT i = 0; i < m_pMeshHeader->NumMaterials; i++)
		{
			...
			{
				if (!m_pMaterialArray[i].pDiffuseRV11
				        && !IsErrorResource(m_pMaterialArray[i].pDiffuseRV11))
				{
			...
			{
				if (!m_pMaterialArray[i].pNormalRV11
				        && !IsErrorResource(m_pMaterialArray[i].pNormalRV11))
				{
					outstandingResources ++;
				}
			}
			...
			{
				if (!m_pMaterialArray[i].pSpecularRV11
				        && !IsErrorResource(m_pMaterialArray[i].pSpecularRV11))
				{
					outstandingResources ++;
				}
			}
                               ...
                               D3DXMATERIAL* pd3dxMaterials, DWORD dwMaterials)
{
	...
	dwOptions |= D3DXMESHOPT_COMPACT | D3DXMESHOPT_ATTRSORT |
	             D3DXMESHOPT_VERTEXCACHE;
	ID3DXMesh* pTempMesh = NULL;

	if (FAILED(pInMesh->Optimize(dwOptions, rgdwAdjacency, NULL, NULL, NULL,
	                             &pTempMesh)))
	{
...
//-----------------------------------------------------------------------------
HRESULT CDXUTXFileMesh::CreateMaterials(LPCWSTR strPath,
                                        IDirect3DDevice9* pd3dDevice, D3DXMATERIAL* d3dxMtrls,
	...
	if (d3dxMtrls && m_dwNumMaterials > 0)
	{
		...
		for (DWORD i = 0; i < m_dwNumMaterials; i++)
		{
			...
			if (d3dxMtrls[i].pTextureFilename)
			{
				...
				MultiByteToWideChar(CP_ACP, 0, d3dxMtrls[i].pTextureFilename, -1,
...
//-----------------------------------------------------------------------------
HRESULT CDXUTXFileMesh::SetVertexDecl(LPDIRECT3DDEVICE9 pd3dDevice,
...
//-----------------------------------------------------------------------------
HRESULT CDXUTXFileMesh::Render(LPDIRECT3DDEVICE9 pd3dDevice,
