...
{
	// Scale to screen
	FLOAT x = -(fScreenPtX - m_Offset.x - m_nWidth / 2) / (m_fRadius * m_nWidth /
	          2);
	FLOAT y = (fScreenPtY - m_Offset.y - m_nHeight / 2) / (m_fRadius * m_nHeight /
	          2);
	FLOAT z = 0.0f;
	FLOAT mag = x * x + y * y;
...
//--------------------------------------------------------------------------------------
D3DXQUATERNION CD3DArcBall::QuatFromBallPoints(const D3DXVECTOR3& vFrom,
...
// Desc:
//--------------------------------------------------------------------------------------
LRESULT CD3DArcBall::HandleMessages(HWND hWnd, UINT uMsg, WPARAM wParam,
                                    LPARAM lParam)
{
...
//--------------------------------------------------------------------------------------
LRESULT CBaseCamera::HandleMessages(HWND hWnd, UINT uMsg, WPARAM wParam,
                                    LPARAM lParam)
{
...
void CBaseCamera::UpdateMouseDelta()
{
	...
	m_vMouseDelta.x = m_vMouseDelta.x * fPercentOfOld + ptCurMouseDelta.x *
	                  fPercentOfNew;
	m_vMouseDelta.y = m_vMouseDelta.y * fPercentOfOld + ptCurMouseDelta.y *
	                  fPercentOfNew;
...
void CBaseCamera::UpdateVelocity(float fElapsedTime)
{
	...
	D3DXVECTOR2 vGamePadRightThumb = D3DXVECTOR2(m_vGamePadRightThumb.x,
	                                 -m_vGamePadRightThumb.z);
...
VOID CFirstPersonCamera::FrameMove(FLOAT fElapsedTime)
{
	...
	D3DXMatrixRotationYawPitchRoll(&mCameraRot, m_fCameraYawAngle,
...
VOID CModelViewerCamera::FrameMove(FLOAT fElapsedTime)
{
	...
	if (m_ViewArcBall.IsBeingDragged() && m_bAttachCameraToModel
	        && !IsKeyDown(m_aKeys[CAM_CONTROLDOWN]))
	{
		...
		D3DXMATRIX mCameraRotDelta = mCameraLastRotInv *
		                             mCameraRot; // local to world matrix
	...
	D3DXMATRIX mTrans;
	D3DXMatrixTranslation(&mTrans, -m_vModelCenter.x, -m_vModelCenter.y,
	                      -m_vModelCenter.z);
...
//--------------------------------------------------------------------------------------
void CModelViewerCamera::SetViewParams(D3DXVECTOR3* pvEyePt,
                                       D3DXVECTOR3* pvLookatPt)
{
...
// Call this from your message proc so this class can handle window messages
//--------------------------------------------------------------------------------------
LRESULT CModelViewerCamera::HandleMessages(HWND hWnd, UINT uMsg, WPARAM wParam,
        LPARAM lParam)
{
...
D3DXHANDLE                  CDXUTDirectionWidget::s_hRenderWith1LightNoTexture =
    NULL;
D3DXHANDLE                  CDXUTDirectionWidget::s_hMaterialDiffuseColor =
    NULL;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDirectionWidget::StaticOnD3D9CreateDevice(IDirect3DDevice9*
        pd3dDevice)
{
	HRESULT hr;
...
#endif
	V_RETURN(D3DXCreateEffect(s_pd3d9Device, g_strBuffer, dwBufferSize, NULL, NULL,
	                          Flags,
	                          NULL, &s_pD3D9Effect, NULL));
	// Save technique handles for use when rendering
	s_hRenderWith1LightNoTexture =
	    s_pD3D9Effect->GetTechniqueByName("RenderWith1LightNoTexture");
	s_hMaterialDiffuseColor = s_pD3D9Effect->GetParameterByName(NULL,
	...
	s_hWorldViewProjection = s_pD3D9Effect->GetParameterByName(NULL,
	...
	V_DX(s_pD3D9Mesh->OptimizeInplace(D3DXMESHOPT_VERTEXCACHE, rgdwAdjacency, NULL,
	                                  NULL, NULL));
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDirectionWidget::OnD3D9ResetDevice(const D3DSURFACE_DESC*
        pBackBufferSurfaceDesc)
{
	m_ArcBall.SetWindow(pBackBufferSurfaceDesc->Width,
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDirectionWidget::OnRender9(D3DXCOLOR color,
                                        ...
                                        const D3DXMATRIX* pmProj, const D3DXVECTOR3* pEyePt)
{
	...
	D3DXMatrixScaling(&mScale, m_fRadius * 0.2f, m_fRadius * 0.2f,
	                  m_fRadius * 0.2f);
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDirectionWidget::StaticOnD3D11CreateDevice(
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDirectionWidget::OnRender11(D3DXCOLOR color,
        const D3DXMATRIX* pmView, const D3DXMATRIX* pmProj,
