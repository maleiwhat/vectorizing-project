...
DWORD                   DXUT_SCREEN_VERTEX::FVF = D3DFVF_XYZRHW | D3DFVF_DIFFUSE
        | D3DFVF_TEX1;
...
DWORD                   DXUT_SCREEN_VERTEX_UNTEX::FVF = D3DFVF_XYZRHW |
        D3DFVF_DIFFUSE;
...
HRESULT CDXUTDialogResourceManager::OnD3D11CreateDevice(
    ID3D11Device* pd3dDevice, ID3D11DeviceContext* pd3d11DeviceContext)
{
	...
	V_RETURN(D3DCompile(g_strUIEffectFile, g_uUIEffectFileSize, "none", NULL, NULL,
	                    "VS", "vs_4_0_level_9_1",
	...
	V_RETURN(D3DCompile(g_strUIEffectFile, g_uUIEffectFileSize, "none", NULL, NULL,
	                    "PSUntex", "ps_4_0_level_9_1",
	                    D3D10_SHADER_ENABLE_BACKWARDS_COMPATIBILITY, 0, &pPSUntexBlob, NULL));
	// Create Shaders
	V_RETURN(pd3dDevice->CreateVertexShader(pVSBlob->GetBufferPointer(),
	                                        pVSBlob->GetBufferSize(), NULL, &m_pVSRenderUI11));
	...
	V_RETURN(pd3dDevice->CreatePixelShader(pPSBlob->GetBufferPointer(),
	                                       pPSBlob->GetBufferSize(), NULL, &m_pPSRenderUI11));
	DXUT_SetDebugName(m_pPSRenderUI11, "CDXUTDialogResourceManager");
	V_RETURN(pd3dDevice->CreatePixelShader(pPSUntexBlob->GetBufferPointer(),
	                                       pPSUntexBlob->GetBufferSize(), NULL, &m_pPSRenderUIUntex11));
	...
	V_RETURN(pd3dDevice->CreateDepthStencilState(&DSDesc,
	         &m_pDepthStencilStateUI11));
	...
	V_RETURN(pd3dDevice->CreateInputLayout(layout, ARRAYSIZE(layout),
	                                       pVSBlob->GetBufferPointer(), pVSBlob->GetBufferSize(), &m_pInputLayout11));
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialogResourceManager::OnD3D11ResizedSwapChain(
...
//--------------------------------------------------------------------------------------
void CDXUTDialogResourceManager::StoreD3D11State(ID3D11DeviceContext*
        pd3dImmediateContext)
{
	pd3dImmediateContext->OMGetDepthStencilState(&m_pDepthStencilStateStored11,
	        &m_StencilRefStored11);
	pd3dImmediateContext->RSGetState(&m_pRasterizerStateStored11);
	pd3dImmediateContext->OMGetBlendState(&m_pBlendStateStored11,
	                                      m_BlendFactorStored11, &m_SampleMaskStored11);
	pd3dImmediateContext->PSGetSamplers(0, 1, &m_pSamplerStateStored11);
}

//--------------------------------------------------------------------------------------
void CDXUTDialogResourceManager::RestoreD3D11State(ID3D11DeviceContext*
        pd3dImmediateContext)
{
	pd3dImmediateContext->OMSetDepthStencilState(m_pDepthStencilStateStored11,
	        m_StencilRefStored11);
	pd3dImmediateContext->RSSetState(m_pRasterizerStateStored11);
	pd3dImmediateContext->OMSetBlendState(m_pBlendStateStored11,
	                                      m_BlendFactorStored11, m_SampleMaskStored11);
...
//--------------------------------------------------------------------------------------
void CDXUTDialogResourceManager::ApplyRenderUI11(ID3D11DeviceContext*
        pd3dImmediateContext)
{
	// Shaders
	...
	pd3dImmediateContext->OMSetBlendState(m_pBlendStateUI11, BlendFactor,
	                                      0xFFFFFFFF);
	pd3dImmediateContext->PSSetSamplers(0, 1, &m_pSamplerStateUI11);
}

//--------------------------------------------------------------------------------------
void CDXUTDialogResourceManager::ApplyRenderUIUntex11(ID3D11DeviceContext*
        pd3dImmediateContext)
{
	// Shaders
	pd3dImmediateContext->VSSetShader(m_pVSRenderUI11, NULL, 0);
...
//--------------------------------------------------------------------------------------
void CDXUTDialogResourceManager::EndSprites11(ID3D11Device* pd3dDevice,
        ID3D11DeviceContext* pd3dImmediateContext)
{
	...
	if (S_OK == pd3dImmediateContext->Map(m_pSpriteBuffer11, 0,
	                                      D3D11_MAP_WRITE_DISCARD, 0, &MappedResource))
	{
		CopyMemory(MappedResource.pData, (void*)m_SpriteVertices.GetData(),
		           SpriteDataBytes);
	...
	UINT Offset = 0;
	pd3dImmediateContext->IASetVertexBuffers(0, 1, &m_pSpriteBuffer11, &Stride,
	        &Offset);
...
HRESULT CDXUTDialog::OnRender9(float fElapsedTime)
{
	...
	BOOL bBackgroundIsVisible = (m_colorTopLeft | m_colorTopRight |
	                             m_colorBottomRight | m_colorBottomLeft) &
	                            0xff000000;
...
HRESULT CDXUTDialog::OnRender11(float fElapsedTime)
{
	...
	if (!m_bMinimized && bBackgroundIsVisible)
	{
		...
		if (SUCCEEDED(pd3dDeviceContext->Map(m_pManager->m_pVBScreenQuad11, 0,
		...
		UINT offset = 0;
		pd3dDeviceContext->IASetVertexBuffers(0, 1, &m_pManager->m_pVBScreenQuad11,
		                                      &stride, &offset);
...
HRESULT CDXUTDialog::AddCheckBox(int ID, LPCWSTR strText, int x, int y,
                                 int width, int height, bool bChecked,
...
HRESULT CDXUTDialog::AddEditBox(int ID, LPCWSTR strText, int x, int y,
                                int width, int height, bool bIsDefault,
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::DrawSprite11(CDXUTElement* pElement, RECT* prcDest,
                                  float fDepth)
{
	// No need to draw fully transparent layers
	if (pElement->TextureColor.Current.a == 0)
...
HRESULT CDXUTDialog::DrawText(LPCWSTR strText, CDXUTElement* pElement,
                              RECT* prcDest, bool bShadow, int nCount, bool bCenter)
{
	if (m_pManager->GetD3D9Device())
	{
	...
	{
		return DrawText11(m_pManager->GetD3D11Device(),
		                  m_pManager->GetD3D11DeviceContext(), strText, pElement, prcDest, bShadow,
		                  nCount, bCenter);
	}
}
                               ...
                               RECT* prcDest, bool bShadow, int nCount)
{
	...
	hr = pFontNode->pFont9->DrawText(m_pManager->m_pSprite, strText, nCount,
	                                 &rcScreen, pElement->dwTextFormat,
	                                 pElement->FontColor.Current);
...
HRESULT InitFont11(ID3D11Device* pd3d11Device, ID3D11InputLayout* pInputLayout)
{
	...
	if (pd3d11Device->GetFeatureLevel() < D3D_FEATURE_LEVEL_10_0)
	{
		...
		V_RETURN(D3DX11CreateShaderResourceViewFromFile(pd3d11Device, str, &dili, NULL,
		         &g_pFont11, &hr));
	}
	else
	{
		V_RETURN(D3DX11CreateShaderResourceViewFromFile(pd3d11Device, str, NULL, NULL,
		         &g_pFont11, &hr));
	}
                    ...
                    float fBBWidth, float fBBHeight, bool bCenter)
{
	...
	if (bCenter)
	{
		...
		float fRectBottom = 1.0f - rcScreen.bottom / fBBHeight;
		fRectBottom = fRectBottom * 2.0f - 1.0f;
		float fcenterx = ((fRectRight - fRectLeft) - (float)NumChars * fGlyphSizeX) *
		                 0.5f;
...
void EndText11(ID3D11Device* pd3dDevice,
               ID3D11DeviceContext* pd3d11DeviceContext)
{
	// ensure our buffer size can hold our sprites
	...
	if (S_OK == pd3d11DeviceContext->Map(g_pFontBuffer11, 0,
	                                     D3D11_MAP_WRITE_DISCARD, 0, &MappedResource))
	{
		CopyMemory(MappedResource.pData, (void*)g_FontVertices.GetData(),
		           FontDataBytes);
	...
	UINT Stride = sizeof(DXUTSpriteVertex);
	UINT Offset = 0;
	pd3d11DeviceContext->IASetVertexBuffers(0, 1, &g_pFontBuffer11, &Stride,
	                                        &Offset);
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::DrawText11(ID3D11Device* pd3dDevice,
                                ID3D11DeviceContext* pd3d11DeviceContext,
                                LPCWSTR strText, CDXUTElement* pElement, RECT* prcDest, bool bShadow,
                                int nCount, bool bCenter)
{
	//HRESULT hr = S_OK;
	...
	D3DXCOLOR vFontColor(pElement->FontColor.Current.r,
	                     pElement->FontColor.Current.g, pElement->FontColor.Current.b, 1.0f);
	DrawText11DXUT(pd3dDevice, pd3d11DeviceContext,
...
void CDXUTDialog::SetBackgroundColors(D3DCOLOR colorTopLeft,
                                      D3DCOLOR colorTopRight, D3DCOLOR colorBottomLeft,
...
HRESULT CDXUTDialogResourceManager::CreateTexture11(UINT iTexture)
{
	...
	if (!pTextureNode->bFileSource)
	{
		if (pTextureNode->nResourceID == 0xFFFF
		        && pTextureNode->hResourceModule == (HMODULE)0xFFFF)
		{
			hr = DXUTCreateGUITextureFromInternalArray11(m_pd3d11Device,
			        &pTextureNode->pTexture11, NULL);
	...
	hr = m_pd3d11Device->CreateShaderResourceView(pTextureNode->pTexture11,
	        &SRVDesc, &pTextureNode->pTexResView11);
	...
	case WM_MOUSEMOVE:
	{
		if (m_bDrag)
		{
			...
			int nMaxFirstItem = m_nEnd - m_nStart - m_nPageSize +
			                    1;  // Largest possible index for first item
...
bool
CDXUTEditBox::s_bHideCaret;   // If true, we don't render the caret.
...
bool CUniBuffer::SetBufferSize(int nNewSize)
{
	...
	int nAllocateSize = (nNewSize == -1
	                     || nNewSize < m_nBufferSize * 2) ? (m_nBufferSize ? m_nBufferSize *
	                             2 : 256) : nNewSize * 2;
