...
typedef void (CALLBACK* LPCREATEINDEXBUFFER9)(IDirect3DDevice9* pDev,
        IDirect3DIndexBuffer9** ppBuffer,
...
typedef void (CALLBACK* LPCREATETEXTUREFROMFILE11)(ID3D11Device* pDev,
        char* szFileName,
        ID3D11ShaderResourceView** ppRV, void* pContext);
typedef void (CALLBACK* LPCREATEVERTEXBUFFER11)(ID3D11Device* pDev,
        ID3D11Buffer** ppBuffer,
        D3D11_BUFFER_DESC BufferDesc, void* pData, void* pContext);
typedef void (CALLBACK* LPCREATEINDEXBUFFER11)(ID3D11Device* pDev,
        ID3D11Buffer** ppBuffer,
        D3D11_BUFFER_DESC BufferDesc, void* pData, void* pContext);
...
protected:
	void                            LoadMaterials(ID3D11Device* pd3dDevice,
	        SDKMESH_MATERIAL* pMaterials,
	        UINT NumMaterials, SDKMESH_CALLBACKS11* pLoaderCallbacks = NULL);

	void                            LoadMaterials(IDirect3DDevice9* pd3dDevice,
	        SDKMESH_MATERIAL* pMaterials,
	...
	//frame manipulation
	void                            TransformBindPoseFrame(UINT iFrame,
	        D3DXMATRIX* pParentWorld);
	void                            TransformFrame(UINT iFrame,
	        D3DXMATRIX* pParentWorld, double fTime);
	void                            TransformFrameAbsolute(UINT iFrame,
	        double fTime);
...
public:
	...
	virtual HRESULT                 Create(ID3D11Device* pDev11, LPCTSTR szFileName,
	                                       bool bCreateAdjacencyIndices =
	                                               false, SDKMESH_CALLBACKS11* pLoaderCallbacks = NULL);
	virtual HRESULT                 Create(IDirect3DDevice9* pDev9,
	                                       LPCTSTR szFileName, bool bCreateAdjacencyIndices =
	                                               false, SDKMESH_CALLBACKS9* pLoaderCallbacks = NULL);
	virtual HRESULT                 Create(ID3D11Device* pDev11, BYTE* pData,
	                                       UINT DataBytes,
	                                       bool bCreateAdjacencyIndices = false, bool bCopyStatic = false,
	                                       SDKMESH_CALLBACKS11* pLoaderCallbacks = NULL);
	virtual HRESULT                 Create(IDirect3DDevice9* pDev9, BYTE* pData,
	                                       UINT DataBytes,
	                                       bool bCreateAdjacencyIndices = false, bool bCopyStatic = false,
	                                       SDKMESH_CALLBACKS9* pLoaderCallbacks = NULL);
	...
	virtual void                    RenderAdjacent(ID3D11DeviceContext*
	        pd3dDeviceContext,
	        UINT iDiffuseSlot = INVALID_SAMPLER_SLOT,
	        UINT iNormalSlot = INVALID_SAMPLER_SLOT,
	        UINT iSpecularSlot = INVALID_SAMPLER_SLOT);
	...
	static D3D11_PRIMITIVE_TOPOLOGY GetPrimitiveType11(SDKMESH_PRIMITIVE_TYPE
	        PrimType);
	...
	static D3DPRIMITIVETYPE         GetPrimitiveType9(SDKMESH_PRIMITIVE_TYPE
	        PrimType);
	...
	const D3DXMATRIX*               GetMeshInfluenceMatrix(UINT iMesh,
	        UINT iInfluence);
	...
	bool                            GetAnimationProperties(UINT* pNumKeys,
	        FLOAT* pFrameTime);
};
...
public:
	...
	HRESULT     Create(LPDIRECT3DDEVICE9 pd3dDevice, ID3DXMesh* pInMesh,
