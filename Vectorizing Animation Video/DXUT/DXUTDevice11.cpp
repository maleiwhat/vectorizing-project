...
//--------------------------------------------------------------------------------------
extern void DXUTGetCallbackD3D11DeviceAcceptable(
    LPDXUTCALLBACKISD3D11DEVICEACCEPTABLE* ppCallbackIsDeviceAcceptable,
    void** ppUserContext);
...
//--------------------------------------------------------------------------------------
CD3D11Enumeration* WINAPI DXUTGetD3D11Enumeration(bool bForceEnumerate,
        bool bEnumerateAllAdapterFormats, D3D_FEATURE_LEVEL forceFL)
{
	...
	if (g_pDXUTD3D11Enumeration && (!g_pDXUTD3D11Enumeration->HasEnumerated()
	                                || bForceEnumerate))
	{
		g_pDXUTD3D11Enumeration->SetEnumerateAllAdapterFormats(
		    bEnumerateAllAdapterFormats);
		LPDXUTCALLBACKISD3D11DEVICEACCEPTABLE pCallbackIsDeviceAcceptable;
		void* pUserContext;
		DXUTGetCallbackD3D11DeviceAcceptable(&pCallbackIsDeviceAcceptable,
		                                     &pUserContext);
...
//--------------------------------------------------------------------------------------
HRESULT CD3D11Enumeration::Enumerate(LPDXUTCALLBACKISD3D11DEVICEACCEPTABLE
                                     ...
                                     void* pIsD3D11DeviceAcceptableFuncUserContext)
{
	...
	m_pIsD3D11DeviceAcceptableFuncUserContext =
	    pIsD3D11DeviceAcceptableFuncUserContext;
	ClearAdapterInfoList();
	...
	for (int i = 0; i < m_AdapterInfoList.GetSize(); i++)
	{
		...
		wcscpy_s(pAdapterInfo->szUniqueDescription, 100,
		...
		if (!bUniqueDesc)
		{
			...
			wcscat_s(pAdapterInfo->szUniqueDescription, DXGI_MAX_DEVICE_IDENTIFIER_STRING,
			         sz);
		}
	}
...
//--------------------------------------------------------------------------------------
HRESULT CD3D11Enumeration::EnumerateDisplayModes(CD3D11EnumOutputInfo*
        pOutputInfo)
{
	HRESULT hr = S_OK;
	...
	int allowedAdapterFormatArrayCount = sizeof(allowedAdapterFormatArray) / sizeof(
	...
	for (int f = 0; f < allowedAdapterFormatArrayCount; ++f)
	{
		...
		if (DXGI_ERROR_NOT_FOUND == hr)
		{
			...
			break;
		         ...
		         && RemoteMode == allowedAdapterFormatArray[f])
		{
...
HRESULT CD3D11Enumeration::EnumerateDevices(CD3D11EnumAdapterInfo* pAdapterInfo)
{
	...
	for (UINT iDeviceType = 0; iDeviceType < devTypeArrayCount; iDeviceType++)
	{
		...
		pd3dDevice->CheckFeatureSupport(D3D11_FEATURE_D3D10_X_HARDWARE_OPTIONS, &ho,
		                                sizeof(ho));
		pDeviceInfo->ComputeShaders_Plus_RawAndStructuredBuffers_Via_Shader_4_x =
...
HRESULT CD3D11Enumeration::EnumerateDeviceCombosNoAdapter(
    CD3D11EnumAdapterInfo* pAdapterInfo)
{
...
//--------------------------------------------------------------------------------------
HRESULT CD3D11Enumeration::EnumerateDeviceCombos(IDXGIFactory1* pFactory,
        CD3D11EnumAdapterInfo* pAdapterInfo)
{
	// Iterate through each combination of device driver type, output,
	// adapter format, and backbuffer format to build the adapter's device combo list.
...
//--------------------------------------------------------------------------------------
void CD3D11Enumeration::SetEnumerateAllAdapterFormats(bool
        bEnumerateAllAdapterFormats)
{
...
//--------------------------------------------------------------------------------------
void CD3D11Enumeration::BuildMultiSampleQualityList(DXGI_FORMAT fmt,
...
//--------------------------------------------------------------------------------------
CD3D11EnumDeviceInfo* CD3D11Enumeration::GetDeviceInfo(UINT AdapterOrdinal,
...
//--------------------------------------------------------------------------------------
CD3D11EnumOutputInfo* CD3D11Enumeration::GetOutputInfo(UINT AdapterOrdinal,
        UINT Output)
{
	CD3D11EnumAdapterInfo* pAdapterInfo = GetAdapterInfo(AdapterOrdinal);
...
//--------------------------------------------------------------------------------------
CD3D11EnumDeviceSettingsCombo* CD3D11Enumeration::GetDeviceSettingsCombo(
    UINT AdapterOrdinal,
	...
	if (pAdapterInfo)
	{
		        ...
		        iDeviceCombo < pAdapterInfo->deviceSettingsComboList.GetSize(); iDeviceCombo++)
		{
			CD3D11EnumDeviceSettingsCombo* pDeviceSettingsCombo =
			    pAdapterInfo->deviceSettingsComboList.GetAt(
			        iDeviceCombo);
...
//--------------------------------------------------------------------------------------
float DXUTRankD3D11DeviceCombo(CD3D11EnumDeviceSettingsCombo*
                              ...
                              )
{
	...
	else if (pDeviceSettingsCombo->DeviceType == D3D_DRIVER_TYPE_WARP
	...
	for (int idm = 0; pDeviceSettingsCombo->pOutputInfo != NULL
	        && idm < pDeviceSettingsCombo->pOutputInfo->displayModeList.GetSize()
	        && !bResolutionFound; idm++)
	{
...
//--------------------------------------------------------------------------------------
HRESULT WINAPI DXUTGetD3D11AdapterDisplayMode(UINT AdapterOrdinal, UINT nOutput,
        DXGI_MODE_DESC* pModeDesc)
{
	...
	CD3D11EnumOutputInfo* pOutputInfo = pD3DEnum->GetOutputInfo(AdapterOrdinal,
	                                    nOutput);
