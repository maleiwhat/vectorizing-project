...
public:
	...
	void                            OnBegin(int nX,
	                                        int nY);    // start the rotation (pass current mouse position)
	void                            OnMove(int nX,
	                                       int nY);     // continue the rotation (pass current mouse position)
	void                            OnEnd();                    // end the rotation

	// Or call this to automatically handle left, middle, right buttons
	LRESULT                         HandleMessages(HWND hWnd, UINT uMsg,
...
protected:
	...
	D3DXVECTOR3                     ScreenToVector(float fScreenPtX,
	        float fScreenPtY);
};
...
public:
	...
	virtual void                Reset();
	virtual void                SetViewParams(D3DXVECTOR3* pvEyePt,
	...
	void                        SetDrag(bool bMovementDrag,
	...
	void                        SetClipToBoundary(bool bClipToBoundary,
	...
	void                        SetScalers(FLOAT fRotationScaler = 0.01f,
...
protected:
	...
	void                        GetInput(bool bGetKeyboardInput,
	                                     bool bGetMouseInput, bool bGetGamepadInput,
	...
	DXUT_GAMEPAD
	m_GamePad[DXUT_MAX_CONTROLLERS];  // XInput controller state
...
public:
	...
	void            SetRadius(float fDefaultRadius = 5.0f, float fMinRadius = 1.0f,
	                          float fMaxRadius = FLT_MAX)
	{
		m_fDefaultRadius = m_fRadius = fDefaultRadius; m_fMinRadius = fMinRadius;
...
public:
	...
	HRESULT                 OnD3D9ResetDevice(const D3DSURFACE_DESC*
	        pBackBufferSurfaceDesc);
	HRESULT                 OnRender9(D3DXCOLOR color, const D3DXMATRIX* pmView,
	                                  const D3DXMATRIX* pmProj,
	                                  const D3DXVECTOR3* pEyePt);
	LRESULT                 HandleMessages(HWND hWnd, UINT uMsg, WPARAM wParam,
	                                       LPARAM lParam);
	static void WINAPI      StaticOnD3D9LostDevice();
	static void WINAPI      StaticOnD3D9DestroyDevice();

	static HRESULT WINAPI   StaticOnD3D11CreateDevice(ID3D11Device* pd3dDevice,
	        ID3D11DeviceContext* pd3dImmediateContext);
	HRESULT                 OnRender11(D3DXCOLOR color, const D3DXMATRIX* pmView,
	                                   const D3DXMATRIX* pmProj,
	                                   const D3DXVECTOR3* pEyePt);
