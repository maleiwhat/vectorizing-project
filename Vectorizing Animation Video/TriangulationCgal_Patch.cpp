...
void	TriangulationCgal_Patch::mark_domains(Triangulation& cdt)
{
	...
	while (! border.empty())
	{
		...
		domain = e.first->vertex(1)->info().nesting_level >= 0 ? e.first->vertex(1)->info().nesting_level : domain;
		domain = e.first->vertex(2)->info().nesting_level >= 0 ? e.first->vertex(2)->info().nesting_level : domain;

		if (n->info().nesting_level == -1 && domain >= 0)
		{
